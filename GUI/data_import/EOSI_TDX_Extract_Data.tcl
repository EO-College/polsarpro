...
global MultiLookSubSamp SubSampCol SubSampRow MultiLookCol MultiLookRow PSPMemory TMPMemoryAllocError
...
if {$OpenDirFile == 0} {
    ...
    if {"$VarWarning"=="ok"} {
        ...
        if {$TestVarError == "ok"} {
            ...
            TextEditorRunTrace "Process The Function Soft/data_import/tandemx_convert_ssc_dual.exe" "k"
            ...
            if {"$ErrorMessage" == ""} {
                ...
                TextEditorRunTrace "Arguments: -id \x22$RGBDirInput\x22 -of \x22$RGBFileOutput\x22 -iodf SPP -ofr 0 -ofc 0 -fnr $NligFullSize -fnc $NcolFullSize -mem $PSPMemory -errf \x22$TMPMemoryAllocError\x22 -rgbf RGB1 $MaskCmd -auto 1" "k"
                set f [ open "| Soft/bmp_process/create_rgb_file_SPPIPPC2.exe -id \x22$RGBDirInput\x22 -of \x22$RGBFileOutput\x22 -iodf SPP -ofr 0 -ofc 0 -fnr $NligFullSize -fnc $NcolFullSize -mem $PSPMemory -errf \x22$TMPMemoryAllocError\x22 -rgbf RGB1 $MaskCmd -auto 1" r]
                PsPprogressBar $f
                TextEditorRunTrace "Check RunTime Errors" "r"
                CheckRunTimeError
                WidgetHideTop28; TextEditorRunTrace "Close Window Message" "b"
        ...
        if {"$VarWarning"=="no"} {Window hide $widget(Toplevel437); TextEditorRunTrace "Close Window $ActiveProgram Extract Data" "b"}
        }
    ...
    if {$configerror == "false"} {
        Window hide $widget(Toplevel437); TextEditorRunTrace "Close Window $ActiveProgram Extract Data" "b"
        MenuRAZ
        CloseAllWidget
