...
proc vTclWindow.top428 {base} {
    ...
    wm overrideredirect $top 0
    wm resizable $top 1 1
    wm title $top "SNAP - Geocode Parameter"
    ...
    button $site_6_0.but71 \
        \
        -command {global DirName DataDir SNAPDirOutput
global VarWarning WarningMessage WarningMessage2

set SNAPOutputDirTmp $SNAPDirOutput
set DirName ""
OpenDir $DataDir "DATA OUTPUT DIRECTORY"
if {$DirName != "" } {
    set SNAPDirOutput $DirName
    } else {
    set SNAPDirOutput $SNAPOutputDirTmp
    }} \
    ...
    button $site_5_0.but71 \
        \
        -command {global FileName SNAPDirInput SNAPSensorPol SNAPParameterFile
...
set FileName ""
OpenFile $SNAPDirInput $types "INPUT PARAMETER FILE"
set SNAPParameterFile $FileName} \
...
$widget(Button428_5) configure -state normal} \
    ...
    button $site_3_0.but93 \
        -background #ffff00 \
        -command {global DataDir OpenDirFile SNAPDirInput SNAPDirOutput SNAPOutputDir
...
if {$TestVarError == "ok"} {
    ...
    if [file isdirectory $DirNameCreate] {
        set SNAPParameterInput [file rootname $SNAPParameterFile]
        set SNAPParameterOutput $SNAPDirOutput
        append SNAPParameterOutput "/"
        append SNAPParameterOutput [file tail $SNAPParameterFile]
        if [file exists $SNAPParameterOutput] { DeleteFile $SNAPParameterOutput }
        set SNAPFileHdr "$SNAPParameterOutput.hdr"
        if [file exists $SNAPFileHdr] { DeleteFile $SNAPFileHdr }
        set VarWarning "ok"
        } else {
    ...
    if {"$VarWarning"=="ok"} {
        ...
        set SNAPOutputDir $SNAPDirOutput
        set SNAPParameterOutput $SNAPDirOutput; append SNAPParameterOutput "/"; append SNAPParameterOutput [file tail $SNAPParameterFile]
    ...
    button $site_3_0.but23 \
        -background #ff8000 -command {HelpPdfEdit "Help/SNAPParameter.pdf"} \
    ...
    button $site_3_0.but24 \
        -background #ffff00 \
        -command {global OpenDirFile TMPDir SNAPDirOutput SNAPParameterOutput SNAPState
