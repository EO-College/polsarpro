...
#############################################################################
## Procedure:  NESTMatrix_C2

proc ::NESTMatrix_C2 {} {
global NESTDirInput NESTOutputDir NESTDirOutput TMPDirSession TMPDir PSPMemory TMPMemoryAllocError 
global NESTSaveDEM NESTSaveIncAng NESTSaveProjIncAng
global NESTFileHdr NESTSensor MapInfoActive MapInfoMapInfo MapInfoUnit NESTParameterFile
...
global ConfigFile ErrorMessage VarError NligFullSize NcolFullSize PolarType PSPViewGimpBMP
    ...
    set NESTState "1"
    set NESTParameterFile "$NESTDirInput/C11.bin"
    set NESTParameterOutput "$NESTDirOutput/C11.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/C12_real.bin"
    set NESTParameterOutput "$NESTDirOutput/C12_real.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/C12_imag.bin"
    set NESTParameterOutput "$NESTDirOutput/C12_imag.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/C22.bin"
    set NESTParameterOutput "$NESTDirOutput/C22.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTState "2"

    if [file exists "$TMPDir/target/config.txt"] {
        set ConfigFile "$NESTDirOutput/config.txt"
        WaitUntilCreated $ConfigFile
        set ErrorMessage ""
        LoadConfig
        if {"$ErrorMessage" == ""} {
            EnviWriteConfigC $NESTDirOutput $NligFullSize $NcolFullSize

            if {$NESTSaveDEM == "1"} {
                set NESTSaveFileInput "$TMPDir/target/elevation.bin"
                set NESTSaveFileOutput "$NESTDirOutput/elevation.bin"
                if [file exists $NESTSaveFileInput ] { 
                    CopyFile $NESTSaveFileInput $NESTSaveFileOutput
                    EnviWriteConfig $NESTSaveFileOutput $NligFullSize $NcolFullSize 4
                    }
                }
            if {$NESTSaveIncAng == "1"} {
                set NESTSaveFileInput "$TMPDir/target/incidenceAngle.bin"
                set NESTSaveFileOutput "$NESTDirOutput/incidenceAngle.bin"
                if [file exists $NESTSaveFileInput ] { 
                    CopyFile $NESTSaveFileInput $NESTSaveFileOutput
                    EnviWriteConfig $NESTSaveFileOutput $NligFullSize $NcolFullSize 4
                    }
                }
            if {$NESTSaveProjIncAng == "1"} {
                set NESTSaveFileInput "$TMPDir/target/projectedIncidenceAngle.bin"
                set NESTSaveFileOutput "$NESTDirOutput/projectedIncidenceAngle.bin"
                if [file exists $NESTSaveFileInput ] { 
                    CopyFile $NESTSaveFileInput $NESTSaveFileOutput
                    EnviWriteConfig $NESTSaveFileOutput $NligFullSize $NcolFullSize 4
                    }
            ...
            set DataDir $NESTOutputDir
    
            set RGBDirInput $NESTDirOutput
            set RGBDirOutput $NESTDirOutput
...
global ConfigFile ErrorMessage VarError NligFullSize NcolFullSize PolarType PSPViewGimpBMP
    ...
    set NESTState "1"
    set NESTParameterFile "$NESTDirInput/T11.bin"
    set NESTParameterOutput "$NESTDirOutput/T11.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T12_real.bin"
    set NESTParameterOutput "$NESTDirOutput/T12_real.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T12_imag.bin"
    set NESTParameterOutput "$NESTDirOutput/T12_imag.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T13_real.bin"
    set NESTParameterOutput "$NESTDirOutput/T13_real.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T13_imag.bin"
    set NESTParameterOutput "$NESTDirOutput/T13_imag.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T22.bin"
    set NESTParameterOutput "$NESTDirOutput/T22.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T23_real.bin"
    set NESTParameterOutput "$NESTDirOutput/T23_real.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T23_imag.bin"
    set NESTParameterOutput "$NESTDirOutput/T23_imag.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput

    set NESTParameterFile "$NESTDirInput/T33.bin"
    set NESTParameterOutput "$NESTDirOutput/T33.bin"
    NESTBatchProcess $NESTParameterFile $NESTParameterOutput
    ...
    if [file exists "$TMPDir/target/config.txt"] {
        ...
        LoadConfig
        if {"$ErrorMessage" == ""} {
            EnviWriteConfigT $NESTDirOutput $NligFullSize $NcolFullSize
...
proc vTclWindow.top427 {base} {
    ...
    wm overrideredirect $top 0
    wm resizable $top 1 1
    wm title $top "NEST - Geocode Matrix"
    ...
    entry $site_6_0.cpd69 \
        -background white -disabledbackground #ffffff \
        -disabledforeground #0000ff -foreground #0000ff -state disabled \
        -textvariable NESTOutputSubDir -width 3 
    ...
    button $site_6_0.but71 \
        \
        -command {global DirName DataDir NESTOutputDir
global VarWarning WarningMessage WarningMessage2

set NESTOutputDirTmp $NESTOutputDir
set DirName ""
...
if {$DirName != "" } {
    ...
    if {"$VarWarning"=="ok"} {
        set NESTOutputDir $DirName
        } else {
        set NESTOutputDir $NESTOutputDirTmp
        }
    } else {
    set NESTOutputDir $NESTOutputDirTmp
    }} \
...
$widget(Button427_5) configure -state normal} \
    ...
    button $site_3_0.but93 \
        -background #ffff00 \
        -command {global DataDir OpenDirFile NESTDirOutput NESTOutputDir NESTOutputSubDir
...
global NESTSensorPol NESTSensor NESTTerrain TMPDir NESTPixelSize NESTLeaderFile
...
if {$TestVarError == "ok"} {
    ...
    set NESTDirOutput $NESTOutputDir 
    if {$NESTOutputSubDir != ""} {append NESTDirOutput "/$NESTOutputSubDir"}
    ...
    if {"$VarWarning"=="ok"} {
        ...
        set NESTState "1"
        if {$NESTSensor == "RS2"} {
            if {$NESTSensorPol == "C2"} { NESTMatrix_C2 }
            if {$NESTSensorPol == "T3"} { NESTMatrix_T3 }     
            }
        if {$NESTSensor == "TSX"} {
            if {$NESTSensorPol == "C2"} { NESTMatrix_C2 }
            if {$NESTSensorPol == "T3"} { NESTMatrix_T3 }     
        ...
        set NESTState "2"
        Window hide $widget(Toplevel427); TextEditorRunTrace "Close Window NEST - Geocode Matrix" "b"
    ...
    button $site_3_0.but23 \
        -background #ff8000 \
        -command {HelpPdfEdit "Help/NESTMatrix.pdf"} \
    ...
    button $site_3_0.but24 \
        -background #ffff00 \
        -command {global OpenDirFile TMPDir NESTTerrain NESTOutputDir NESTState
