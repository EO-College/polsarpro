...
File   : quicklook_rawbinary_C4_ModPha.c
...
Description :  Creation of a QuickLook Pauli RGB BMP file of
        Raw Binary Data Files (Real-Imag data type)
with
Blue = 10log(T11)
Green = 10log(T33)
Red = 10log(T22)
...
/******************************************************************************/
  ...
  printf("TYPE: quicklook_rawbinary_C4_ModPha FileInput1 FileInput2\n");
  printf("FileInput3 FileInput4 FileInput5 FileInput6 FileInput7 FileInput8\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
    if (IEEE == 0) {
     ...
     fread(&M_in[C14m][0], sizeof(float), Ncol, in_file[C14m]);
     fread(&M_in[C14p][0], sizeof(float), Ncol, in_file[C14p]);
     fread(&M_in[C22][0], sizeof(float), Ncol, in_file[C22]);
     fread(&M_in[C23m][0], sizeof(float), Ncol, in_file[C23m]);
     fread(&M_in[C23p][0], sizeof(float), Ncol, in_file[C23p]);
     fread(&M_in[C33][0], sizeof(float), Ncol, in_file[C33]);
     fread(&M_in[C44][0], sizeof(float), Ncol, in_file[C44]);
     }
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C14m]);pc[2] = getc(in_file[C14m]);
        pc[1] = getc(in_file[C14m]);pc[0] = getc(in_file[C14m]);
        M_in[C14m][col] = fl1;

        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C14p]);pc[2] = getc(in_file[C14p]);
        pc[1] = getc(in_file[C14p]);pc[0] = getc(in_file[C14p]);
        M_in[C14p][col] = fl1;
        ...
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C23m]);pc[2] = getc(in_file[C23m]);
        pc[1] = getc(in_file[C23m]);pc[0] = getc(in_file[C23m]);
        M_in[C23m][col] = fl1;

        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C23p]);pc[2] = getc(in_file[C23p]);
        pc[1] = getc(in_file[C23p]);pc[0] = getc(in_file[C23p]);
        M_in[C23p][col] = fl1;
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    M_out[T11][lig][col] = fabs(M_in[C11][ind] + 2 * M_in[C14m][ind]*cos(M_in[C14p][ind]) + M_in[C44][ind]) / 2.;
    M_out[T22][lig][col] = fabs(M_in[C11][ind] - 2 * M_in[C14m][ind]*cos(M_in[C14p][ind]) + M_in[C44][ind]) / 2.;
    M_out[T33][lig][col] = fabs(M_in[C22][ind] + 2 * M_in[C23m][ind]*cos(M_in[C23p][ind]) + M_in[C33][ind]) / 2.;
