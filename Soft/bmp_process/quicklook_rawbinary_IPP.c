...
File   : quicklook_rawbinary_IPP.c
...
/******************************************************************************/
  ...
  printf("TYPE: quicklook_rawbinary_IPP FileInput1 FileInput2\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (col = 0; col < Ncolfin; col++) {
    ind = 2 * col * Coeff;
    M_out[T11][lig][col] = fabs(M_in[ch1][ind]);
    M_out[T22][lig][col] = fabs(M_in[ch2][ind]);
    M_out[T33][lig][col] =  fabs(M_in[ch1][ind] - M_in[ch2][ind]);
