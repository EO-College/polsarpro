...
File   : quicklook_rawbinary_C3_ModPha.c
...
/* CONSTANTS  */
#define Npolar_in  5  /* nb of input/output files */
...
/* LOCAL VARIABLES */
  ...
  char File1[FilePathLength],File2[FilePathLength],File3[FilePathLength],File4[FilePathLength],File5[FilePathLength];
...
/******************************************************************************/
  ...
  printf("TYPE: quicklook_rawbinary_C3_ModPha FileInput1 FileInput2 FileInput3 FileInput4 FileInput5\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
    if (IEEE == 0) {
     ...
     fread(&M_in[C13m][0], sizeof(float), Ncol, in_file[C13m]);
     fread(&M_in[C13p][0], sizeof(float), Ncol, in_file[C13p]);
    ...
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C13m]);pc[2] = getc(in_file[C13m]);
        pc[1] = getc(in_file[C13m]);pc[0] = getc(in_file[C13m]);
        v = &fl2;pc = (char *) v;
        pc[3] = getc(in_file[C13p]);pc[2] = getc(in_file[C13p]);
        pc[1] = getc(in_file[C13p]);pc[0] = getc(in_file[C13p]);
        M_in[C13m][col] = fl1;M_in[C13p][col] = fl2;
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    M_out[T11][lig][col] = fabs(M_in[C11][ind] + 2 * M_in[C13m][ind]*cos(M_in[C13p][ind]) + M_in[C33][ind]) / 2.;
    M_out[T22][lig][col] = fabs(M_in[C11][ind] - 2 * M_in[C13m][ind]*cos(M_in[C13p][ind]) + M_in[C33][ind]) / 2.;
    M_out[T33][lig][col] = fabs(M_in[C22][ind]);
