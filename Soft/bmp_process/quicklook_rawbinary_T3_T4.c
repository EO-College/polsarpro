...
File   : quicklook_rawbinary_T3_T4.c
...
Description :  Creation of a QuickLook Pauli RGB BMP file of
        Raw Binary Data Files (Cmplx, Real-Imag or Mod-Phase data type)
with
Blue = 10log(T11)
Green = 10log(T33)
Red = 10log(T22)
...
/* LOCAL VARIABLES */
  ...
  char File1[FilePathLength],File2[FilePathLength],File3[FilePathLength];
...
/******************************************************************************/
  ...
  printf("TYPE: quicklook_rawbinary_T3_T4 FileInput1 FileInput2 FileInput3\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (col = 0; col < Ncolfin; col++) {
    ind = col * Coeff;
    M_out[T11][lig][col] = fabs(M_in[T11][ind]);
    M_out[T22][lig][col] = fabs(M_in[T22][ind]);
    M_out[T33][lig][col] = fabs(M_in[T33][ind]);
