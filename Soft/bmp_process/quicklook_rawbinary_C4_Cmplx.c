...
File   : quicklook_rawbinary_C4_Cmplx.c
...
Description :  Creation of a QuickLook Puali RGB BMP file of
        Raw Binary Data Files (Cmplx data type)
with
Blue = 10log(T11)
Green = 10log(T33)
Red = 10log(T22)
...
/******************************************************************************/
  ...
  IEEE = atoi(argv[10]);
  Coeff = atoi(argv[11]);
  strcpy(FileOutput, argv[12]);
  } else {
  printf("TYPE: quicklook_rawbinary_C4_Cmplx FileInput1 FileInput2\n");
  printf("FileInput3 FileInput4 FileInput5 FileInput6\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
    if (IEEE == 0) {
     ...
     fread(&M_in[C14][0], sizeof(float), 2 * Ncol, in_file[C14]);
     fread(&M_in[C22][0], sizeof(float), Ncol, in_file[C22]);
     fread(&M_in[C23][0], sizeof(float), 2 * Ncol, in_file[C23]);
    ...
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        pc[3] = getc(in_file[C14]);pc[2] = getc(in_file[C14]);
        pc[1] = getc(in_file[C14]);pc[0] = getc(in_file[C14]);
        M_in[C14][2 * col] = fl1;M_in[C14][2 * col + 1] = fl2;
        ...
        pc[3] = getc(in_file[C23]);pc[2] = getc(in_file[C23]);
        pc[1] = getc(in_file[C23]);pc[0] = getc(in_file[C23]);
        M_in[C23][2 * col] = fl1;M_in[C23][2 * col + 1] = fl2;
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    M_out[T11][lig][col] = fabs(M_in[C11][ind] + 2 * M_in[C14][2*ind] + M_in[C44][ind]) / 2.;
    M_out[T22][lig][col] = fabs(M_in[C11][ind] - 2 * M_in[C14][2*ind] + M_in[C44][ind]) / 2.;
    M_out[T33][lig][col] = fabs(M_in[C22][ind] + 2 * M_in[C23][2*ind] + M_in[C33][ind]) / 2.;
