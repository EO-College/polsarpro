...
File   : quicklook_rawbinary_C4_RealImag.c
...
/******************************************************************************/
  ...
  printf("TYPE: quicklook_rawbinary_C4_RealImag FileInput1 FileInput2\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
    if (IEEE == 0) {
     ...
     fread(&M_in[C14][0], sizeof(float), Ncol, in_file[C14]);
     fread(&M_in[C22][0], sizeof(float), Ncol, in_file[C22]);
     fread(&M_in[C23][0], sizeof(float), Ncol, in_file[C23]);
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    M_out[T11][lig][col] = fabs(M_in[C11][ind] + 2 * M_in[C14][ind] + M_in[C44][ind]) / 2.;
    M_out[T22][lig][col] = fabs(M_in[C11][ind] - 2 * M_in[C14][ind] + M_in[C44][ind]) / 2.;
    M_out[T33][lig][col] = fabs(M_in[C22][ind] + 2 * M_in[C23][ind] + M_in[C33][ind]) / 2.;
