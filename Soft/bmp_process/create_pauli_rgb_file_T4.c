...
File   : create_pauli_rgb_file_T4.c
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {  
        ...
        if (strcmp(RGBFormat,"RGB1") == 0) {
          if (MasterSlave == 1) datatmp[Npts] =  0.5*fabs(M_in[T411][lig][col]+M_in[T422][lig][col]+2.*M_in[T412_re][lig][col]);
          if (MasterSlave == 2) datatmp[Npts] =  0.5*fabs(M_in[T433][lig][col]+M_in[T444][lig][col]+2.*M_in[T434_re][lig][col]);
          }
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {  
        Npts++;
        if (MasterSlave == 1) datatmp[Npts] =  fabs(2.*M_in[T422][lig][col]);
        if (MasterSlave == 2) datatmp[Npts] =  fabs(2.*M_in[T444][lig][col]);
        if (datatmp[Npts] > eps) datatmp[Npts] = 10. * log10(datatmp[Npts]);
        else Npts--;
        } /* valid */
      } /*col*/
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NligBlock[Nb] - 1 - lig][col] == 1.) {  
        if (strcmp(RGBFormat,"RGB1") == 0) {
          if (MasterSlave == 1) xx =  0.5*fabs(M_in[T411][NligBlock[Nb] - 1 - lig][col]+M_in[T422][NligBlock[Nb] - 1 - lig][col]+2.*M_in[T412_re][NligBlock[Nb] - 1 - lig][col]);
          if (MasterSlave == 2) xx =  0.5*fabs(M_in[T433][NligBlock[Nb] - 1 - lig][col]+M_in[T444][NligBlock[Nb] - 1 - lig][col]+2.*M_in[T434_re][NligBlock[Nb] - 1 - lig][col]);
          }
        if (strcmp(RGBFormat,"RGB2") == 0) {
          if (MasterSlave == 1) xx =  0.5*fabs(M_in[T411][NligBlock[Nb] - 1 - lig][col]+M_in[T422][NligBlock[Nb] - 1 - lig][col]-2.*M_in[T412_re][NligBlock[Nb] - 1 - lig][col]);
          if (MasterSlave == 2) xx =  0.5*fabs(M_in[T433][NligBlock[Nb] - 1 - lig][col]+M_in[T444][NligBlock[Nb] - 1 - lig][col]-2.*M_in[T434_re][NligBlock[Nb] - 1 - lig][col]);
        ...
        if (MasterSlave == 1) xx =  fabs(2.*M_in[T422][NligBlock[Nb] - 1 - lig][col]);
        if (MasterSlave == 2) xx =  fabs(2.*M_in[T444][NligBlock[Nb] - 1 - lig][col]);
        if (xx <= eps) xx = eps;
        xx = 10 * log10(xx);
