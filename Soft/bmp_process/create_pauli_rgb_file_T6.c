...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {  
        Npts++;
        if (MasterSlave == 1) datatmp[Npts] =  fabs(M_in[T611][lig][col]);
        if (MasterSlave == 2) datatmp[Npts] =  fabs(M_in[T644][lig][col]);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {  
        Npts++;
        if (MasterSlave == 1) datatmp[Npts] =  fabs(M_in[T622][lig][col]);
        if (MasterSlave == 2) datatmp[Npts] =  fabs(M_in[T655][lig][col]);
        if (datatmp[Npts] > eps) datatmp[Npts] = 10. * log10(datatmp[Npts]);
        else Npts--;
        } /* valid */
      } /*col*/
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {  
        Npts++;
        if (MasterSlave == 1) datatmp[Npts] =  fabs(M_in[T633][lig][col]);
        if (MasterSlave == 2) datatmp[Npts] =  fabs(M_in[T666][lig][col]);
        if (datatmp[Npts] > eps) datatmp[Npts] = 10. * log10(datatmp[Npts]);
        else Npts--;
        } /* valid */
      } /*col*/
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NligBlock[Nb] - 1 - lig][col] == 1.) {  
        if (MasterSlave == 1) xx = fabs(M_in[T611][NligBlock[Nb] - 1 - lig][col]);
        if (MasterSlave == 2) xx = fabs(M_in[T644][NligBlock[Nb] - 1 - lig][col]);
        if (xx <= eps) xx = eps;
        xx = 10 * log10(xx);
        ...
        if (MasterSlave == 1) xx = fabs(M_in[T633][NligBlock[Nb] - 1 - lig][col]);
        if (MasterSlave == 2) xx = fabs(M_in[T666][NligBlock[Nb] - 1 - lig][col]);
        if (xx <= eps) xx = eps;
        xx = 10 * log10(xx);
        ...
        if (MasterSlave == 1) xx = fabs(M_in[T622][NligBlock[Nb] - 1 - lig][col]);
        if (MasterSlave == 2) xx = fabs(M_in[T655][NligBlock[Nb] - 1 - lig][col]);
        if (xx <= eps) xx = eps;
        xx = 10 * log10(xx);
