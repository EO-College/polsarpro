...
File   : create_sinclair_rgb_cce_file.c
...
strcpy(UsageHelp,"\ncreate_sinclair_rgb_cce_file.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (strcmp(PolTypeIn,"S2")==0) {
    ...
    read_block_S2_noavg(in_datafile, M_in, "C3", 9, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, 1, 1, 0, Off_col, Ncol);
    for (Np = 0; Np < NpolarIn; Np++)
      fseek(in_datafile[Np], -NligBlock[Nb]*2*Ncol*sizeof(float), SEEK_CUR);
