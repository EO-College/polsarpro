...
/*******************************************************************/
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++)
      {
      if (my_isfinite(bufferdata[2 * (col + Off_col)]) == 0) bufferdata[2 * (col + Off_col)] = eps;
      if (my_isfinite(bufferdata[2 * (col + Off_col) + 1]) == 0) bufferdata[2 * (col + Off_col)+1] = eps;
      }
