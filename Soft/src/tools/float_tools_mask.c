...
Description :  Float data file processing - Apply Mask
...
/* READ INPUT DATA FILE AND CREATE DATATMP */
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      bufferdata[col] = bufferdata[col + Off_col] * buffermask[col + Off_col];
      }
