...
Description :  Float data file processing - Extract ROI
...
/* LOCAL VARIABLES */
  ...
  char FileInput[FilePathLength], FileOutput[FilePathLength], FileROI[FilePathLength];
...
if(argc < 19) {
  ...
  get_commandline_prm(argc,argv,"-rf",str_cmd_prm,FileROI,1,UsageHelp);
...
********************************************************************/
  ...
  buffermask = vector_float(Sub_Ncol);
...
/******************************************************************************/
  ...
  if ((filemask = fopen(FileROI, "rb")) == NULL)
  edit_error("Could not open input file : ", FileROI);
  rewind(filemask);
...
/* READ INPUT DATA FILE AND CREATE DATATMP */
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    fread(&bufferdata[0], sizeof(float), 2*Ncol, fileinput);
    fread(&buffermask[0], sizeof(float), Sub_Ncol, filemask);
      for (col = 0; col < Sub_Ncol; col++) {
        bufferdata[2*col] = bufferdata[2*(col + Off_col)] * buffermask[col];
        bufferdata[2*col+1] = bufferdata[2*(col + Off_col)+1] * buffermask[col];
        }
