...
Description :  Non Local Sigma Mean Filtering.
...
#pragma omp parallel for private(col) shared(ligDone) schedule(dynamic)
		...
		ligDone = 0;
#pragma omp parallel for private(col,Np,Dz1,Dx1,Mm1,Dz2,Dx2,Mm2,Dzij,Mij,Dxij,kij,bij,vc2,temp,wk,Lee) shared(ligDone) schedule(dynamic)
        for (lig = 0; lig < Sub_Nlig; lig++) {
          ...
          for (col = 0; col < Sub_Ncol; col++) {
            ...
            if (my_isfinite(kij) == 0) kij = 1.; /* Test NaN value */
            bij = Mm2 / (Mm1+eps);
