...
#pragma omp parallel for private(col) shared(ligDone) schedule(dynamic)
  for (lig = 0; lig < NligBlock[Nb]+Nwin; lig++) {
    ...
    for (col = 0; col < Sub_Ncol+Nwin; col++) {
      if ((strcmp(PolTypeOut,"C2")==0)||(strcmp(PolTypeOut,"C2pp1")==0)||(strcmp(PolTypeOut,"C2pp2")==0)||(strcmp(PolTypeOut,"C2pp3")==0)) span[lig][col] = M_in[0][lig][col]+M_in[3][lig][col];
      if ((strcmp(PolTypeOut,"T2")==0)||(strcmp(PolTypeOut,"T2pp1")==0)||(strcmp(PolTypeOut,"T2pp2")==0)||(strcmp(PolTypeOut,"T2pp3")==0)) span[lig][col] = M_in[0][lig][col]+M_in[3][lig][col];
      if ((strcmp(PolTypeOut,"C3")==0) || (strcmp(PolTypeOut,"T3")==0)) span[lig][col] = M_in[0][lig][col]+M_in[5][lig][col]+M_in[8][lig][col];
      if ((strcmp(PolTypeOut,"C4")==0) || (strcmp(PolTypeOut,"T4")==0)) span[lig][col] = M_in[0][lig][col]+M_in[7][lig][col]+M_in[12][lig][col]+M_in[15][lig][col];
      }
    }
...
#pragma omp parallel for private(col, Np, k, l, mean, Npoints) shared(ligDone) schedule(dynamic)
...
/*
  ...
  free_matrix_float(coeff, NligBlock[0] + Nwin);
  free_matrix_int(Nmax, NligBlock[0]);
...
/* FILTERING */
#pragma omp parallel for private(col,k,l,kk,ll,subwin,MaxDist,m_span,m_span2,v_span,cv_span,Dist, Npoints)
