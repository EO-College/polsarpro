...
Description :  System Noise polarimetric filter
...
/* LOCAL VARIABLES */
  int Config;
  char *PolTypeConf[NPolType] = { "C4", "T4"};
...
/* POLAR TYPE CONFIGURATION */
  ...
  NpolarOut = 9;
  if (strcmp(PolTypeIn,"C4")==0) strcpy(PolTypeOut, "C3");
  if (strcmp(PolTypeIn,"T4")==0) strcpy(PolTypeOut, "T3");
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (strcmp(PolTypeIn,"C4")==0) C4_to_T4(M_in, NligBlock[Nb], Sub_Ncol, 0, 0);

  if (FlagValid == 1) read_block_matrix_float(in_valid, Valid, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, NwinL, NwinC, Off_lig, Off_col, Ncol);

#pragma omp parallel for private(col,M,V,lambda,Np,noise) shared(ligDone) schedule(dynamic)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      if (Valid[lig][col] == 1.) {
        ...
        M_out[T311][lig][col] = M_in[T411][lig][col] - noise;
        M_out[T312_re][lig][col] = M_in[T412_re][lig][col];
        M_out[T312_im][lig][col] = M_in[T412_im][lig][col];
        M_out[T313_re][lig][col] = M_in[T413_re][lig][col];
        M_out[T313_im][lig][col] = M_in[T413_im][lig][col];
        M_out[T322][lig][col] = M_in[T422][lig][col] - noise;
        M_out[T323_re][lig][col] = M_in[T423_re][lig][col];
        M_out[T323_im][lig][col] = M_in[T423_im][lig][col];
        M_out[T333][lig][col] = M_in[T433][lig][col] - noise;
        }
      }
  ...
  if (strcmp(PolTypeIn,"C4")==0) T3_to_C3(M_out, NligBlock[Nb], Sub_Ncol, 0, 0);
