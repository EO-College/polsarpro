...
#pragma omp parallel for private(col, Np, k, l, idxY, idxG) shared(ligDone) schedule(dynamic)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      for (Np = 0; Np < NpolarOut; Np++) {
        M_out[Np][lig][col] = 0.; 
        for (k = -NwinLM1S2; k < 1 + NwinLM1S2; k++) {
          idxY = NwinLM1S2+lig+k; idxG = NwinLM1S2+k;
          for (l = -NwinCM1S2; l < 1 +NwinCM1S2; l++) {
            M_out[Np][lig][col] += M_in[Np][idxY][NwinCM1S2+col+l]*Valid[idxY][NwinCM1S2+col+l]*Gauss[idxG][NwinCM1S2 + l];
            }
          }
