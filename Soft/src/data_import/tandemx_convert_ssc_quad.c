...
File   : tandemx_convert_ssc_quad.c
...
strcat(UsageHelp," (string)	-pp  	polar type (full)\n");
...
********************************************************************/
  ...
  if (strcmp(PolarPP, "FULL") == 0) strcpy(PolarType, "full");
  if (strcmp(PolarPP, "full") == 0) strcpy(PolarType, "full");
    ...
    for (Np = 0; Np < Npol; Np++) {
      ...
      if (BistCorrect == 1) {
        for (col = 0; col < Ncol; col++) {
          ...
          S_in[3][lig][2*col] = -S_in[3][lig][2*col]; S_in[3][lig][2*col + 1] = -S_in[3][lig][2*col + 1];
          }
        }
      }
    }
