...
File   : rawbinary_convert_ModPha_C3.c
...
/******************************************************************************/
  ...
  if (argc < 21) {
  printf("TYPE: rawbinary_convert_ModPha_C3 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[C311][col] = M_tmp[C11m][col];
     M_in[C312_re][col] = M_tmp[C12m][col]*cos(M_tmp[C12p][col]);
     M_in[C312_im][col] = M_tmp[C12m][col]*sin(M_tmp[C12p][col]);
     M_in[C313_re][col] = M_tmp[C13m][col]*cos(M_tmp[C13p][col]);
     M_in[C313_im][col] = M_tmp[C13m][col]*sin(M_tmp[C13p][col]);
     M_in[C322][col] = M_tmp[C22m][col];
     M_in[C323_re][col] = M_tmp[C23m][col]*cos(M_tmp[C23p][col]);
     M_in[C323_im][col] = M_tmp[C23m][col]*sin(M_tmp[C23p][col]);
     M_in[C333][col] = M_tmp[C33m][col];
