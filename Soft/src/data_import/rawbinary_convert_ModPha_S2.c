...
File   : rawbinary_convert_ModPha_S2.c
...
/******************************************************************************/
  ...
  if (argc < 20) {
  printf("TYPE: rawbinary_convert_ModPha_S2 DirOutput Ncol OffsetLig OffsetCol\n");
  ...
  printf("FileInput5 FileInput6 FileInput7 FileInput8\n");
  exit(1);
  } else {
...
/* Nb of lines and rows sub-sampled image */
  ...
  Npolar_in = 8;
  if (strcmp(DataFormat, "S2") == 0) Npolar_out = 4;
  ...
  M_in = matrix_float(4, 2 * Ncol);
  if (strcmp(DataFormat, "S2") == 0) M_out = matrix_float(Npolar_out, 2 * Ncolfin);
...
/******************************************************************************/
  ...
  if (strcmp(DataFormat, "S2") == 0) sprintf(file_name, "%s%s", DirOutput, FileOutputS2[np]);
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (np = 0; np < Npolar_in; np++) {
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[hh][2 * col] = M_tmp[hhm][col]*cos(M_tmp[hhp][col]);M_in[hh][2 * col + 1] = M_tmp[hhm][col]*sin(M_tmp[hhp][col]);
     M_in[hv][2 * col] = M_tmp[hvm][col]*cos(M_tmp[hvp][col]);M_in[hv][2 * col + 1] = M_tmp[hvm][col]*sin(M_tmp[hvp][col]);
     M_in[vh][2 * col] = M_tmp[vhm][col]*cos(M_tmp[vhp][col]);M_tmp[vh][2 * col + 1] = M_tmp[vhm][col]*sin(M_tmp[vhp][col]);
     M_in[vv][2 * col] = M_tmp[vvm][col]*cos(M_tmp[vvp][col]);M_in[vv][2 * col + 1] = M_tmp[vvm][col]*sin(M_tmp[vvp][col]);
     for (np = 0; np < 4; np++) {
  ...
  for (col = 0; col < Ncolfin; col++) {
    ind = 2 * (col * SubSampRG + Ncoloffset);
    k1r = M_in[hh][ind]; k1i = M_in[hh][ind + 1];
    k2r = M_in[hv][ind]; k2i = M_in[hv][ind + 1];
    k3r = M_in[vh][ind]; k3i = M_in[vh][ind + 1];
    k4r = M_in[vv][ind]; k4i = M_in[vv][ind + 1];
