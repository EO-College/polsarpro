...
File   : rawbinary_convert_RealImag_SPP.c
...
/******************************************************************************/
  ...
  if (argc < 17) {
  printf("TYPE: rawbinary_convert_RealImag_SPP DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (np = 0; np < Npolar_in; np++) {
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[0][2 * col] = M_tmp[0][col];M_in[0][2 * col + 1] = M_tmp[1][col];
     M_in[1][2 * col] = M_tmp[2][col];M_in[1][2 * col + 1] = M_tmp[3][col];
     for (np = 0; np < 2; np++) {
