...
File   : sentinel1_convert_dual_all.c
...
Description :  Convert SENTINEL-1 Binary Data Files 
               (GEOTIFF Format) and merge all the bursts
...
//int samplesPerBurst;
  ...
  int passInfo, BurstNumMax;
  int OffLig1, OffLig2, OffCol1, OffCol2;
  int Length, NLig1, NLig2;
  int FlagM1, FlagBurst, NligBurst;
  double azimuthTimeInterval, HH, MM, SS, Time;
  char OffColChar[100];
  double *azimuthTime;
  int *BurstOffLig;
  int *BurstSubLig;
...
/* Internal variables */
  int ii, jj, lig, col, k, l;
  int indlig, indcol, OOff_lig;
...
strcpy(UsageHelp,"\nsentinel1_convert_dual_all.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
if(argc < 33) {
  ...
  get_commandline_prm(argc,argv,"-if",str_cmd_prm,FileConfig,1,UsageHelp);
...
/********************************************************************
********************************************************************/
  strcpy(PolTypeTmp, "SPP");
/* POLAR TYPE CONFIGURATION */
  PolTypeConfig(PolTypeTmp, &NpolarIn, PolTypeIn, &NpolarOut, PolTypeOut, PolarType);
...
/********************************************************************
********************************************************************/
  if ((ftmp = fopen(FileConfig, "r")) == NULL)
  ...
  while( !feof(ftmp) ) {
    ...
    if (strstr(Buf,"productInformation") != NULL) {
      ...
      if (strstr(Buf,"pass") != NULL) {
        ...
        if (strcmp(Tmp,"Ascending") == 0) passInfo = 1;
        else passInfo = 0;
        }
      }
    if (strstr(Buf,"numberOfSamples") != NULL) {
      ...
      Sin = matrix_short_int(Npol, 2*numberOfSamples);
      Sout = matrix_float(Npol, 2*numberOfSamples);   
      }
    ...
    if (strstr(Buf,"azimuthTimeInterval") != NULL) {
      p1 = strstr(Buf,".: ");
      strcpy(Tmp, ""); strncat(Tmp, &p1[3], strlen(p1) - 3); 
      azimuthTimeInterval = atof(Tmp);
      }
    }
  ...
  while( !feof(ftmp) ) {
    ...
    if (FlagSwath == 1) {
      if (strstr(Buf,"linesPerBurst") != NULL) {
        ...
        azimuthTime = vector_double_float(BurstNumMax);
        BurstOffLig = vector_int(BurstNumMax);
        BurstSubLig = vector_int(BurstNumMax);
        BurstOffCol = vector_int(BurstNumMax);
        BurstSubCol = vector_int(BurstNumMax);
        }
      if (strstr(Buf,"burstList") != NULL) {
        for (ii = 0; ii < BurstNumMax; ii++) {
          fgets(&Buf[0], 1024, ftmp); //burst
          fgets(&Buf[0], 1024, ftmp); //azimuthTime
          p1 = strstr(Buf,".: ");
          strcpy(Tmp, ""); strncat(Tmp, &p1[3], strlen(p1) - 3);      
          strcpy(Buf, ""); strncat(Buf, &Tmp[11], 2); HH = atof(Buf);      
          strcpy(Buf, ""); strncat(Buf, &Tmp[14], 2); MM = atof(Buf);      
          strcpy(Buf, ""); strncat(Buf, &Tmp[17], strlen(p1) - 17); SS = atof(Buf);      
          azimuthTime[ii] = HH*0. + MM*60. + SS;
          ...
          BurstOffLig[ii] = (OffLig1+OffLig2)/2;
          BurstSubLig[ii] = (NLig1+NLig2)/2;        
          BurstOffCol[ii] = OffCol1; 
          BurstSubCol[ii] = OffCol2-OffCol1+1;        
          }
        }
      }
  ...
  if (numberOfSamples & 1) {
    BurstSubColMax = numberOfSamples- 1;
    } else {
    BurstSubColMax = numberOfSamples;
    }
...
/********************************************************************
********************************************************************/
  for (col = 0; col < BurstSubColMax; col++) {
    for (Np = 0; Np < Npol; Np++) {
      Sout[Np][2*col] = 0.; Sout[Np][2*col+1] = 0.;
      }
    }         
  for (lig = 0; lig < BurstOffLig[0]; lig++)
    for (Np = 0; Np < Npol; Np++) fwrite(&Sout[Np][0],sizeof(float), 2 * BurstSubColMax, out_datafile[Np]);
  ...
  for (ii = 0; ii < BurstNumMax; ii++) {
    ...
    for (jj = 0; jj < ii; jj++) {
      for (lig = 0; lig < linesPerBurst; lig++) {
        PointerPosition = Strip_Offset[Strip];
        for (Np = 0; Np < Npol; Np++) my_fseek_position(in_file[Np], PointerPosition);
        for (Np = 0; Np < Npol; Np++) fread(&Sin[Np][0], sizeof(short int), 2 * numberOfSamples, in_file[Np]);
        Strip++;
        }
      }
    Time = azimuthTime[ii];
    //Offset Lines Reading
    for (lig = 0; lig < BurstOffLig[ii]; lig++) {
      PointerPosition = Strip_Offset[Strip];
      for (Np = 0; Np < Npol; Np++) my_fseek_position(in_file[Np], PointerPosition);
      for (Np = 0; Np < Npol; Np++) fread(&Sin[Np][0], sizeof(short int), 2 * numberOfSamples, in_file[Np]);
      Strip++;
      Time = Time + azimuthTimeInterval;
      }

    if (ii == BurstNumMax - 1) {
      for (lig = 0; lig < BurstSubLig[ii]; lig++) {
        PointerPosition = Strip_Offset[Strip];
        for (Np = 0; Np < Npol; Np++) my_fseek_position(in_file[Np], PointerPosition);    
        for (Np = 0; Np < Npol; Np++) {
          if (IEEE == 0)
            fread(&Sin[Np][0], sizeof(short int), 2 * numberOfSamples, in_file[Np]);
          if (IEEE == 1) {
            for (col = 0; col < numberOfSamples; col++) {
              ...
              v = &fl2;pc = (char *) v;
              pc[1] = getc(in_file[Np]);pc[0] = getc(in_file[Np]);
              Sin[Np][2*col] = fl1; Sin[Np][2*col+1] = fl2;
              }
            }
          }
        for (col = 0; col < BurstSubColMax; col++) {
        ...
        for (col = 0; col < BurstSubCol[ii]; col++) {
          for (Np = 0; Np < Npol; Np++) {
            Sout[Np][2*(col+BurstOffCol[ii])] = (float)Sin[Np][2*(col+BurstOffCol[ii])];
            Sout[Np][2*(col+BurstOffCol[ii])+1] = (float)Sin[Np][2*(col+BurstOffCol[ii])+1];
            if (my_isfinite(Sout[Np][2*(col+BurstOffCol[ii])]) == 0) Sout[Np][2*(col+BurstOffCol[ii])] = eps;
            if (my_isfinite(Sout[Np][2*(col+BurstOffCol[ii]) + 1]) == 0) Sout[Np][2*(col+BurstOffCol[ii]) + 1] = eps;
            }
          }
        for (Np = 0; Np < Npol; Np++) fwrite(&Sout[Np][0],sizeof(float), 2 * BurstSubColMax, out_datafile[Np]);
        NligBurst++; printf("%f\r", 100. * NligBurst / numberOfLines);fflush(stdout);
        Strip++;   
        }
      } else {
      FlagBurst = 0;
      while (FlagBurst == 0) {
        ...
        Strip++;   
        Time = Time + azimuthTimeInterval;
        if (Time >= (azimuthTime[ii+1] + BurstOffLig[ii+1]*azimuthTimeInterval)) FlagBurst = 1;
        }
      }
    }
  ...
  for (lig = 0; lig < (linesPerBurst - BurstOffLig[BurstNumMax - 1] - BurstSubLig[BurstNumMax - 1]); lig++)
    for (Np = 0; Np < Npol; Np++) fwrite(&Sout[Np][0],sizeof(float), 2 * BurstSubColMax, out_datafile[Np]);

/* OUPUT CONFIGURATIONS */
  strcpy(PolarCase, "monostatic");
  NligBurst += NligBurst + BurstOffLig[0] + (linesPerBurst - BurstOffLig[BurstNumMax - 1] - BurstSubLig[BurstNumMax - 1]);
  write_config(tmp_dir, NligBurst, BurstSubColMax, PolarCase, PolarType);
...
/* INPUT/OUPUT CONFIGURATIONS */
  read_config(tmp_dir, &Nlig, &Ncol, PolarCase, PolarType);
  if (Sub_Nlig > Nlig) Sub_Nlig = Nlig;
  if (Sub_Ncol > Ncol) Sub_Ncol = Ncol;
...
/* INPUT/OUTPUT FILE CONFIGURATION */
  init_file_name(PolTypeIn, tmp_dir, file_name_in);
...
/* DATA PROCESSING */
  ...
  if (passInfo == 0) OOff_lig = Off_lig;
  if (passInfo == 1) OOff_lig = Nlig - Sub_Nlig*SubSampLig*NLookLig - Off_lig;
    
  /* Offset Lines Reading */
  for (lig = 0; lig < OOff_lig; lig++) {
    for (Np = 0; Np < Npol; Np++) fread(&S_in[Np][0][0], sizeof(float), 2 * Ncol, in_datafile[Np]);
    }
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) fread(&S_in[Np][lig][0], sizeof(float), 2 * Ncol, in_datafile[Np]);
...
void read_tiff_strip (char FileInput[FilePathLength])
{
  ...
  int Nbyte, Nstrip, Count, Value, IEEEFormat;
...
/* Tiff File Header */
  ...
  for (i=0; i<Ndir; i++) {
    ...
    if (Flag == 279) Nbyte = Count;
    if (Flag == 279) offset_strip_byte = Value;
    }
   
  Strip_Offset = vector_int(Nstrip);
  Strip_Bytes = vector_int(Nbyte);
