...
File   : rawbinary_convert_ModPha_MLK_S2.c
...
/******************************************************************************/
  ...
  if (argc < 20) {
  printf("TYPE: rawbinary_convert_ModPha_MLK_S2 DirOutput Ncol OffsetLig OffsetCol\n");
...
/* Nb of lines and rows sub-sampled image */
  ...
  M_in = matrix3d_float(4, Nlook_lig, 2 * Ncol);
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (ii = 0; ii < Nlook_lig; ii++) {
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[hh][ii][2 * col] = M_tmp[hhm][ii][col]*cos(M_tmp[hhp][ii][col]);M_in[hh][ii][2 * col + 1] = M_tmp[hhm][ii][col]*sin(M_tmp[hhp][ii][col]);
     M_in[hv][ii][2 * col] = M_tmp[hvm][ii][col]*cos(M_tmp[hvp][ii][col]);M_in[hv][ii][2 * col + 1] = M_tmp[hvm][ii][col]*sin(M_tmp[hvp][ii][col]);
     M_in[vh][ii][2 * col] = M_tmp[vhm][ii][col]*cos(M_tmp[vhp][ii][col]);M_in[vh][ii][2 * col + 1] = M_tmp[vhm][ii][col]*sin(M_tmp[vhp][ii][col]);
     M_in[vv][ii][2 * col] = M_tmp[vvm][ii][col]*cos(M_tmp[vvp][ii][col]);M_in[vv][ii][2 * col + 1] = M_tmp[vvm][ii][col]*sin(M_tmp[vvp][ii][col]);
     for (np = 0; np < 4; np++) {
