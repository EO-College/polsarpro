...
/*EMISAR DECLARATION*/
#define UBYTE unsigned char    /*  8 bit unsigned integer.  */
#define SWORD signed short int   /* 16 bit signed integer.    */
#define SREAL float        /* 32 bit floating point real. */

/* Union used to extract or set the first 16 bit of a 32 bit float. */
typedef union {
  SREAL r;
  SWORD s[2];
} RS_U;
...
/* Internal variables */
  ...
  int IEEE, Symmetrisation, first, second;
  RS_U rs;
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) {
      for (col = 0; col < Ncol; col++) {
        v = &SW;pc = (unsigned char *) v;
        pc[first] = getc(in_file[Np]);pc[second] = getc(in_file[Np]);
        rs.s[first] = SW;rs.s[second] = 0;S_in[Np][lig][2 * col] = (SREAL)rs.r;
        v = &SW;pc = (unsigned char *) v;
        pc[first] = getc(in_file[Np]);pc[second] = getc(in_file[Np]);
        rs.s[first] = SW;rs.s[second] = 0;S_in[Np][lig][2 * col + 1] = (SREAL)rs.r;
        }
      }
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
