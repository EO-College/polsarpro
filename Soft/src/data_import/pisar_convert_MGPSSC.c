...
Description :  Convert PISAR Binary Data Files 
               (Format MGPSSC)
...
/* Internal variables */
  ...
  int IEEE, Symmetrisation, Offset;
...
strcat(UsageHelp," (int)   	-off 	Offset\n");
strcat(UsageHelp,"\nOptional Parameters:\n");
...
if(argc < 39) {
  ...
  get_commandline_prm(argc,argv,"-off",int_cmd_prm,&Offset,1,UsageHelp);
...
/* SKIP HEADER */
for (Np = 0; Np < Npol; Np++) fseek(in_file[Np], 32L, SEEK_SET);

  /* Offset Lines Reading */
  for (lig = 0; lig < Off_lig; lig++)
    for (Np = 0; Np < Npol; Np++) {
//      for (k=0; k<Offset; k++) buf = getc(in_file[Np]);
      for (k=0; k<Offset; k++) getc(in_file[Np]);
