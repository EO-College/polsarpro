...
Description :  Convert ALOS-PALSAR Binary Data Files
               ...
               Apply a Data Un-Calibration Procedure
...
/* READ CONFIG FILE */
  ...
  if (calfac != 0.) calfac = sqrt(pow(10.,(calfac-32.)/10.));
  else calfac = 1.;
  fscanf(headerfile, "%s\n", Tmp); fscanf(headerfile, "%s\n", Tmp); 
  fscanf(headerfile, "%s\n", Tmp); fscanf(headerfile, "%s\n", Tmp); 
  fscanf(headerfile, "%s\n", Tmp); fscanf(headerfile, "%s\n", Tmp); 
...
/* OFFSET HEADER DATA READING */
  for (Np = 0; Np < Npol; Np++) fseek(in_file[Np], iheader, 1);

  /* Offset Lines Reading */
  for (lig = 0; lig < Off_lig; lig++)
    for (Np = 0; Np < Npol; Np++) 
      my_fseek(in_file[Np], 1, ireclen, 1);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) {
      my_fseek(in_file[Np], 1, iprefix, 1);
      for (col = 0; col < Ncol; col++) {
        ...
        pc[3] = getc(in_file[Np]);pc[2] = getc(in_file[Np]);
        pc[1] = getc(in_file[Np]);pc[0] = getc(in_file[Np]);
        S_in[Np][lig][2 * col] = fl1*calfac;S_in[Np][lig][2 * col + 1] = fl2*calfac;
        }
      }
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
...
/* UNCALIBRATION PROCEDURE */
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    PrintfLine(lig,NligBlock[Nb]);
    for (col = 0; col < Ncol; col++) {
      M1[0][0].re = S_in[0][lig][2*col];M1[0][0].im = S_in[0][lig][2*col + 1];
      M1[0][1].re = S_in[1][lig][2*col];M1[0][1].im = S_in[1][lig][2*col + 1];
      M1[1][0].re = S_in[2][lig][2*col];M1[1][0].im = S_in[2][lig][2*col + 1];
      M1[1][1].re = S_in[3][lig][2*col];M1[1][1].im = S_in[3][lig][2*col + 1];
      cplx_mul_mat(M1,DT,M2,2,2); cplx_mul_mat(DR,M2,M1,2,2);
      S_in[0][lig][2*col] = M1[0][0].re;S_in[0][lig][2*col + 1] = M1[0][0].im;
      S_in[1][lig][2*col] = M1[0][1].re;S_in[1][lig][2*col + 1] = M1[0][1].im;
      S_in[2][lig][2*col] = M1[1][0].re;S_in[2][lig][2*col + 1] = M1[1][0].im;
      S_in[3][lig][2*col] = M1[1][1].re;S_in[3][lig][2*col + 1] = M1[1][1].im;
      }
    }
