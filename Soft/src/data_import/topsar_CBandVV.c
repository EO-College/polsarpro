...
/* INPUT/OUTPUT FILE OPENING*/
  ...
  sprintf(file_name, "%sCBandVV.bin", out_dir);
...
/* READING */
  ...
  for (j = 0; j < M_Ncol; j++) {
    ...
    M_out[j] = M_out[j]*M_out[j] / gen_fac;
    if (M_out[j] < eps) M_out[j] = my_eps_random();
