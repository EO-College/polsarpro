...
File   : terrasarx_convert_mgd_gec_eec_dual.c
...
Description :  Convert TERRASARX Binary Data Files (GEOTIFF Format)
...
strcpy(UsageHelp,"\nterrasarx_convert_mgd_gec_eec_dual.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
********************************************************************/
  ...
  is_a_tiff = 0;
  fread(buffer, 1, 4, in_file[0]);
  if(buffer[0] == 0x49 && buffer[1] == 0x49 && buffer[2] == 0x2a && buffer[3] == 0x00) { is_a_tiff = 1; MSBLSB = 0; }
  if(buffer[0] == 0x4d && buffer[1] == 0x4d && buffer[2] == 0x00 && buffer[3] == 0x2a) { is_a_tiff = 1; MSBLSB = 1; }
  if (is_a_tiff == 0) edit_error(File11," : is not a TIFF file");
...
/* OFFSET HEADER DATA READING */
  for (Np = 0; Np < Npol; Np++) fseek(in_file[Np], 8, SEEK_SET);
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) {
      ...
      for (col = 0; col < Ncol; col++) {
        if (MSBLSB == 0) {LS = dataread[2*col]; MS = dataread[2*col + 1];} 
        if (MSBLSB == 1) {MS = dataread[2*col]; LS = dataread[2*col + 1];} 
        if (MS < 0) MS = MS + 256; if (LS < 0) LS = LS + 256;
        I_in[Np][lig][col] = (256. * MS + LS)*calfac[Np];
        if (I_in[Np][lig][col] < eps) I_in[Np][lig][col] = my_eps_random();
        ...
        for (Np = 0; Np < NpolarOut; Np++) {
          ...
          for (k = 0; k < NLookLig; k++)
            for (l = 0; l < NLookCol; l++)
              M_out[Np][lig][col] += I_in[Np][indlig+k][indcol+l+Off_col];
