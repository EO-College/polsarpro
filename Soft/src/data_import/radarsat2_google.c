...
/*******************************************************************/
  ...
  while (Flag == 0) {
    ...
    pstr = strstr(Buf,"imageAttributes");
    if (pstr != NULL) Flag = 1;
  }
  Flag = 0;
  while (Flag == 0) {
    fgets(&Buf[0], 100, filename);
    pstr = strstr(Buf,"geolocationGrid");
    if (pstr != NULL) Flag = 1;
  }
...
//Point 0,0
  fgets(&Buf[0], 100, filename);
  fgets(&Buf[0], 100, filename);
  fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); Nlig = floor(atof(Tmp));
  fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); //Ncol = floor(atof(Tmp));
  fgets(&Buf[0], 100, filename);
...
//Point 0,N
  ...
  while (Flag == 0) {
    fgets(&Buf[0], 100, filename);
    fgets(&Buf[0], 100, filename);
    fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); 
    if (Nlig == floor(atof(Tmp))) {
      fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); //Ncol = floor(atof(Tmp));
      fgets(&Buf[0], 100, filename);
...
//Point N,N
  ...
  while (Flag == 0) {
    fgets(&Buf[0], 100, filename);
    pstr = strstr(Buf,"rationalFunctions");
    if (pstr == NULL) {
      fgets(&Buf[0], 100, filename);
      fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); Nlig = floor(atof(Tmp));
      fgets(&Buf[0], 100, filename); 
...
//Point N,0
  ...
  while (Flag == 0) {
    fgets(&Buf[0], 100, filename);
    fgets(&Buf[0], 100, filename);
    fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); Nl = floor(atof(Tmp));
    fgets(&Buf[0], 100, filename); strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); Nc = floor(atof(Tmp));
    if ((Nlig == Nl)&&(Nc == 0)) {
      fgets(&Buf[0], 100, filename);
...
//Point Center
  ...
  while (Flag == 0) {
    fgets(&Buf[0], 100, filename);
    pstr = strstr(Buf,"latitudeOffset");
    if (pstr != NULL) Flag = 1;
  }
  strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); LatCenter = atof(Tmp);
  Flag = 0;
  while (Flag == 0) {
    fgets(&Buf[0], 100, filename);
    pstr = strstr(Buf,"longitudeOffset");
    if (pstr != NULL) Flag = 1;
  }
  strcpy(Tmp, ""); strncat(Tmp, &Buf[71], strlen(Buf) - 71); LonCenter = atof(Tmp);
...
/*******************************************************************/
  ...
  fprintf(filename, "Image RADARSAT 2\n");
  fprintf(filename,"</name>\n");
