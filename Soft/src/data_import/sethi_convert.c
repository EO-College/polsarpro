...
Description :  Convert SETHI Binary Data Files 
               (Format SLC)
...
/* LOCAL VARIABLES */
  ...
  char FileConfig[FilePathLength], Tmp[1024];
...
/* Internal variables */
  ...
  float xx;
  float MagicNumber, theta;
  float DistanceRadar, HauteurRadar;
  float PixCol, PixSurf;
...
strcat(UsageHelp," (string)	-cfg 	input config file\n");
...
if(argc < 39) {
  ...
  get_commandline_prm(argc,argv,"-cfg",str_cmd_prm,FileConfig,1,UsageHelp);
...
/* DATA PROCESSING */
  ...
  if ((fconfig = fopen(FileConfig, "r")) == NULL)
...
/* SKIP HEADER */
  for (Np = 0; Np < Npol; Np++) {
    fread(&MagicNumber, sizeof(float), 1, in_file[Np]);
    fread(&Header[0], sizeof(float), 2*Ncol, in_file[Np]);
    }
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Ncol; col++) {
      theta = acos(HauteurRadar/(DistanceRadar + col*PixCol));
      for (Np = 0; Np < Npol; Np++) {
        S_in[Np][lig][2*col] = S_in[Np][lig][2*col]*sqrt(sin(theta)/PixSurf);
        S_in[Np][lig][2*col+1] = S_in[Np][lig][2*col+1]*sqrt(sin(theta)/PixSurf);
