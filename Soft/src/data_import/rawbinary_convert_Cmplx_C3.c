...
File   : rawbinary_convert_Cmplx_C3.c
...
/******************************************************************************/
  ...
  if (argc < 18) {
  printf("TYPE: rawbinary_convert_Cmplx_C3 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[C311][col] = M_tmp[C11][2 * col];
     M_in[C312_re][col] = M_tmp[C12][2 * col];
     M_in[C312_im][col] = M_tmp[C12][2 * col + 1];
     M_in[C313_re][col] = M_tmp[C13][2 * col];
     M_in[C313_im][col] = M_tmp[C13][2 * col + 1];
     M_in[C322][col] = M_tmp[C22][2 * col];
     M_in[C323_re][col] = M_tmp[C23][2 * col];
     M_in[C323_im][col] = M_tmp[C23][2 * col + 1];
     M_in[C333][col] = M_tmp[C33][2 * col];
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    M_out[C311][col] = M_in[C311][ind];
    M_out[C312_re][col] = M_in[C312_re][ind];
    M_out[C312_im][col] = M_in[C312_im][ind];
    M_out[C313_re][col] = M_in[C313_re][ind];
    M_out[C313_im][col] = M_in[C313_im][ind];
    M_out[C322][col] = M_in[C322][ind];
    M_out[C323_re][col] = M_in[C323_re][ind];
    M_out[C323_im][col] = M_in[C323_im][ind];
    M_out[C333][col] = M_in[C333][ind];
    }
  for (np = 0; np < Npolar_out; np++)
  ...
  for (col = 0; col < Ncolfin; col++) {
    ind = col * SubSampRG + Ncoloffset;
    M_out[T311][col] = (M_in[C311][ind] + 2 * M_in[C313_re][ind] + M_in[C333][ind]) / 2;
    M_out[T312_re][col] = (M_in[C311][ind] - M_in[C333][ind]) / 2;
    M_out[T312_im][col] = -M_in[C313_im][ind];
    M_out[T313_re][col] = (M_in[C312_re][ind] + M_in[C323_re][ind]) / sqrt(2);
    M_out[T313_im][col] = (M_in[C312_im][ind] - M_in[C323_im][ind]) / sqrt(2);
    M_out[T322][col] = (M_in[C311][ind] - 2 * M_in[C313_re][ind] + M_in[C333][ind]) / 2;
    M_out[T323_re][col] = (M_in[C312_re][ind] - M_in[C323_re][ind]) / sqrt(2);
    M_out[T323_im][col] = (M_in[C312_im][ind] + M_in[C323_im][ind]) / sqrt(2);
    M_out[T333][col] = M_in[C322][ind];
    }
  for (np = 0; np < Npolar_out; np++)
    fwrite(&M_out[np][0], sizeof(float), Ncolfin, out_file[np]);
  }
