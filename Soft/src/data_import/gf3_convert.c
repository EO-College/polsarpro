...
Description :  Convert GF-3  Binary Data Files 
               (GEOTIFF Format)
...
/* Internal variables */
  ...
  int IEEE, Symmetrisation, Strip;
  int NNb, NNlig;
  
  float xx;
  float QualifyValue[4];
...
strcat(UsageHelp," (float) 	-qv1 	Qualify Value HH\n");
strcat(UsageHelp," (float) 	-qv2 	Qualify Value HV\n");
strcat(UsageHelp," (float) 	-qv3 	Qualify Value VH\n");
strcat(UsageHelp," (float) 	-qv4 	Qualify Value VV\n");
strcat(UsageHelp,"\nOptional Parameters:\n");
...
if(argc < 45) {
  ...
  get_commandline_prm(argc,argv,"-qv1",flt_cmd_prm,&QualifyValue[0],1,UsageHelp);
  get_commandline_prm(argc,argv,"-qv2",flt_cmd_prm,&QualifyValue[1],1,UsageHelp);
  get_commandline_prm(argc,argv,"-qv3",flt_cmd_prm,&QualifyValue[2],1,UsageHelp);
  get_commandline_prm(argc,argv,"-qv4",flt_cmd_prm,&QualifyValue[3],1,UsageHelp);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
        S_in[Np][lig][2*col] = (float)Min[Np][2*col] * QualifyValue[Np] / 32767.;
        S_in[Np][lig][2*col+1] = (float)Min[Np][2*col+1] * QualifyValue[Np] / 32767.;
