	...
	char const * const aSource) /* aoDestination and aSource can be the same */
{
	...
	if ( aSource )
	{
		...
		{			
			...
			if (lTmp - aSource == 2)
			{
				aoDestination[lTmp - aSource] = '\\';
				aoDestination[lTmp - aSource+1] = '\0';
			}
			else
...
static int dirExists ( char const * const aDirPath )
{
	...
	if ( stat ( aDirPath , & lInfo ) != 0 )
		return 0 ;
	else if ((lInfo.st_mode & S_IFDIR) == S_IFDIR)
		return 1 ;
	else
		return 0 ;
}
...
#else
			FMT_SWPRINTF(aoResultHexRGB, 8, L"#%02hhx%02hhx%02hhx", aRGB[0], aRGB[1], aRGB[2]);
#endif
