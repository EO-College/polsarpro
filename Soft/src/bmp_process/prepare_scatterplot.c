...
Description :  Prepare a Scatter-Plot file
...
/* LOCAL VARIABLES */
  FILE *fileinput, *fileoutput, *fileoutputmask;

  char FileInput[FilePathLength], FileOutputBin[FilePathLength];
  char FileOutputMask[FilePathLength], FileOutputTxt[FilePathLength];
...
strcat(UsageHelp," (string)	-omf 	output mask file\n");
...
if(argc < 31) {
  ...
  get_commandline_prm(argc,argv,"-omf",str_cmd_prm,FileOutputMask,1,UsageHelp);
...
/*******************************************************************/
  ...
  databmpmask = vector_float(Sub_Ncol);

  if ((fileoutput = fopen(FileOutputBin, "wb")) == NULL)
    edit_error("ERREUR DANS L'OUVERTURE DU FICHIER", FileOutputBin);
  if ((fileoutputmask = fopen(FileOutputMask, "wb")) == NULL)
    edit_error("ERREUR DANS L'OUVERTURE DU FICHIER", FileOutputMask);
...
#pragma omp parallel for private(xr, xi, xx)
  for (col = 0; col < Sub_Ncol; col++) {
    ...
    if (ValidMask[col+ Off_col] == 1.) {
      ...
      databmp[col] = DATA_NULL;     
      databmpmask[col] = 0.;
      } /* valid */
    } /* col */
  fwrite(&databmp[0], sizeof(float), Sub_Ncol, fileoutput);
  fwrite(&databmpmask[0], sizeof(float), Sub_Ncol, fileoutputmask);
  } /* lig */
  fclose(fileoutput);
  fclose(fileoutputmask);
