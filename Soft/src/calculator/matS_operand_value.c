...
Description :  MatS (operand) Value = MatS
...
#pragma omp parallel for private(col, Np)
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {   
        if (strcmp(operand,"addval") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = M_in[Np][lig][2*col]+ValR;
            M_out[Np][lig][2*col+1] = M_in[Np][lig][2*col+1]+ValI;
            }
          }
        ...
        if (strcmp(operand,"mulval") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = M_in[Np][lig][2*col]*ValR-M_in[Np][lig][2*col+1]*ValI;
            M_out[Np][lig][2*col+1] = M_in[Np][lig][2*col+1]*ValR+M_in[Np][lig][2*col]*ValI;
            }
          }
        if (strcmp(operand,"divval") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = (M_in[Np][lig][2*col]*ValR+M_in[Np][lig][2*col+1]*ValI)/(ValR*ValR+ValI*ValI);
            M_out[Np][lig][2*col+1] = (M_in[Np][lig][2*col+1]*ValR-M_in[Np][lig][2*col]*ValI)/(ValR*ValR+ValI*ValI);
            }
          }
        } else {
        for (Np = 0; Np < NpolarOut; Np++) {
