...
/* Local Variables */
  ...
  if ((strcmp(operand,"eig1S")==0)||(strcmp(operand,"eig2S")==0)) {
    /* Fout = Nlig*2*Sub_Ncol */
    NBlockA += 2*Sub_Ncol; NBlockB += 0;
    }
  if ((strcmp(operand,"eig1G")==0)||(strcmp(operand,"eig2G")==0)) {
    /* Fout = Nlig*Sub_Ncol */
...
#pragma omp parallel for private(col, k, MM1, MM2, G, V, lambda)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      if ((strcmp(operand,"eig1G")==0)||(strcmp(operand,"eig2G")==0)) M_out[lig][col] = 0.;
      if (Valid[lig][col] == 1.) {
        /* Graves */
        ...
        MM2[0][0][0] = M_in[s11][lig][2*col]; MM2[0][0][1] = -M_in[s11][lig][2*col+1];
        MM2[0][1][0] = M_in[s12][lig][2*col]; MM2[0][1][1] = -M_in[s12][lig][2*col+1];
        MM2[1][0][0] = M_in[s21][lig][2*col]; MM2[1][0][1] = -M_in[s21][lig][2*col+1];
        MM2[1][1][0] = M_in[s22][lig][2*col]; MM2[1][1][1] = -M_in[s22][lig][2*col+1];
        ProductCmplxMatrix(MM1,MM2,G,2);
        Diagonalisation(2, G, V, lambda);
        for (k = 0; k < 2; k++) if (lambda[k] < 0.) lambda[k] = 0.;
        
        if (strcmp(operand,"eig1G") == 0 ) {
          M_out[lig][col] = lambda[0];
          }
        if (strcmp(operand,"eig2G") == 0 ) {
          M_out[lig][col] = lambda[1];
          }
        if (strcmp(operand,"eig1S") == 0 ) {
          MM1[0][0][0] = M_in[s11][lig][2*col]; MM1[0][0][1] = M_in[s11][lig][2*col+1];
          MM1[0][1][0] = M_in[s12][lig][2*col]; MM1[0][1][1] = M_in[s12][lig][2*col+1];
          MM1[1][0][0] = M_in[s21][lig][2*col]; MM1[1][0][1] = M_in[s21][lig][2*col+1];
          MM1[1][1][0] = M_in[s22][lig][2*col]; MM1[1][1][1] = M_in[s22][lig][2*col+1];
          ProductCmplxMatrix(MM1,V,MM2,2);
          MM1[0][0][0] = V[0][0][0]; MM1[0][0][1] = V[0][0][1];
          MM1[0][1][0] = V[1][0][0]; MM1[0][1][1] = V[1][0][1];
          MM1[1][0][0] = V[0][1][0]; MM1[1][0][1] = V[0][1][1];
          MM1[1][1][0] = V[1][1][0]; MM1[1][1][1] = V[1][1][1];
          ProductCmplxMatrix(MM1,MM2,V,2);
          M_out[lig][2*col] = V[0][0][0]; M_out[lig][2*col+1] = V[0][0][1];
          }
        if (strcmp(operand,"eig1S") == 0 ) {
          MM1[0][0][0] = M_in[s11][lig][2*col]; MM1[0][0][1] = M_in[s11][lig][2*col+1];
          MM1[0][1][0] = M_in[s12][lig][2*col]; MM1[0][1][1] = M_in[s12][lig][2*col+1];
          ...
          MM1[1][1][0] = V[1][1][0]; MM1[1][1][1] = V[1][1][1];
          ProductCmplxMatrix(MM1,MM2,V,2);
          M_out[lig][2*col] = V[1][1][0]; M_out[lig][2*col+1] = V[1][1][1];
          }
        }
      }
