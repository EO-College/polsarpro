...
Description :  MatM (operand) Value = MatM
...
strcpy(UsageHelp,"\nmatM_operand_value.exe\n");
strcat(UsageHelp,"\nParameters:\n");
strcat(UsageHelp," (string)	-id  	input directroy\n");
strcat(UsageHelp," (float) 	-iv  	input value\n");
...
if(argc < 19) {
  ...
  get_commandline_prm(argc,argv,"-iv",flt_cmd_prm,&Value,1,UsageHelp);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if (strcmp(operand,"addval") == 0 ) 
          for (Np = 0; Np < NpolarOut; Np++) 
            M_out[Np][lig][col] = M_in[Np][lig][col]+Value;
        ...
        if (strcmp(operand,"divval") == 0 ) 
          for (Np = 0; Np < NpolarOut; Np++) 
            M_out[Np][lig][col] = M_in[Np][lig][col]/(Value+eps);
        } else {
