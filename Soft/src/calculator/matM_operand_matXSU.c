...
Description :  MatM (operand) MatXSU = MatM
...
/*-----------------------------------------------------------------*/   
  ...
  U = matrix3d_float(Npp, Npp, 2);
  U_m1 = matrix3d_float(Npp, Npp, 2);
...
/* READ matX DATA */
  ...
  if ((strcmp(PolTypeOut,"C2")==0)||(strcmp(PolTypeOut,"T2")==0)) {
    ...
    U[0][0][0] = eps + MatX[0][0];
    U[0][0][1] = 0.;
    U[0][1][0] = eps + MatX[1][0];
    U[0][1][1] = eps + MatX[2][0];
    U[1][0][0] =  U[0][1][0];
    U[1][0][1] = -U[0][1][1];
    U[1][1][0] = eps + MatX[3][0];
    U[1][1][1] = 0.;
    InverseHermitianMatrix2(U, U_m1);
    }
  if ((strcmp(PolTypeOut,"C3")==0)||(strcmp(PolTypeOut,"T3")==0)) {
    ...
    U[0][1][0] = eps + MatX[1][0];
    U[0][1][1] = eps + MatX[2][0];
    U[0][2][0] = eps + MatX[3][0];
    U[0][2][1] = eps + MatX[4][0];
    U[1][0][0] =  U[0][1][0];
    U[1][0][1] = -U[0][1][1];
    U[1][1][0] = eps + MatX[5][0];
    U[1][1][1] = 0.;
    U[1][2][0] = eps + MatX[6][0];
    U[1][2][1] = eps + MatX[7][0];
    U[2][0][0] =  U[0][2][0];
    U[2][0][1] = -U[0][2][1];
    U[2][1][0] =  U[1][2][0];
    U[2][1][1] = -U[1][2][1];
    U[2][2][0] = eps + MatX[8][0];
    U[2][2][1] = 0.;
    InverseHermitianMatrix3(U, U_m1);
    }
  if ((strcmp(PolTypeOut,"C4")==0)||(strcmp(PolTypeOut,"T4")==0)) {
    ...
    U[0][2][0] = eps + MatX[3][0];
    U[0][2][1] = eps + MatX[4][0];
    U[0][3][0] = eps + MatX[5][0];
    U[0][3][1] = eps + MatX[6][0];
    U[1][0][0] =  U[0][1][0];
    U[1][0][1] = -U[0][1][1];
    U[1][1][0] = eps + MatX[7][0];
    U[1][1][1] = 0.;
    U[1][2][0] = eps + MatX[8][0];
    U[1][2][1] = eps + MatX[9][0];
    U[1][3][0] = eps + MatX[10][0];
    U[1][3][1] = eps + MatX[11][0];
    U[2][0][0] =  U[0][2][0];
    U[2][0][1] = -U[0][2][1];
    U[2][1][0] =  U[1][2][0];
    U[2][1][1] = -U[1][2][1];
    U[2][2][0] = eps + MatX[12][0];
    U[2][2][1] = 0.;
    U[2][3][0] = eps + MatX[13][0];
    U[2][3][1] = eps + MatX[14][0];
    U[3][0][0] =  U[0][3][0];
    U[3][0][1] = -U[0][3][1];
    U[3][1][0] =  U[1][3][0];
    U[3][1][1] = -U[1][3][1];
    U[3][2][0] =  U[2][3][0];
    U[3][2][1] = -U[2][3][1];
    U[3][3][0] = eps + MatX[15][0];
    U[3][3][1] = 0.;
    InverseHermitianMatrix4(U, U_m1);
    }
...
#pragma omp parallel for private(col, M, MM)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if ((strcmp(PolTypeOut,"C2")==0)||(strcmp(PolTypeOut,"T2")==0)) {
          ...
          M[1][1][1] = 0.;
          ProductCmplxMatrix(U,M,MM,2);
          ProductCmplxMatrix(MM,U_m1,M,2);
          M_out[C211][lig][col] = M[0][0][0];
          M_out[C212_re][lig][col] = M[0][1][0];
          M_out[C212_im][lig][col] = M[0][1][1];
          M_out[C222][lig][col] = M[1][1][0];
          }
        if ((strcmp(PolTypeOut,"C3")==0)||(strcmp(PolTypeOut,"T3")==0)) {
          /* Average complex coherency matrix determination*/
          M[0][0][0] = eps + M_in[0][lig][col];
          ...
          M[2][2][1] = 0.;
          ProductCmplxMatrix(U,M,MM,3);
          ProductCmplxMatrix(MM,U_m1,M,3);
          M_out[C311][lig][col] = M[0][0][0];
          M_out[C312_re][lig][col] = M[0][1][0];
          M_out[C312_im][lig][col] = M[0][1][1];
          M_out[C313_re][lig][col] = M[0][2][0];
          M_out[C313_im][lig][col] = M[0][2][1];
          M_out[C322][lig][col] = M[1][1][0];
          M_out[C323_re][lig][col] = M[1][2][0];
          M_out[C323_im][lig][col] = M[1][2][1];
          M_out[C333][lig][col] = M[2][2][0];
          }
        if ((strcmp(PolTypeOut,"C4")==0)||(strcmp(PolTypeOut,"T4")==0)) {
          /* Average complex coherency matrix determination*/
          M[0][0][0] = eps + M_in[0][lig][col];
          ...
          M[3][3][1] = 0.;
          ProductCmplxMatrix(U,M,MM,4);
          ProductCmplxMatrix(MM,U_m1,M,4);
          M_out[C411][lig][col] = M[0][0][0];
          M_out[C412_re][lig][col] = M[0][1][0];
          M_out[C412_im][lig][col] = M[0][1][1];
          M_out[C413_re][lig][col] = M[0][2][0];
          M_out[C413_im][lig][col] = M[0][2][1];
          M_out[C414_re][lig][col] = M[0][3][0];
          M_out[C414_im][lig][col] = M[0][3][1];
          M_out[C422][lig][col] = M[1][1][0];
          M_out[C423_re][lig][col] = M[1][2][0];
          M_out[C423_im][lig][col] = M[1][2][1];
          M_out[C424_re][lig][col] = M[1][3][0];
          M_out[C424_im][lig][col] = M[1][3][1];
          M_out[C433][lig][col] = M[2][2][0];
          M_out[C434_re][lig][col] = M[2][3][0];
          M_out[C434_im][lig][col] = M[2][3][1];
          M_out[C444][lig][col] = M[3][3][0];
          }
        } else {
    ...
    free_matrix3d_float(MM, Npp, Npp);
    }
