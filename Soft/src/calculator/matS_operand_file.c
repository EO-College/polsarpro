...
Description :  MatS (operand) File = MatS
...
/* LOCAL VARIABLES */
  FILE *in_file;
  char fileformat[10], operand[10];
  char file_in[FilePathLength];
...
if(argc < 19) {
  ...
  get_commandline_prm(argc,argv,"-it",str_cmd_prm,fileformat,1,UsageHelp);
...
/*-----------------------------------------------------------------*/   
  ...
  if (strcmp(fileformat,"float")==0) {
    F_in = matrix_float(NligBlock[0], Ncol); 
    } else {
    F_in = matrix_float(NligBlock[0], 2*Ncol); 
    }
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (strcmp(fileformat,"float")==0) {
    read_block_matrix_float(in_file, F_in, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, NwinL, NwinC, Off_lig, Off_col, Ncol);
    } else {
    read_block_matrix_cmplx(in_file, F_in, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, NwinL, NwinC, Off_lig, Off_col, Ncol);
    }

  read_block_S2_noavg(in_datafile, M_in, "S2", 4, Nb, NbBlock, NligBlock[Nb], Ncol, 1, 1, Off_lig, Off_col, Ncol);
  
xr = xi = 0.;
#pragma omp parallel for private(col, Np) firstprivate(xr, xi)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    if (omp_get_thread_num() == 0) PrintfLine(lig,NligBlock[Nb]);
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if (strcmp(fileformat,"float")==0) {
          xr = F_in[lig][col]; xi = 0.;
          } else {
          xr = F_in[lig][2*col]; xi = F_in[lig][2*col+1];
          }
        if (strcmp(operand,"addfile") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = M_in[Np][lig][2*col]+xr;
            M_out[Np][lig][2*col+1] = M_in[Np][lig][2*col+1]+xi;
            }
          }
        ...
        if (strcmp(operand,"mulfile") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = M_in[Np][lig][2*col]*xr-M_in[Np][lig][2*col+1]*xi;
            M_out[Np][lig][2*col+1] = M_in[Np][lig][2*col+1]*xr+M_in[Np][lig][2*col]*xi;
            }
          }
        if (strcmp(operand,"divfile") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = (M_in[Np][lig][2*col]*xr+M_in[Np][lig][2*col+1]*xi)/(xr*xr+xi*xi+eps);
            M_out[Np][lig][2*col+1] = (M_in[Np][lig][2*col+1]*xr-M_in[Np][lig][2*col]*xi)/(xr*xr+xi*xi+eps);
            }
          }
        } else {
        for (Np = 0; Np < NpolarOut; Np++) {
