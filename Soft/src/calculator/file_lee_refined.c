...
#pragma omp parallel for private(col)
  for (lig = 0; lig < NligBlock[Nb]+Nwin; lig++) {
    if (omp_get_thread_num() == 0) if (NbBlock <= 2) PrintfLine(lig,NligBlock[Nb]+Nwin);
...
mean = Npoints = 0.;
#pragma omp parallel for private(col, k, l) firstprivate(mean, Npoints)
  /* Filtering Element per Element */  
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    if (omp_get_thread_num() == 0) if (NbBlock <= 2) PrintfLine(lig,NligBlock[Nb]);
