...
Description :  MatM (operand) MatM = MatM
...
/*-----------------------------------------------------------------*/   
  ...
  M_in1 = matrix3d_float(NpolarIn, NligBlock[0], Ncol);
  M_in2 = matrix3d_float(NpolarIn, NligBlock[0], Ncol);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  read_block_TCI_noavg(in_datafile1, M_in1, NpolarIn, Nb, NbBlock, NligBlock[Nb], Ncol, 1, 1, Off_lig, Off_col, Ncol);
  read_block_TCI_noavg(in_datafile2, M_in2, NpolarIn, Nb, NbBlock, NligBlock[Nb], Ncol, 1, 1, Off_lig, Off_col, Ncol);
...
#pragma omp parallel for private(col, Np)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        for (Np = 0; Np < NpolarOut; Np++) 
          M_out[Np][lig][col] = M_in1[Np][lig][col]+M_in2[Np][lig][col];
        } else {
