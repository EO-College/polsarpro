...
File   : complex_coherence_estimation_PP.c
...
strcat(UsageHelp," (string)	-type	coherence type : Ch1, Ch2, Ch1pCh2, Ch1mCh2\n");
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
     ...
     if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
      if (strcmp(OutType,"Ch1") == 0) {
        a_re = 0.5*(Mean[T413_re] + Mean[T414_re] + Mean[T423_re] + Mean[T424_re]);
        a_im = 0.5*(Mean[T413_im] + Mean[T414_im] + Mean[T423_im] + Mean[T424_im]);
        b_re = 0.5*(Mean[T411]+Mean[T422]) + Mean[T412_re];
        c_re = 0.5*(Mean[T433]+Mean[T444]) + Mean[T434_re];
        }
      if (strcmp(OutType,"Ch2") == 0) {
        a_re = 0.5*(Mean[T413_re] - Mean[T414_re] - Mean[T423_re] + Mean[T424_re]);
        a_im = 0.5*(Mean[T413_im] - Mean[T414_im] - Mean[T423_im] + Mean[T424_im]);
        b_re = 0.5*(Mean[T411]+Mean[T422]) - Mean[T412_re];
        c_re = 0.5*(Mean[T433]+Mean[T444]) - Mean[T434_re];
        }
      if (strcmp(OutType,"Ch1pCh2") == 0) {
        a_re = 2.0 * Mean[T413_re];
        a_im = 2.0 * Mean[T413_im];
        b_re = 2.0 * Mean[T411];
        c_re = 2.0 * Mean[T433];
        }
      if (strcmp(OutType,"Ch1mCh2") == 0) {
        a_re = 2.0 * Mean[T424_re];
        a_im = 2.0 * Mean[T424_im];
        b_re = 2.0 * Mean[T422];
        c_re = 2.0 * Mean[T444];
        }
