...
File   : complex_coherence_opt_NR.c
...
/* OUTPUT FILE OPENING*/
  ...
  sprintf(file_name, "%scmplx_coh_Opt_NR3.bin", out_dir);
...
#pragma omp parallel for private(col, Np, k, l, Mean, Buffer, theta, theta0, mod0, T, TT11, TT12, TT22, Tmp11, Tmp12, Tmp22, Tmp, V1, hV1, iV1, L, A, H) firstprivate(Nvalid, trace_re, trace_im, epstheta, theta1, p)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
      ...
      M_out3[lig][2*col] = mod0[2]*cos(theta0[2]);
      M_out3[lig][2*col+1] = mod0[2]*sin(theta0[2]);
    ...
    cplx_free_matrix(iV1,3);
    cplx_free_matrix(hV1,3);
    free_vector_float(L);
    cplx_free_matrix(A,3);
    cplx_free_matrix(H,3);
    }
