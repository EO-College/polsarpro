...
File   : complex_coherence_opt_PD_PP.c
...
strcpy(UsageHelp,"\ncomplex_coherence_opt_PD_PP.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%scmplx_coh_PDLow.bin", out_dir);
  if ((out_file1 = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%scmplx_coh_PDHigh.bin", out_dir);
...
theta = trace_re = trace_im = 0.;
Nvalid = 0.;
#pragma omp parallel for private(col, Np, k, l, Mean, Buffer, coh_pha, coh_mag, gmin2, gmax2, T, TT11, TT12, TT22, TT12p, hTT12p, Tmp11, Tmp12, Tmp, V1, hV1, L) firstprivate(Nvalid, theta, trace_re, trace_im)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    if (omp_get_thread_num() == 0) PrintfLine(lig,NligBlock[Nb]);
    Mean = vector_float(NpolarOut);
    Buffer = vector_float(NpolarOut);
    coh_pha = vector_float(nparam_out);
    coh_mag = vector_float(nparam_out);
    gmin2  = vector_float(nparam_out);
    gmax2  = vector_float(nparam_out);
    T  = cplx_matrix(2,2);
    ...
    if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
      ...
      trace_im = 0.;
      for(k=0; k<2; k++) {
        trace_re = trace_re + TT12[k][k].re;
        trace_im = trace_im + TT12[k][k].im;
        }

      theta = pi/2. - atan2(trace_im, trace_re);
      for(k=0; k<2; k++) {
        for(l=0; l<2; l++) {
          Tmp[k][l].re = 0.;
          Tmp[k][l].im = 0.;
      ...
      for(k=0; k<2; k++) {
        for(l=0; l<2; l++) {
          Tmp[k][l].re =  (TT12p[k][l].im - hTT12p[k][l].im) / 2.;
          Tmp[k][l].im = -(TT12p[k][l].re - hTT12p[k][l].re) / 2.;
          }
        }

      cplx_inv_mat2(Tmp,Tmp11);
      cplx_mul_mat(Tmp11,Tmp12,Tmp,2,2);
      ...
      gmax2[0] = Tmp12[0][0].re / sqrt(cmod(Tmp11[0][0]) * cmod(Tmp11[0][0]));
      gmax2[1] = Tmp12[0][0].im / sqrt(cmod(Tmp11[0][0]) * cmod(Tmp11[0][0]));
      gmin2[0] = Tmp12[1][1].re / sqrt(cmod(Tmp11[1][1]) * cmod(Tmp11[1][1]));
      gmin2[1] = Tmp12[1][1].im / sqrt(cmod(Tmp11[1][1]) * cmod(Tmp11[1][1]));
    
      coh_pha[0] = atan2(gmax2[1], gmax2[0]);  
      coh_pha[1] = atan2(gmin2[1], gmin2[0]);
      coh_mag[0] = sqrt(gmax2[0]*gmax2[0] + gmax2[1]*gmax2[1]);
      coh_mag[1] = sqrt(gmin2[0]*gmin2[0] + gmin2[1]*gmin2[1]);

      M_out1[lig][2*col] = coh_mag[0]*cos(coh_pha[0]);
      M_out1[lig][2*col+1] = coh_mag[0]*sin(coh_pha[0]);
      if(isnan(M_out1[lig][2*col])+isnan(M_out1[lig][2*col+1])) {
        M_out1[lig][2*col]=1.; M_out1[lig][2*col+1]=0.;
        }
      M_out2[lig][2*col] = coh_mag[1]*cos(coh_pha[1]);
      M_out2[lig][2*col+1] = coh_mag[1]*sin(coh_pha[1]);
    ...
    free_vector_float(coh_pha);
    free_vector_float(coh_mag);
    free_vector_float(gmin2);
    free_vector_float(gmax2);
    cplx_free_matrix(T,2);
    cplx_free_matrix(TT11,2);
    cplx_free_matrix(TT12,2);
    cplx_free_matrix(TT22,2);
    cplx_free_matrix(TT12p,2);
    cplx_free_matrix(hTT12p,2);
