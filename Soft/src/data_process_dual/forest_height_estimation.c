...
int main(int argc, char *argv[])
{
  ...
  FILE *file_kz;
  FILE *file_opt1, *file_opt2, *file_opt3;
  FILE *file_hhhh, *file_xxxx, *file_vvvv;
  FILE *file_llll, *file_lrlr, *file_rrrr;
...
********************************************************************/
  ...
  opt3= cplx_vector(Ncol);
  hhhh= cplx_vector(Ncol);
  vvvv= cplx_vector(Ncol);
  xxxx= cplx_vector(Ncol);
  llll= cplx_vector(Ncol);
  rrrr= cplx_vector(Ncol);
  lrlr= cplx_vector(Ncol);
  pauli1= cplx_vector(Ncol);
  pauli2= cplx_vector(Ncol);
  hhvv= cplx_vector(Ncol);
...
********************************************************************/
  ...
  /* OPT3 */
  sprintf(filename, "%scmplx_coh_Opt3.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_Opt3.bin", in_dir);
  if ((file_opt3 = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* HHHH */
  sprintf(filename, "%scmplx_coh_HH.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_HH.bin", in_dir);
  if ((file_hhhh = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* VVVV */
  sprintf(filename, "%scmplx_coh_VV.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_VV.bin", in_dir);
  if ((file_vvvv = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* XXXX */
  sprintf(filename, "%scmplx_coh_HV.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_HV.bin", in_dir);
  if ((file_xxxx = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* LLLL */
  sprintf(filename, "%scmplx_coh_LL.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_LL.bin", in_dir);
  if ((file_llll = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* RRRR */
  sprintf(filename, "%scmplx_coh_RR.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_RR.bin", in_dir);
  if ((file_rrrr = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* LRLR */
  sprintf(filename, "%scmplx_coh_LR.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_LR.bin", in_dir);
  if ((file_lrlr = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* PAULI1 */
  sprintf(filename, "%scmplx_coh_HHpVV.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_HHpVV.bin", in_dir);
  if ((file_pauli1 = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* PAULI2 */
  sprintf(filename, "%scmplx_coh_HHmVV.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_HHmVV.bin", in_dir);
  if ((file_pauli2 = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);

  /* HHVV */
  sprintf(filename, "%scmplx_coh_HHVV.bin", in_dir);
  if (CohAvgFlag != 0) sprintf(filename, "%scmplx_coh_avg_HHVV.bin", in_dir);
  if ((file_hhvv = fopen(filename, "rb")) == NULL) edit_error("Could not open input file : ", filename);
...
for (iii= 0; iii< Nlig; iii++) {
  ...
  /* OPT3 */
  fread(header,sizeof(float),2*Ncol,file_opt3);
  for (i= 0; i< Ncol; i++) {
    opt3[i].re= header[2*i];
    opt3[i].im= header[2*i+1];
    }

  /* HHHH */
  fread(header,sizeof(float),2*Ncol,file_hhhh);
  for (i= 0; i< Ncol; i++) {
    hhhh[i].re= header[2*i];
    hhhh[i].im= header[2*i+1];
    }

  /* VVVV */
  fread(header,sizeof(float),2*Ncol,file_vvvv);
  for (i= 0; i< Ncol; i++) {
    vvvv[i].re= header[2*i];
    vvvv[i].im= header[2*i+1];
    }

  /* XXXX */
  fread(header,sizeof(float),2*Ncol,file_xxxx);
  for (i= 0; i< Ncol; i++) {
    xxxx[i].re= header[2*i];
    xxxx[i].im= header[2*i+1];
    }

  /* LLLL */
  fread(header,sizeof(float),2*Ncol,file_llll);
  for (i= 0; i< Ncol; i++) {
    llll[i].re= header[2*i];
    llll[i].im= header[2*i+1];
    }

  /* RRRR */
  fread(header,sizeof(float),2*Ncol,file_rrrr);
  for (i= 0; i< Ncol; i++) {
    rrrr[i].re= header[2*i];
    rrrr[i].im= header[2*i+1];
    }

  /* LRLR */
  fread(header,sizeof(float),2*Ncol,file_lrlr);
  for (i= 0; i< Ncol; i++) {
    lrlr[i].re= header[2*i];
    lrlr[i].im= header[2*i+1];
    }
  ...
  /* HHVV */
  fread(header,sizeof(float),2*Ncol,file_hhvv);
  for (i= 0; i< Ncol; i++) {
    hhvv[i].re= header[2*i];
    hhvv[i].im= header[2*i+1];
    }
  ...
  for (i= 0; i< Ncol; i++) {
    ...
    coh_org[i][2]= coh_rot[i][2]= opt3[i];
    coh_org[i][3]= coh_rot[i][3]= vvvv[i];
    coh_org[i][4]= coh_rot[i][4]= xxxx[i];
    coh_org[i][5]= coh_rot[i][5]= hhhh[i];
    coh_org[i][6]= coh_rot[i][6]= llll[i];
    coh_org[i][7]= coh_rot[i][7]= lrlr[i];
    coh_org[i][8]= coh_rot[i][8]= rrrr[i];
    coh_org[i][9]= coh_rot[i][9]= pauli1[i];
    coh_org[i][10]= coh_rot[i][10]= pauli2[i];
    coh_org[i][11]= coh_rot[i][11]= hhvv[i];
    }
