...
/* OUTPUT FILE OPENING*/
  if (element_index == 12) {
    sprintf(FileData, "%sRoM-Ch1_M-Ch2.bin", out_dir);
    if ((out_file = fopen(FileData, "wb")) == NULL)
      edit_error("Could not open output file : ", FileData);
    }
  if (element_index == 13) {
    sprintf(FileData, "%sRoM-Ch1_S-Ch1.bin", out_dir);
    if ((out_file = fopen(FileData, "wb")) == NULL)
      edit_error("Could not open output file : ", FileData);
    }
  if (element_index == 14) {
    sprintf(FileData, "%sRoM-Ch1_S-Ch2.bin", out_dir);
    if ((out_file = fopen(FileData, "wb")) == NULL)
      edit_error("Could not open output file : ", FileData);
    }
  if (element_index == 23) {
    sprintf(FileData, "%sRoM-Ch2_S-Ch1.bin", out_dir);
    if ((out_file = fopen(FileData, "wb")) == NULL)
      edit_error("Could not open output file : ", FileData);
    }
  if (element_index == 24) {
    sprintf(FileData, "%sRoM-Ch2_S-Ch2.bin", out_dir);
    if ((out_file = fopen(FileData, "wb")) == NULL)
      edit_error("Could not open output file : ", FileData);
    }
  if (element_index == 34) {
    sprintf(FileData, "%sRoS-Ch1_S-Ch2.bin", out_dir);
    if ((out_file = fopen(FileData, "wb")) == NULL)
      edit_error("Could not open output file : ", FileData);
    }
...
#pragma omp parallel for private(col, Np, k, l, mean, Buffer) firstprivate(Nvalid, Cxy_re, Cxy_im, Cxx, Cyy)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
      if (element_index == 12) {
        Cxx = (Buffer[T411]+2.*Buffer[T412_re]+Buffer[T422])/2.;
        Cyy = (Buffer[T411]-2.*Buffer[T412_re]+Buffer[T422])/2.;
        Cxy_re = (Buffer[T411]-Buffer[T422])/2.;
        Cxy_im = (-2.*Buffer[T412_im])/2.;
        }
      if (element_index == 13) {
        Cxx = (Buffer[T411]+2.*Buffer[T412_re]+Buffer[T422])/2.;
        Cyy = (Buffer[T433]+2.*Buffer[T434_re]+Buffer[T444])/2.;
        Cxy_re = (Buffer[T413_re]+Buffer[T414_re]+Buffer[T423_re]+Buffer[T424_re])/2.;
        Cxy_im = (Buffer[T413_im]+Buffer[T414_im]+Buffer[T423_im]+Buffer[T424_im])/2.;
        }
      if (element_index == 14) {
        Cxx = (Buffer[T411]+2.*Buffer[T412_re]+Buffer[T422])/2.;
        Cyy = (Buffer[T433]-2.*Buffer[T434_re]+Buffer[T444])/2.;
      ...
      if (element_index == 34) {
        Cxx = (Buffer[T433]+2.*Buffer[T434_re]+Buffer[T444])/2.;
        Cyy = (Buffer[T433]-2.*Buffer[T434_re]+Buffer[T444])/2.;
        Cxy_re = (Buffer[T433]-Buffer[T444])/2.;
        Cxy_im = (-2.*Buffer[T434_im])/2.;
        }
