...
File   : complex_coherence_loci_difference_PP.c
...
/* Internal variables */
  ...
  float theta, coh_re1, coh_im1, coh_re2, coh_im2;
...
/* Matrix arrays */
  ...
  float coh_pha_diff, coh_mag_diff, coh_pha_max, coh_mag_max;  
...
strcpy(UsageHelp,"\ncomplex_coherence_loci_difference.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%scmplx_coh_maxdiff_PhaLow.bin", out_dir);
  if ((out_file1 = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%scmplx_coh_maxdiff_PhaHigh.bin", out_dir);
  if ((out_file2 = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%scmplx_coh_maxdiff_MagLow.bin", out_dir);
  if ((out_file3 = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%scmplx_coh_maxdiff_MagHigh.bin", out_dir);
...
ks = ls = kss = lss = 0;
theta = coh_re1 = coh_im1 = coh_re2 = coh_im2 = 0.;
coh_pha_diff = coh_mag_diff = coh_pha_max = coh_mag_max = 0.;  
Nvalid = 0.;
#pragma omp parallel for private(col, Np, i, k, l, Mean, Buffer, coh_pha, coh_mag, gmin2, gmax2, T, iT, TT11, TT12, TT22, TT12p, hTT12p, Tmp11, Tmp12, Tmp, V1, hV1, L) firstprivate(Nvalid, ks, ls, kss, lss, theta, coh_re1, coh_im1, coh_re2, coh_im2, coh_pha_diff, coh_mag_diff, coh_pha_max, coh_mag_max)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
      ...
      for (k=0; k<2*p; k++) {
        coh_re1 = coh_mag[k]*cos(coh_pha[k]);
        coh_im1  = coh_mag[k]*sin(coh_pha[k]);
        for (l=(k+1); l<2*p; l++) {
          coh_re2 = coh_mag[l]*cos(coh_pha[l]);
          coh_im2 = coh_mag[l]*sin(coh_pha[l]);
          coh_pha_diff = sqrt((coh_pha[k]-coh_pha[l])*(coh_pha[k]-coh_pha[l]));  
          coh_mag_diff = sqrt((coh_re2-coh_re1)*(coh_re2-coh_re1) + (coh_im2-coh_im1)*(coh_im2-coh_im1));
    
          if (coh_pha_diff>coh_pha_max) {
            coh_pha_max = coh_pha_diff;
            lss = l; kss = k;
            if (coh_pha[lss]>coh_pha[kss]) { lss = k; kss = l; } 
            }
  
          if (coh_mag_diff>coh_mag_max) {
            coh_mag_max = coh_mag_diff; 
            ls = l; ks = k; 
            if (coh_pha[ls]>coh_pha[ks]) { ls = k; ks = l; }
            }
          }
        }
