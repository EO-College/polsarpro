...
/* MATRIX ALLOCATION */
  ...
  TT  = cplx_matrix(2,2);
  iTT  = cplx_matrix(2,2);
  Z12p  = cplx_matrix(2,2);
  Z12r  = cplx_matrix(2,2);
  hZ12p  = cplx_matrix(2,2);
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        cplx_htransp_mat(Z12p,hZ12p,2,2);
        for (k = 0; k < 2; k++) {
          for (l = 0; l < 2; l++) {
