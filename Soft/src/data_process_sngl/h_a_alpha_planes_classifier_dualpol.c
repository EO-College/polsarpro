...
strcpy(UsageHelp,"\nh_a_alpha_planes_classifier_dualpol.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE CLOSING*/
  ...
  bmp_seg_pl_dual(seg_pl, N_pl, N_pl, filename, type_H_alpha, ColorMap9);
  */
}
/********************************************************************
********************************************************************/
...
/* OUTPUT FILE CLOSING*/
  ...
  bmp_occ_pl_dual(occ_pl, N_pl, N_pl, "jet", filename, type_A_alpha);

  sprintf(filename, "%s%s", out_dir, "A_alpha_segmented_plane");
  bmp_seg_pl_dual(seg_pl, N_pl, N_pl, filename, type_A_alpha, ColorMap9);
  */
}
/********************************************************************
********************************************************************/
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      /*** Comparison to the alpha and h borders for each pixel ***/
        ...
        /* segment values ranging from 1 to 9 */
        seg_im[ligg][col] = (float) r6 * 8 + r5 * 9 + r3 * 7 + r4 * 6 + r1 * 5 + r2 * 4;
...
/* OUTPUT FILE CLOSING*/
  ...
  bmp_occ_pl_dual(occ_pl, N_pl, N_pl, "jet", filename, type_H_A);

  sprintf(filename, "%s%s", out_dir, "H_A_segmented_plane");
  bmp_seg_pl_dual(seg_pl, N_pl, N_pl, filename, type_H_A, ColorMap9);
  */
}
/********************************************************************
********************************************************************/
...
/* Horizontal borders */
  for (col = 0; col < ncol; col++)
    border_im[(int) floor(ncol * lim_A)+1][col] = 1;
