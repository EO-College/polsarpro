...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (strcmp(PolTypeOut,"T4")==0) T4_to_T3(M_in, NligBlock[Nb], Sub_Ncol + NwinC, 0, 0);
  if (strcmp(PolTypeOut,"C4")==0) C4_to_T3(M_in, NligBlock[Nb], Sub_Ncol + NwinC, 0, 0);

mu_s = mu_d = mu_c = fv = x11 = x12 = x22 = P = T11s = T12s = T22s = x = delta = LL = RR = LRre = LRim = gamma_re = gamma_im = 0.;
#pragma omp parallel for private(col, k, l, M, V, lambda, M_avg) firstprivate(p, mu_s, mu_d, mu_c, fv, x11, x12, x22, P, T11s, T12s, T22s, x, delta, LL, RR, LRre, LRim, gamma_re, gamma_im) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        mu_s = (M_avg[T311][col] - M_avg[T333][col]) / (M_avg[T311][col] + M_avg[T333][col] + eps);
        mu_d = (M_avg[T322][col] - M_avg[T333][col]) / (M_avg[T322][col] + M_avg[T333][col] + eps);
        mu_c = (M_avg[T311][col] + M_avg[T322][col] - 2.*M_avg[T333][col]) / (M_avg[T311][col] + M_avg[T322][col] + 2.*M_avg[T333][col] + eps);
        ...
        if (VegModel == 2) {
          P = 10.*log10((M_avg[T311][col]+M_avg[T322][col]-2.*M_avg[T312_re][col])/(M_avg[T311][col]+M_avg[T322][col]+2.*M_avg[T312_re][col]));
          if (P < -2.) {
        ...
        T11s = M_avg[T311][col] - (1. - mu_s)*x11*fv;
        T12s = M_avg[T312_re][col] - (1. - mu_c)*x12*fv;
        T22s = M_avg[T322][col] - (1. - mu_d)*x22*fv;
        M_out[Flag[fT11s]][lig][col] = T11s;
        M_out[Flag[fT12s]][lig][col] = T12s;
        M_out[Flag[fT22s]][lig][col] = T22s;
        
        if (Flag[RbyG] != -1) {
          x = (T22s - mu_d*M_avg[T333][col]) / (T22s + mu_d*M_avg[T333][col]);
          l = 0;
          for (k = 1; k < 1001; k++) {
        ...
        if (Flag[RbyC] != -1) {
          /*
          RRre = M_avg[T322][col]/2.; RRim = M_avg[T333][col]/2.;
          LLre = -M_avg[T322][col]/2.; LLim = M_avg[T333][col]/2.;
          ...
          */
          LL = 0.5*(M_avg[T333][col] + M_avg[T322][col] + 2.*M_avg[T323_im][col]);
          RR = 0.5*(M_avg[T333][col] + M_avg[T322][col] - 2.*M_avg[T323_im][col]);
          LRre = 0.5*(M_avg[T333][col] - M_avg[T322][col]);
          LRim = -M_avg[T323_re][col];
        ...
        if (Flag[SfromX] != -1) {
          if ((T11s > 0.)&&(T22s > 0.)) {
            M_out[Flag[SfromX]][lig][col] = (T22s + (mu_c * M_avg[T333][col])) / (T11s+eps);
