...
HHHHv = HVHVv = VVVVv = HHVVvre = sq_rt = alp1 = alp2 = alp3 = alpmin = Lambda1 = Lambda2 = OMEGA1 = OMEGA2 = A0A0 = B0pB = 0.;
ALPre = ALPim = BETre = BETim = HHHH = HVHV = VVVV = HHVVre = HHVVim = ODD = DBL = VOL = p1 = p2 = p3 = 0.;
CC11 = CC13_re = CC13_im = CC22 = CC33 = FV = ratio = 0.;
#pragma omp parallel for private(col, M_avg) firstprivate(CC11, CC13_re, CC13_im, CC22, CC33, FV, ratio, ALPre, ALPim, BETre, BETim, HHHH, HVHV, VVVV, HHVVre, HHVVim, ODD, DBL, VOL, p1, p2, p3, HHHHv, HVHVv, VVVVv, HHVVvre, sq_rt, alp1, alp2, alp3, alpmin, Lambda1, Lambda2, OMEGA1, OMEGA2, A0A0, B0pB) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_avg = matrix_float(NpolarOut,Sub_Ncol);
