...
FV = FS = FSre = FSim = FD = FDre = FDim = FH = FHim = FW = FWre = FWim = 0.;
ALPre = ALPim = BETre = BETim = GAM = GAMre = GAMim = RHO = RHOre = RHOim = 0.;
HHHH = HVHV = VVVV = HHVVre = HHVVim = HHHVre = HHHVim = HVVVre = HVVVim = 0.;
X = Y = Zre = Zim = ratio = numre = numim = denre = denim = den = 0.;
#pragma omp parallel for private(col, M_avg) firstprivate(FV, FS, FSre, FSim, FD, FDre, FDim, FH, FHim, FW, FWre, FWim, ALPre, ALPim, BETre, BETim, GAM, GAMre, GAMim, RHO, RHOre, RHOim, HHHH, HVHV, VVVV, HHVVre, HHVVim, HHHVre, HHHVim, HVVVre, HVVVim, X, Y, Zre, Zim, ratio, numre, numim, denre, denim, den) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_avg = matrix_float(NpolarOut,Sub_Ncol);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        HHHH = M_avg[C311][col];
        HHHVre = M_avg[C312_re][col]/sqrt(2.);
        HHHVim = M_avg[C312_im][col]/sqrt(2.);
        HHVVre = M_avg[C313_re][col];
        HHVVim = M_avg[C313_im][col];
        HVHV = M_avg[C322][col]/2.;
        HVVVre = M_avg[C323_re][col]/sqrt(2.);
        HVVVim = M_avg[C323_im][col]/sqrt(2.);
        VVVV = M_avg[C333][col];
        ...
        denre = GAMre*RHOre+GAMim*RHOim-RHOre;
        denim = GAMim*RHOre-GAMre*RHOim-RHOim;
...
//      FHre = HHHVre+HVVVre -FW*(GAMre*RHOre+GAMim*RHOim+RHOre);
        ...
        if (M_hlx[lig][col] < SpanMin) M_hlx[lig][col] = SpanMin;
        if (M_wir[lig][col] < SpanMin) M_wir[lig][col] = SpanMin;
        if (M_dblhlx[lig][col] < SpanMin) M_dblhlx[lig][col] = SpanMin;
