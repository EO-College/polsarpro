...
File  : yamaguchi_3components_reconstruction.c
...
Description :  Yamaguchi 3 components reconstruction
...
strcpy(UsageHelp,"\nyamaguchi_3components_reconstruction.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
#pragma omp parallel for private(col, Np, M_avg) firstprivate(CC11, CC13_re, CC13_im, CC22, CC33, FS, FD, FV, ALPre, ALPim, BETre, BETim, HHHH, HVHV, VVVV, HHVVre, HHVVim, rtemp, ratio) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_avg = matrix_float(NpolarOut,Sub_Ncol);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        M_odd[C311][lig][col] = FS * (1. + BETre * BETre + BETim * BETim);
        M_odd[C312_re][lig][col] = 0.; M_odd[C312_im][lig][col] = 0.;
        M_odd[C313_re][lig][col] = FS * BETre; M_odd[C313_im][lig][col] = FS * BETim;
        M_odd[C322][lig][col] = 0.;
        M_odd[C323_re][lig][col] = 0.; M_odd[C323_im][lig][col] = 0.;
        M_odd[C333][lig][col] = FS;

        M_dbl[C311][lig][col] = FD * (1. + ALPre * ALPre + ALPim * ALPim);
        M_dbl[C312_re][lig][col] = 0.; M_dbl[C312_im][lig][col] = 0.;
        M_dbl[C313_re][lig][col] = FD * ALPre; M_dbl[C313_im][lig][col] = FD * ALPim;
        M_dbl[C322][lig][col] = 0.;
        M_dbl[C323_re][lig][col] = 0.; M_dbl[C323_im][lig][col] = 0.;
        M_dbl[C333][lig][col] = FD;

        M_vol[C311][lig][col] = FV * (3. / 8.);
        M_vol[C312_re][lig][col] = 0.; M_vol[C312_im][lig][col] = 0.;
        M_vol[C313_re][lig][col] = FV / 8.; M_vol[C313_im][lig][col] = 0.;
        M_vol[C322][lig][col] = FV * (2. / 8.);
        M_vol[C323_re][lig][col] = 0.; M_vol[C323_im][lig][col] = 0.;
        M_vol[C333][lig][col] = FV * (3. / 8.);

        if (ratio <= -2.) {
          M_vol[C311][lig][col] = FV * (8. / 15.);
          M_vol[C312_re][lig][col] = 0.; M_vol[C312_im][lig][col] = 0.;
          M_vol[C313_re][lig][col] = FV * (2. / 15.); M_vol[C313_im][lig][col] = 0.;
          M_vol[C322][lig][col] = FV * (4. / 15.);
          M_vol[C323_re][lig][col] = 0.; M_vol[C323_im][lig][col] = 0.;
          M_vol[C333][lig][col] = FV * (3. / 15.);      
          }

        if (ratio > 2.) {
          M_vol[C311][lig][col] = FV * (3. / 15.);
          M_vol[C312_re][lig][col] = 0.; M_vol[C312_im][lig][col] = 0.;
          M_vol[C313_re][lig][col] = FV * (2. / 15.); M_vol[C313_im][lig][col] = 0.;
          M_vol[C322][lig][col] = FV * (4. / 15.);
          M_vol[C323_re][lig][col] = 0.; M_vol[C323_im][lig][col] = 0.;
          M_vol[C333][lig][col] = FV * (8. / 15.);
          }
