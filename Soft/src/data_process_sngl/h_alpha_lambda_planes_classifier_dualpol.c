...
File  : h_alpha_lambda_planes_classifier_dualpol.c
...
void bmp_occ_pl_dual(float **mat, int li, int co, char *cmap, char *nom, int type);
void bmp_seg_pl_dual(float **mat, int li, int co, char *name, int type, char *ColorMap);
void define_borders_dual(float **quad_mat, int type, int nlig, int ncol);
...
strcpy(UsageHelp,"\nh_alpha_lambda_planes_classifier_dualpol.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/*-----------------------------------------------------------------*/   
  ...
  seg_im_mask = matrix_float(Sub_Nlig, Sub_Ncol);
...
#pragma omp parallel for private(col,c,l) firstprivate(ligg, a1, a2, a3, a4, a5, h1, h2, r1, r2, r3, r4, r5, r6, r7, r8, r9) shared(ligDone, occ_pl, seg_pl)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      seg_im[ligg][col] = 0.;
      seg_im_mask[ligg][col] = 0.;
      MH_out[ligg][col] = 0.;
      MA_out[ligg][col] = 0.;
      if (Valid[lig][col] == 1.) {
        if (ML_in[lig][col] <= medianmin) {
          seg_im_mask[ligg][col] = 1.;
  ...
  sprintf(filename, "%smask_low_lambda.bin", out_dir);
  if ((out_file = fopen(filename, "wb")) == NULL)
    edit_error("Could not open input file : ", filename);
  write_block_matrix_float(out_file, seg_im_mask, Sub_Nlig, Sub_Ncol, 0, 0, Sub_Ncol);
  fclose(out_file);
  ...
  bmp_occ_pl_dual(occ_pl, N_pl, N_pl, "jet", filename, type_H_alpha);

  sprintf(filename, "%s%s", out_dir, "H_alpha_lambda_segmented_plane1");
  bmp_seg_pl_dual(seg_pl, N_pl, N_pl, filename, type_H_alpha, ColorMap27);
  */
...
#pragma omp parallel for private(col,c,l) firstprivate(ligg, a1, a2, a3, a4, a5, h1, h2, r1, r2, r3, r4, r5, r6, r7, r8, r9) shared(ligDone, occ_pl, seg_pl)
  ...
  sprintf(filename, "%smask_medium_lambda.bin", out_dir);
  if ((out_file = fopen(filename, "wb")) == NULL)
    edit_error("Could not open input file : ", filename);
  write_block_matrix_float(out_file, seg_im_mask, Sub_Nlig, Sub_Ncol, 0, 0, Sub_Ncol);
  fclose(out_file);
...
#pragma omp parallel for private(col,c,l) firstprivate(ligg, a1, a2, a3, a4, a5, h1, h2, r1, r2, r3, r4, r5, r6, r7, r8, r9) shared(ligDone, occ_pl, seg_pl)
  ...
  sprintf(filename, "%smask_high_lambda.bin", out_dir);
  if ((out_file = fopen(filename, "wb")) == NULL)
    edit_error("Could not open input file : ", filename);
  write_block_matrix_float(out_file, seg_im_mask, Sub_Nlig, Sub_Ncol, 0, 0, Sub_Ncol);
  fclose(out_file);
...
/* Boundaries definition */
  ...
  define_borders_dual(border_im, type, nlig, ncol);
...
********************************************************************/
void define_borders_dual(float **border_im, int type, int nlig, int ncol)
{
  int lig, col, l, c;
  float m, al, en, an;
