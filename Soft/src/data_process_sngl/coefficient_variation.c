...
Description :  Calculates the coefficient of variation of a binary 
               data file
...
#pragma omp parallel for private(col, xr, xi)
    for (lig = 0; lig < NligBlock[Nb]+NwinL; lig++) {
      if (omp_get_thread_num() == 0) PrintfLine(lig,NligBlock[Nb]+NwinL);
      for (col = 0; col < Sub_Ncol+NwinC; col++) {
        M_in[lig][col] = 0.;
        if (Valid[lig][col] == 1.) {
...
#pragma omp parallel for private(col, xr)
    for (lig = 0; lig < NligBlock[Nb]+NwinL; lig++) {
      if (omp_get_thread_num() == 0) PrintfLine(lig,NligBlock[Nb]+NwinL);
      for (col = 0; col < Sub_Ncol+NwinC; col++) {
        M_in[lig][col] = 0.;
...
Nvalid = value = mean = mean2 = Mmean = Mmean2 = 0.;
#pragma omp parallel for private(col, k, l) firstprivate(Nvalid, value, mean, mean2, Mmean, Mmean2)
