...
Description :  Obtains the statistics of the matrix T3/C3 for a given
               area
...
int main(int argc, char *argv[])
{
  ...
  char *X_Elements[18]     = {"C11","C12","C13","C22","C23","C33","rho_12","rho_13","rho_23","T11","T12","T13","T22","T23","T33","rho_12","rho_13","rho_23"};
  char *X_Elements_part[4] = {"Real Part","Imaginary Part","Amplitude","Phase"};
  char *PDF_test[4]        = {"Gaussian","Exponential","Rayleigh","Uniform"};
  int Elem_jump[6]         = {0,1,3,5,6,8};  
  int Elem_jump2[6]        = {0,6,30,54,60,84};  /* Jumps for Histograms */
  
  /* Temporal matrices */
  float *tmp_file, *tmp_elem, *tmp_elem2, *tmp_elem3, *tmp_elem4, **X3_hist, ***X3, *Xhist, *Yhist;
  ...
  if (argc < 5){
    edit_error("statistics_T3_C3 file_data file_stat file_hist file_hist_label\n","");
  } else {
  ...
  X3       = matrix3d_float(Nelement_X + Rho_Factors,4,Ncol); /* Includes also the correlation factors */
  ...
  X3_hist  = matrix_float((3 + 4 * 3) * 6 + Rho_Factors * 8,BINS); /* Histograms of the elements & correlation Factors */
  ...
  for (np = 0; np < Nelement_X; np++){
    ...
    if (np == 0 || np == 3 || np == 5){      /* Reads Diagonal Elements */
      for (col = 0; col < Ncol; col++){  
        ind = (Elem_jump[np] * Ncol) + col;
        X3[np][sab][col] = tmp_file[ind];
      }
    }
    else{
      for (col = 0; col < Ncol; col++){    /* Reads Off-diagonal Elements Real Part */
        ind = (Elem_jump[np] * Ncol) + col;
        X3[np][sre][col] = tmp_file[ind];
      }
      for (col = 0; col < Ncol; col++){    /* Reads Off-diagonal Elements Imaginary Part */
        ind = ((Elem_jump[np] + 1) * Ncol) + col;
        X3[np][sim][col] = tmp_file[ind];
        X3[np][sab][col] = AmplitudeComplex(X3[np][sre][col],X3[np][sim][col]);
        X3[np][sph][col] = PhaseComplex(X3[np][sre][col],X3[np][sim][col]);
      }
    }
  }
  ...
  for (col = 0; col < Ncol; col++){
    ...
    tmp2 = X3[X22][sab][col];
    if ( (tmp>0) && (tmp2>0) ){
      X3[Nelement_X][sre][col] = X3[X12][sre][col] / ( sqrt(tmp * tmp2) );
      X3[Nelement_X][sim][col] = X3[X12][sim][col] / ( sqrt(tmp * tmp2) );
      X3[Nelement_X][sab][col] = AmplitudeComplex(X3[Nelement_X][sre][col],X3[Nelement_X][sim][col]);
      X3[Nelement_X][sph][col] = PhaseComplex(X3[Nelement_X][sre][col],X3[Nelement_X][sim][col]);
    }
    else{
      X3[Nelement_X][sre][col] = 0.;
      X3[Nelement_X][sim][col] = 0.;
      X3[Nelement_X][sab][col] = 0.;
      X3[Nelement_X][sph][col] = 0.;
    }
  }
  /* Factor rho_13*/
  for (col = 0; col < Ncol; col++){
    ...
    tmp2 = X3[X33][sab][col];
    if ( (tmp>0) && (tmp2>0) ){
      X3[Nelement_X + 1][sre][col] = X3[X13][sre][col] / ( sqrt(tmp * tmp2) );
      X3[Nelement_X + 1][sim][col] = X3[X13][sim][col] / ( sqrt(tmp * tmp2) );
      X3[Nelement_X + 1][sab][col] = AmplitudeComplex(X3[Nelement_X + 1][sre][col],X3[Nelement_X + 1][sim][col]);
      X3[Nelement_X + 1][sph][col] = PhaseComplex(X3[Nelement_X + 1][sre][col],X3[Nelement_X + 1][sim][col]);
    }
    else{
      X3[Nelement_X + 1][sre][col] = 0.;
      X3[Nelement_X + 1][sim][col] = 0.;
      X3[Nelement_X + 1][sab][col] = 0.;
      X3[Nelement_X + 1][sph][col] = 0.;
    }
  }
  /* Factor rho_23*/
  for (col = 0; col < Ncol; col++){
    ...
    tmp2 = X3[X33][sab][col];
    if ( (tmp>0) && (tmp2>0) ){
      X3[Nelement_X + 2][sre][col] = X3[X23][sre][col] / ( sqrt(tmp * tmp2) );
      X3[Nelement_X + 2][sim][col] = X3[X23][sim][col] / ( sqrt(tmp * tmp2) );
      X3[Nelement_X + 2][sab][col] = AmplitudeComplex(X3[Nelement_X + 2][sre][col],X3[Nelement_X + 2][sim][col]);
      X3[Nelement_X + 2][sph][col] = PhaseComplex(X3[Nelement_X + 2][sre][col],X3[Nelement_X + 2][sim][col]);
    }
    else{
      X3[Nelement_X + 2][sre][col] = 0.;
      X3[Nelement_X + 2][sim][col] = 0.;
      X3[Nelement_X + 2][sab][col] = 0.;
      X3[Nelement_X + 2][sph][col] = 0.;
    }
  }
  ...
  fprintf(out_file_hist_label,"%i \n",((3 + 3 * 4) * 5) + Rho_Factors * 4); /* Number of labels*/
  
  if (TypeData == 0)
    fprintf(out_file_stat,"C3 MATRIX STATISTICS\n");
  else
    fprintf(out_file_stat,"T3 MATRIX STATISTICS\n");
  ...
  for (np = 0; np < Nelement_X; np++){ 
    ...
    if (np == 0 || np == 3 || np == 5){  /* To differentiate the diagional elements */
      dt_ini = 2; dt_fin = 3;
    }
    else{
      dt_ini = 0; dt_fin = 4;
    ...
    for (dt = dt_ini; dt < dt_fin; dt++){  
      ...
      if (np == 0 || np == 3 || np == 5)      /* Selects the indices for the diagonal and off-diagonal elements */
        ind = Elem_jump2[np];
      else
        ind = Elem_jump2[np] + dt * 6;
        
      for(col = 0;col < BINS; col++){
        X3_hist[ind][col]   = Xhist[col];
        X3_hist[ind + 1][col] = Yhist[col];
      }
      ...
      if( finite(Yhist[col]) && Xhist[col] > 0.0)
          X3_hist[ind + 3][col] = Yhist[col];
        else
          X3_hist[ind + 3][col] = 0.0;
      }
      ...
      for(col = 0;col < BINS; col++){
        if( finite(Yhist[col]) && Xhist[col] > 0.0)
          X3_hist[ind + 4][col] = Yhist[col];
        else
          X3_hist[ind + 4][col] = 0.0;
      }    
      if(dt==0 ||dt==1)
  ...
  for (np = Nelement_X; np < Nelement_X + Rho_Factors; np++){ 
    ...
    for (dt = 0; dt < 4; dt++){  
      ...
      ind = 6 * (3 + 4 * 3) + (np - Nelement_X) * 8 + dt * 2;
  ...
  for(col = 0; col < BINS; col++){
    ind = (3 + 4 * 3);
    for (np = 0; np < ind ; np++){ /* Elements of X3 */
      fprintf(out_file_hist,"%f ",X3_hist[np * 6][col]);
      fprintf(out_file_hist,"%f ",X3_hist[np * 6 + 1][col]);
      fprintf(out_file_hist,"%f ",X3_hist[np * 6 + 2][col]);
      fprintf(out_file_hist,"%f ",X3_hist[np * 6 + 3][col]);
      fprintf(out_file_hist,"%f ",X3_hist[np * 6 + 4][col]);
      fprintf(out_file_hist,"%f ",X3_hist[np * 6 + 5][col]);
    }
    ind = (3 + 4 * 3) * 6;
    for (np = 0; np < Rho_Factors ; np++){ /* Correlations */
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 1][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 2][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 3][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 4][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 5][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 6][col]);
      fprintf(out_file_hist,"%f ",X3_hist[ind + np * 8 + 7][col]);
    }
  ...
  free_matrix3d_float(X3,Nelement_X + Rho_Factors,4);
  ...
  free_matrix_float(X3_hist,6 * (3 + 4 * 3) + Rho_Factors * 8);
