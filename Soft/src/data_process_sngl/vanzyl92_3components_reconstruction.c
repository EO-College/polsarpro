...
#pragma omp parallel for private(col, Np, M_avg) firstprivate(ALPre, ALPim, BETre, BETim, A0A0, B0pB, HHHH, HVHV, VVVV, HHVVre, HHVVim, sq_rt, Lambda1, Lambda2, OMEGA1, OMEGA2) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_avg = matrix_float(NpolarOut,Sub_Ncol);
