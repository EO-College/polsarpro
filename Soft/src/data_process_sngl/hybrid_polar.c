...
/* Local Variables */
  ...
  NBlockA += NpolarIn*(Ncol+NwinC); NBlockB += NpolarOut*NwinL*(Ncol+NwinC);
  /* Mout2 = NpolarOut */
  NBlockA += 0; NBlockB += NpolarIn*Sub_Ncol;
  /* Mout3 = NpolarOut*Nlig*Sub_Ncol */
...
/*-----------------------------------------------------------------*/   
  ...
  M_in = matrix3d_float(NpolarIn, NligBlock[0] + NwinL, Ncol + NwinC);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  /* Case of C,T or I */
  read_block_TCI_noavg(in_datafile, M_in, NpolarIn, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, NwinL, NwinC, Off_lig, Off_col, Ncol);

FlagStop = Niteration = 0;
c11 = c12r = c12i = c13r = c13i = c22 = c23r = c23i = c33 = 0.;
rho_r = rho_i = rho_mod = X = Xnew = 0.;
#pragma omp parallel for private(col, M_out2) firstprivate(c11,c12r,c12i,c13r,c13i,c22,c23r,c23i,c33,rho_r, rho_i, rho_mod, X, Xnew) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_out2 = matrix_float(NpolarIn,Sub_Ncol);
    average_TCI(M_in, Valid, NpolarIn, M_out2, lig, Sub_Ncol, NwinL, NwinC, NwinLM1S2, NwinCM1S2);
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {

        /* RECONSTRUCT PSEUDO QUAD-POL DATA */
        if (strcmp(method,"polar") == 0) {
          rho_r = M_out2[J12re][col] / sqrt(M_out2[J11][col] * M_out2[J22][col]);
          rho_i = M_out2[J12im][col] / sqrt(M_out2[J11][col] * M_out2[J22][col]);
          rho_mod = sqrt(rho_r * rho_r + rho_i * rho_i);
          X = 0.5*(M_out2[J11][col] + M_out2[J22][col]) * (1. - rho_mod) / (3. - rho_mod);
          FlagStop = 0; Niteration = 0;
          while (FlagStop == 0) {
            if (strcmp(mode,"pi4") == 0) { 
              rho_r = (M_out2[J12re][col] - X ) / sqrt((M_out2[J11][col]-X) * (M_out2[J22][col]-X));
              rho_i = M_out2[J12im][col] / sqrt((M_out2[J11][col]-X) * (M_out2[J22][col]-X));
              }
            if (strcmp(mode,"lhv") == 0) { 
              rho_r = (-M_out2[J12im][col] - X ) / sqrt((M_out2[J11][col]-X) * (M_out2[J22][col]-X));
              rho_i = M_out2[J12re][col] / sqrt((M_out2[J11][col]-X) * (M_out2[J22][col]-X));
              }
            ...
            rho_mod = sqrt(rho_r * rho_r + rho_i * rho_i);
            Xnew = 0.5*(M_out2[J11][col] + M_out2[J22][col]) * (1. - rho_mod) / (3. - rho_mod);
            if (Xnew < 0.) FlagStop = 1;
            if ((M_out2[J11][col] - Xnew) <0.) FlagStop = 1;
            if ((M_out2[J22][col] - Xnew) <0.) FlagStop = 1;
          ...
          if (strcmp(mode,"pi4") == 0) { 
            c11 = M_out2[J11][col] - X; c22 = 2.*X; c33 = M_out2[J22][col]-X;
            c13r = M_out2[J12re][col] - X; c13i = M_out2[J12im][col];
            c12r = c12i = c23r = c23i = 0.;
            }
          if (strcmp(mode,"lhv") == 0) { 
            c11 = M_out2[J11][col] - X; c22 = 2.*X; c33 = M_out2[J22][col]-X;
            c13r = -M_out2[J12im][col] + X; c13i = M_out2[J12re][col];
            c12r = c12i = c23r = c23i = 0.;
            }
          if (strcmp(mode,"rhv") == 0) { 
            c11 = M_out2[J11][col] - X; c22 = 2.*X; c33 = M_out2[J22][col]-X;
        ...
        if (strcmp(method,"rotsym") == 0) {
          if (strcmp(mode,"pi4") == 0) { 
            c11 = 0.25*(M_out2[J11][col] + M_out2[J22][col] + 2.*M_out2[J12re][col]);
            c22 = 0.5*(M_out2[J11][col] + M_out2[J22][col] - 2.*M_out2[J12re][col]);
            c33 = 0.25*(M_out2[J11][col] + M_out2[J22][col] + 2.*M_out2[J12re][col]);
            c12r = 0.;
            c12i = 0.5*sqrt(2.)*M_out2[J12im][col];
            c13r = 0.25*(-M_out2[J11][col] - M_out2[J22][col] + 6.*M_out2[J12re][col]);
            c13i = 0.;
            c23r = c12r;
            c23i = c12i;
            }
        ...
        if (strcmp(method,"rotrefsym") == 0) {
          if (strcmp(mode,"pi4") == 0) { 
            c11 = 0.125*(7.*M_out2[J11][col] - M_out2[J22][col] + 2.*M_out2[J12re][col]);
            c22 = 0.25*(M_out2[J11][col] + M_out2[J22][col] - 2.*M_out2[J12re][col]);
            c33 = 0.125*(-M_out2[J11][col] + 7.*M_out2[J22][col] + 2.*M_out2[J12re][col]);
            c12r = 0.;
            c12i = 0.;
            c13r = 0.125*(-M_out2[J11][col] - M_out2[J22][col] + 6.*M_out2[J12re][col]);
            c13i = 0.125*M_out2[J12im][col];
            c23r = c12r;
            c23i = c12i;
            }
          if (strcmp(mode,"lhv") == 0) { 
            c11 = 0.125*(7.*M_out2[J11][col] - M_out2[J22][col] + 2.*M_out2[J12im][col]);
            c22 = 0.125*(M_out2[J11][col] + M_out2[J22][col] + 2.*M_out2[J12im][col]);
            c33 = 0.125*(-M_out2[J11][col] + 7.*M_out2[J22][col] - 2.*M_out2[J12im][col]);
            c12r = 0.;
            c12i = 0.;
            c13r = 0.125*(M_out2[J11][col] + M_out2[J22][col] - 6.*M_out2[J12im][col]);
            c13i = 0.5*M_out2[J12re][col];
            c23r = c12r;
            c23i = c12i;
            }
          if (strcmp(mode,"rhv") == 0) { 
    ...
    free_matrix_float(M_out2,NpolarOut);
    }
