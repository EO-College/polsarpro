...
File  : scattering_mechanism_entropy_freeman.c
...
Description :  Freeman - Yamaguchi Decomposition
...
strcpy(UsageHelp,"\nscattering_mechanism_entropy_freeman.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%s%s", out_dir, "entropy_scatt_mecha_freeman.bin");
...
ALPre = ALPim = BETre = BETim = HHHH = HVHV = VVVV = HHVVre = HHVVim = ODD = DBL = VOL = p1 = p2 = p3 = 0.;
CC11 = CC13_re = CC13_im = CC22 = CC33 = FV = FD = FS = rtemp = ratio = 0.;
#pragma omp parallel for private(col, M_avg) firstprivate(CC11, CC13_re, CC13_im, CC22, CC33, FV, FD, FS, rtemp, ratio, ALPre, ALPim, BETre, BETim, HHHH, HVHV, VVVV, HHVVre, HHVVim, ODD, DBL, VOL, p1, p2, p3) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_avg = matrix_float(NpolarOut,Sub_Ncol);
