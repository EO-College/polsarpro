...
Description :  Barnes2 decomposition
...
MB0 = MC = MD = ME = MF = MG = MH = MB0pB = MB0mB = k1r = k1i = k2r = k2i = k3r = k3i = 0.;
#pragma omp parallel for private(col, Np, M_avg) firstprivate(MB0, MC, MD, ME, MF, MG, MH, MB0pB, MB0mB, k1r, k1i, k2r, k2i, k3r, k3i) shared(ligDone)
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        if (strcmp(PolTypeOut,"T3")==0) {
          MB0 = (M_avg[T322][col] + M_avg[T333][col]) / 2.;
          MB0pB = M_avg[T322][col]; MB0mB = M_avg[T333][col];
          MC = M_avg[T312_re][col]; MD = -M_avg[T312_im][col];
          MH = M_avg[T313_re][col]; MG = M_avg[T313_im][col];
          ME = M_avg[T323_re][col]; MF = M_avg[T323_im][col];

          k1r = (MH + MD) / sqrt(2. * (MB0 + MF));
          k1i = (MC + MG) / sqrt(2. * (MB0 + MF));
          k2r = ME / sqrt(2. * (MB0 + MF));
          k2i = (MB0pB + MF) / sqrt(2. * (MB0 + MF));
          k3r = (MB0mB + MF) / sqrt(2. * (MB0 + MF));
          k3i = ME / sqrt(2. * (MB0 + MF));
          }

        if (strcmp(PolTypeOut,"C3")==0) {
          MC = (M_avg[C311][col] - M_avg[C333][col]) / 2;
          MD = M_avg[C313_im][col];
          MH = (M_avg[C312_re][col] + M_avg[C323_re][col]) / sqrt(2);
          MG = (M_avg[C312_im][col] - M_avg[C323_im][col]) / sqrt(2);
          ME = (M_avg[C312_re][col] - M_avg[C323_re][col]) / sqrt(2);
          MF = (M_avg[C312_im][col] + M_avg[C323_im][col]) / sqrt(2);
          MB0pB = (M_avg[C311][col] - 2 * M_avg[C313_re][col] + M_avg[C333][col]) / 2;
          MB0mB = M_avg[C322][col];
          MB0 = (MB0pB + MB0mB) / 2.;

          k1i = (MC + MG + MB0pB + MF) / 2. / sqrt(MB0 + MF);
          k1r = (MH + MD + ME) / 2. / sqrt(MB0 + MF);
          k2i = sqrt(2.) * ME / 2. / sqrt(MB0 + MF);
          k2r = sqrt(2.) * (MB0mB - MF) / 2. / sqrt(MB0 + MF);
          k3i = (MC + MG - MB0pB - MF) / 2. / sqrt(MB0 + MF);
          k3r = (MH + MD - ME) / 2. / sqrt(MB0 + MF);
          }
