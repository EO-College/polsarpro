...
T11_phi = T22_phi = T33_phi = T12_re_phi = T13_im_phi = T23_im_phi = NewT11 = NewT22 = NewT33 = NewT12re = 0.;
#pragma omp parallel for private(col, M_avg) firstprivate(T11_phi, T22_phi, T33_phi, T12_re_phi,  T13_im_phi, T23_im_phi, NewT11, NewT22, NewT33, NewT12re) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        /* Real Rotation Phi */
        T11_phi = M_avg[T311][col];
        T12_re_phi = M_avg[T312_re][col] * cos(2 * Phi) + M_avg[T313_re][col] * sin(2 * Phi);
//      T12_im_phi = M_avg[T312_im][col] * cos(2 * Phi) + M_avg[T313_im][col] * sin(2 * Phi);
//      T13_re_phi = -M_avg[T312_re][col] * sin(2 * Phi) + M_avg[T313_re][col] * cos(2 * Phi);
        T13_im_phi = -M_avg[T312_im][col] * sin(2 * Phi) + M_avg[T313_im][col] * cos(2 * Phi);
        T22_phi = M_avg[T322][col] * cos(2 * Phi) * cos(2 * Phi) + M_avg[T323_re][col] * sin(4 * Phi) + M_avg[T333][col] * sin(2 * Phi) * sin(2 * Phi);
//      T23_re_phi = 0.5 * (M_avg[T333][col] - M_avg[T322][col]) * sin(4 * Phi) + M_avg[T323_re][col] * cos(4 * Phi);
        T23_im_phi = M_avg[T323_im][col];
        T33_phi = M_avg[T322][col] * sin(2 * Phi) * sin(2 * Phi) - M_avg[T323_re][col] * sin(4 * Phi) + M_avg[T333][col] * cos(2 * Phi) * cos(2 * Phi);
