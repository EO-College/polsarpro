...
Description :  Holm 1 decomposition
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        /*Holm1 algorithm*/
        k1r = sqrt(lambda[0] - lambda[1]) * V[0][0][0];
        k1i = sqrt(lambda[0] - lambda[1]) * V[0][0][1];
        k2r = sqrt(lambda[0] - lambda[1]) * V[1][0][0];
        k2i = sqrt(lambda[0] - lambda[1]) * V[1][0][1];
        k3r = sqrt(lambda[0] - lambda[1]) * V[2][0][0];
        k3i = sqrt(lambda[0] - lambda[1]) * V[2][0][1];
