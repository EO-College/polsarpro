...
Version  : 1.0
Creation : 07/2005 (v2.0 08/2011, v3.0 08/2015)
Update  :
*--------------------------------------------------------------------
...
/* LOCAL VARIABLES */
  ...
  int Config;
  char *PolTypeConf[NPolType] = {"S2", "C3", "T3", "SPP", "C2"};
...
/* INPUT FILE OPENING*/
  for (sub = sub_init; sub < sub_number; sub++) {
    ...
    if (strcmp(PolType,"SPP")==0) sprintf(in_file_name, "%s%i/%s", in_dir, sub, file_in[Np]);
    if (strcmp(PolType,"C2")==0) sprintf(in_file_name, "%s%i/C2/%s", in_dir, sub, file_in[Np]);
...
/* OUTPUT FILE OPENING*/
  ...
  if (strcmp(PolType,"SPP")==0) sprintf(out_file_name, "%s%i/%s", out_dir, sub_init, file_out[Np]);
  if (strcmp(PolType,"C2")==0) sprintf(out_file_name, "%s%i/C2/%s", out_dir, sub_init, file_out[Np]);
...
#pragma omp parallel for private(col, k, l) firstprivate(Nvalid, Mmean, Mmean2)
    for (lig = 0; lig < NligBlock[Nb]; lig++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        M_out[lig][col] = sqrt(CV) / (M_moy[lig][col] + eps);
        }
      }
    }
