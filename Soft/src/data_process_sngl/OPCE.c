...
/* Local Variables */
  ...
  /* Mavg = NpolarOut */
  NBlockA += 0; NBlockB += NpolarOut*Sub_Ncol_OPCE;
...
/*-----------------------------------------------------------------*/   
  ...
  Valid = matrix_float(NligBlock[0] + NwinL, Sub_Ncol_OPCE + NwinC);
...
#pragma omp parallel for private(col)
    for (lig = 0; lig < NligBlock[0] + NwinL; lig++) 
      for (col = 0; col < Sub_Ncol_OPCE + NwinC; col++) 
        Valid[lig][col] = 1.;
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (FlagValid == 1) read_block_matrix_float(in_valid, Valid, Nb, NbBlock, NligBlock[Nb], Sub_Ncol_OPCE, NwinL, NwinC, Off_lig_OPCE, Off_col_OPCE, Ncol);

  if (strcmp(PolTypeIn,"S2")==0) {
    read_block_S2_noavg(in_datafile, M_in, PolTypeOut, NpolarOut, Nb, NbBlock, NligBlock[Nb], Sub_Ncol_OPCE, NwinL, NwinC, Off_lig_OPCE, Off_col_OPCE, Ncol);
    } else {
    /* Case of C,T or I */
    read_block_TCI_noavg(in_datafile, M_in, NpolarOut, Nb, NbBlock, NligBlock[Nb], Sub_Ncol_OPCE, NwinL, NwinC, Off_lig_OPCE, Off_col_OPCE, Ncol);
    }
  if (strcmp(PolTypeOut,"C3")==0) C3_to_T3(M_in, NligBlock[Nb], Sub_Ncol_OPCE + NwinC, 0, 0);
  if (strcmp(PolTypeOut,"C4")==0) C4_to_T3(M_in, NligBlock[Nb], Sub_Ncol_OPCE + NwinC, 0, 0);
  if (strcmp(PolTypeOut,"T4")==0) T4_to_T3(M_in, NligBlock[Nb], Sub_Ncol_OPCE + NwinC, 0, 0);
...
#pragma omp parallel for private(col, M_avg) firstprivate(KT1, KT2, KT3, KT4, KT5, KT6, KT7, KT8, KT9, KT10, A0, A1, A2, A3) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    M_avg = matrix_float(NpolarOut,Sub_Ncol_OPCE);
    average_TCI(M_in, Valid, NpolarOut, M_avg, lig, Sub_Ncol_OPCE, NwinL, NwinC, NwinLM1S2, NwinCM1S2);
    for (col = 0; col < Sub_Ncol_OPCE; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        /* Average Kennaugh matrix determination*/
        KT1 = eps + 0.5 * (M_avg[T311][col] + M_avg[T322][col] + M_avg[T333][col]);
        KT2 = eps + M_avg[T312_re][col];
        KT3 = eps + M_avg[T313_re][col];
        KT4 = eps + M_avg[T323_im][col];
        KT5 = eps + 0.5 * (M_avg[T311][col] + M_avg[T322][col] - M_avg[T333][col]);
        KT6 = eps + M_avg[T323_re][col];
        KT7 = eps + M_avg[T313_im][col];
        KT8 = eps + 0.5 * (M_avg[T311][col] - M_avg[T322][col] + M_avg[T333][col]);
        KT9 = eps - M_avg[T312_im][col];
        KT10 = eps + 0.5 * (-M_avg[T311][col] + M_avg[T322][col] + M_avg[T333][col]);
