...
Description :  Faraday Rotation Angle Estimation
...
/* LOCAL VARIABLES */
  ...
  int Config;
  char *PolTypeConf[NPolType] = {"S2", "C4", "T4"};
...
if(argc < 19) {
  ...
  if (strcmp(PolType,"S2")==0) strcpy(PolType,"S2T4");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sfaraday_rotation_freeman.bin", out_dir);
  if ((out_file1 = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open output file : ", file_name);

  sprintf(file_name, "%sfaraday_rotation_bickel_bates.bin", out_dir);
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        M_out1[lig][col] = 0.5 * atan2(sqrt(M_avg[T444][col]),sqrt(M_avg[T411][col]));
        M_out1[lig][col] = M_out1[lig][col] * 180. / pi;
        M_out2[lig][col] = -0.25 * atan2(-2*M_avg[T414_re][col],M_avg[T411][col]-M_avg[T444][col]);
        M_out2[lig][col] = M_out2[lig][col] * 180. / pi;
        } else {
        M_out1[lig][col] = 0.; M_out2[lig][col] = 0.;
        }
      }
    free_matrix_float(M_avg,NpolarOut);
    }

  write_block_matrix_float(out_file1, M_out1, NligBlock[Nb], Sub_Ncol, 0, 0, Sub_Ncol);
  write_block_matrix_float(out_file2, M_out2, NligBlock[Nb], Sub_Ncol, 0, 0, Sub_Ncol);
