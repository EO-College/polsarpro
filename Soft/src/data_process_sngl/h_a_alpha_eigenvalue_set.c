...
/* OUTPUT FILE OPENING*/
  ...
  if (FlagProbabilites == 1) {
    ...
    Flag[Proba3] = Nout; Nout++; Flag[Proba4] = Nout; Nout++;
    }
  //Flag Anisotropy
  if (FlagAnisotropy == 1) {
    Flag[A] = Nout; Nout++;
...
rau_re = rau_im = nu = zeta = abs_rau_2 = delta2 = 0.;
k1_re = k1_im = k2_re = k2_im = alpha1 = alpha2 = 0.;
#pragma omp parallel for private(col, k, M, V, lambda, M_avg) firstprivate(p, mask, min, max, D, I, DegPol, CC11, CC13_re, CC13_im, CC22, CC33, rau_re, rau_im, nu, zeta, abs_rau_2, delta2, k1_re, k1_im, k2_re, k2_im, alpha1, alpha2) shared(ligDone, minHS, maxHS, minHSI, maxHSI, minHSP, maxHSP)
        ...
        if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
          ...
          if (FlagErd == 1) {
            if (strcmp(PolTypeOut,"T3")==0) {
              CC11 = (M_avg[T311][col] + 2 * M_avg[T312_re][col] + M_avg[T322][col]) / 2;
              CC13_re = (M_avg[T311][col] - M_avg[T322][col]) / 2;
              CC13_im = -M_avg[T312_im][col];
              CC22 = M_avg[T333][col];
              CC33 = (M_avg[T311][col] - 2 * M_avg[T312_re][col] + M_avg[T322][col]) / 2;
              }
        ...
        if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
          ...
          if (FlagErd == 1) {
            if (strcmp(PolTypeOut,"T4")==0) {
              CC11 = (M_avg[T411][col] + 2 * M_avg[T412_re][col] + M_avg[T422][col]) / 2;
              CC13_re = (M_avg[T411][col] - M_avg[T422][col]) / 2;
              CC13_im = -M_avg[T412_im][col];
              CC22 = M_avg[T433][col];
              CC33 = (M_avg[T411][col] - 2 * M_avg[T412_re][col] + M_avg[T422][col]) / 2;
              }
            if (strcmp(PolTypeOut,"C4")==0) {
              CC11 = M_avg[C411][col];
              CC13_re = M_avg[C414_re][col];
              CC13_im = M_avg[C414_im][col];
              CC22 = (M_avg[C422][col] + M_avg[C433][col] + 2 * M_avg[C423_re][col]) / 2.;
              CC33 = M_avg[C444][col];
              }
...
********************************************************************/
  ...
  ValidMask = vector_float(Sub_Ncol);
  if (FlagValid == 0) 
  ...
  if ((OutFile3[1] = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);
  Npts = -1;
  rewind(in_valid); rewind(OutFile3[0]);
  for (lig = 0; lig < Sub_Nlig; lig++) {
    PrintfLine(lig,Sub_Nlig);
    fread(&bufferdatafloat[0], sizeof(float), Sub_Ncol, OutFile3[0]);
    if (FlagValid == 1) fread(&ValidMask[0], sizeof(float), Sub_Ncol, in_valid);
    for (col = 0; col < Sub_Ncol; col++) {
      if (ValidMask[col] == 1.) {
        Npts++;
        datatmp[Npts] = bufferdatafloat[col];
        } /* valid */
      } /* col */
    } /* lig */
  Npts++;
  MinMaxContrastMedian(datatmp, &minHS, &maxHS, Npts);
  rewind(in_valid); rewind(OutFile3[0]);
  for (lig = 0; lig < Sub_Nlig; lig++) {
    PrintfLine(lig,Sub_Nlig);
    fread(&bufferdatafloat[0], sizeof(float), Sub_Ncol, OutFile3[0]);
    if (FlagValid == 1) fread(&ValidMask[0], sizeof(float), Sub_Ncol, in_valid);
    for (col = 0; col < Sub_Ncol; col++) {
      bufferdatafloat[col] = ValidMask[col]*(bufferdatafloat[col] - minHS) / (maxHS - minHS);
      if (bufferdatafloat[col] < 0.) bufferdatafloat[col] = 0.;
      if (bufferdatafloat[col] > 1.) bufferdatafloat[col] = 1.;
      } /* col */
    fwrite(&bufferdatafloat[0], sizeof(float), Sub_Ncol, OutFile3[1]);
    } /* lig */
  fclose(OutFile3[0]);
  fclose(OutFile3[1]);
...
#pragma omp parallel for private(col) shared(ligDone)
    for (lig = 0; lig < NligBlock[Nb]; lig++) {
      ligDone++;
      if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
      for (col = 0; col < Sub_Ncol; col++) {
