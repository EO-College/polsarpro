...
Description :  Freeman reconstruction
...
#pragma omp parallel for private(col, Np, M_avg) firstprivate(CC11, CC13_re, CC13_im, CC22, CC33, FV, FD, FS, ALP, BET, rtemp) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    M_avg = matrix_float(NpolarOut,Sub_Ncol);
