...
File  : singh_4components_decomposition.c
...
Description :  Singh 4 components Decomposition
...
int unitary_transformation(float *TT, float phi);
...
strcpy(UsageHelp,"\nsingh_4components_decomposition.exe\n");
strcat(UsageHelp,"\nParameters:\n");
strcat(UsageHelp," (string)	-id  	input directory\n");
strcat(UsageHelp," (string)	-od  	output directory\n");
strcat(UsageHelp," (string)	-iodf	input-output data format\n");
strcat(UsageHelp," (string)	-mod 	decomposition mode (G4U1, G4U2)\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sSingh4_%s_Odd.bin", out_dir, YamMode);
  if ((out_odd = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh4_%s_Dbl.bin", out_dir, YamMode);
  if ((out_dbl = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%sSingh4_%s_Vol.bin", out_dir, YamMode);
  if ((out_vol = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);
  
  sprintf(file_name, "%sSingh4_%s_Hlx.bin", out_dir, YamMode);
...
rtemp = ratio = S = D = Cre = Cim = CO = C1 = teta = phi = 0.;
Ps = Pd = Pv = Pc = TP = 0.;
#pragma omp parallel for private(col, Np, TT, M_avg) firstprivate(HV_type, FS, FD, FV, ALPre, ALPim, BETre, BETim, HHHH, HVHV, VVVV, HHVVre, HHVVim, rtemp, ratio, S, D, Cre, Cim, CO, C1, teta, phi, Ps, Pd, Pv, Pc, TP) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ligDone++;
    if (omp_get_thread_num() == 0) PrintfLine(ligDone,NligBlock[Nb]);
    TT = vector_float(NpolarOut);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        phi = 0.5 * atan(2*TT[T323_im]/(TT[T322]-TT[T333]));
        unitary_transformation(TT,phi);
          
        Pc = 2. * fabs(TT[T323_im]);

        HV_type = 1; /* Surface scattering */
        
        if (strcmp(YamMode,"G4U2")==0) {
...
int unitary_transformation(float *TT, float phi)
{
  ...
  TT[T311] = T11;
  TT[T312_re] = T12_re*cos(phi)+T13_im*sin(phi);
  TT[T312_im] = T12_im*cos(phi)-T13_re*sin(phi);
  TT[T313_re] = T12_im*sin(phi)+T13_re*cos(phi);
  TT[T313_im] = -T12_re*sin(phi)+T13_im*cos(phi);
  TT[T322] = T22*cos(phi)*cos(phi)+2.*T23_im*cos(phi)*sin(phi)+T33*sin(phi)*sin(phi);
  TT[T323_re] = T23_re;
  TT[T323_im] = -T22*cos(phi)*sin(phi)+T23_im*cos(phi)*cos(phi)-T23_im*sin(phi)*sin(phi)+T33*cos(phi)*sin(phi);
  TT[T333] = T22*sin(phi)*sin(phi)-2.*T23_im*cos(phi)*sin(phi)+T33*cos(phi)*cos(phi);
