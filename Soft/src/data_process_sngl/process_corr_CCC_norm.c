...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sCCCnorm.bin", out_dir);
...
a = b = c = d = 0.;
#pragma omp parallel for private(col, M_avg) firstprivate(a, b, c, d) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        a = M_avg[T323_re][col]*M_avg[T323_re][col];
        b = M_avg[T323_im][col]*M_avg[T323_im][col];
        c = (M_avg[T322][col]/2. - M_avg[T333][col]/2.)*(M_avg[T322][col]/2. - M_avg[T333][col]/2.);
        d = (M_avg[T322][col]/2. + M_avg[T333][col]/2.)*(M_avg[T322][col]/2. + M_avg[T333][col]/2.);

        M_out[lig][col] = sqrt(1. + (a/c)) / sqrt(1. - (b/d) + eps);
        } else {
        M_out[lig][col] = 0.;
        }
      }
