...
/* INPUT FILE OPENING*/
  sprintf(file_name, "%ssupervised_class_%dx%d.bin", out_dir, NwinL, NwinC);
  if (Rej_flag == 1)
    sprintf(file_name, "%ssupervised_class_rej_%dx%d.bin", out_dir, NwinL, NwinC);
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sconfusion_matrix_%dx%d.txt", out_dir, NwinL, NwinC);
  if (Rej_flag == 1)
    sprintf(file_name, "%sconfusion_matrix_rej_%dx%d.txt", out_dir, NwinL, NwinC);
...
/* Create BMP file*/
if (Bmp_flag == 1) {
  sprintf(file_name, "%s%s_%dx%d", out_dir, "classified_cluster_set", NwinL, NwinC);
  if (Rej_flag == 1)
    sprintf(file_name, "%s%s_rej_%dx%d", out_dir, "classified_cluster_set", NwinL, NwinC);
