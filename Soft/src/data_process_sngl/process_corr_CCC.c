...
if(argc < 19) {
  ...
  if (strcmp(PolType,"C4")==0) strcpy(PolType, "C4T3");
  if (strcmp(PolType,"T4")==0) strcpy(PolType, "T4T3");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sCCC.bin", out_dir);
...
GG = DD = GDre = GDim = 0.;
#pragma omp parallel for private(col, M_avg) firstprivate(GG, DD, GDre, GDim) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        GG = 0.5*(M_avg[T333][col] + M_avg[T322][col] + 2.*M_avg[T323_im][col]);
        DD = 0.5*(M_avg[T333][col] + M_avg[T322][col] - 2.*M_avg[T323_im][col]);
        GDre = 0.5*(M_avg[T333][col] - M_avg[T322][col]);
        GDim = -M_avg[T323_re][col];

        M_out[lig][2*col] = GDre / sqrt(GG * DD + eps);
        M_out[lig][2*col + 1] = GDim / sqrt(GG * DD + eps);
        } else {
