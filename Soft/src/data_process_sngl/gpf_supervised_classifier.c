...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (strcmp(PolTypeOut,"T2")==0) T2_to_C2(M_in, NligBlock[Nb], Sub_Ncol + NwinC, 0, 0);
  if (strcmp(PolTypeOut,"C3")==0) C3_to_T3(M_in, NligBlock[Nb], Sub_Ncol + NwinC, 0, 0); 
  if (strcmp(PolTypeOut,"C4")==0) C4_to_T4(M_in, NligBlock[Nb], Sub_Ncol + NwinC, 0, 0);

dist_min = Ptgt = Ptgtclass = PtgtclassRe = PtgtclassIm = 0.;
#pragma omp parallel for private(col, k, area, Ttgt, M_avg) firstprivate(ligg, dist_min, Ptgt, Ptgtclass, PtgtclassRe, PtgtclassIm) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        if (Npp == 2) {
          Ttgt[0] = M_avg[X211][col]; Ttgt[1] = M_avg[X222][col];
          Ttgt[2] = M_avg[X212_re][col]; Ttgt[3] = M_avg[X212_im][col];
          Ptgt = 0.;
        ...
        if (Npp == 3) {
          Ttgt[0] = M_avg[X311][col]; Ttgt[1] = M_avg[X322][col]; Ttgt[2] = M_avg[X333][col];
          Ttgt[3] = M_avg[X312_re][col]; Ttgt[4] = M_avg[X312_im][col];
          Ttgt[5] = M_avg[X313_re][col]; Ttgt[6] = M_avg[X313_im][col];
          Ttgt[7] = M_avg[X323_re][col]; Ttgt[8] = M_avg[X323_im][col];
          Ptgt = 0.;
          for (k=0; k<Npp*Npp; k++) Ptgt += Ttgt[k]*Ttgt[k];
          for (area = 1; area < Narea; area++) {
            Ptgtclass = 0.; PtgtclassRe = 0.; PtgtclassIm = 0.;
        ...
        if (Npp == 4) {
          Ttgt[0] = M_avg[X411][col]; Ttgt[1] = M_avg[X422][col];
          Ttgt[2] = M_avg[X433][col]; Ttgt[3] = M_avg[X444][col];
          Ttgt[4] = M_avg[X412_re][col]; Ttgt[5] = M_avg[X412_im][col];
          Ttgt[6] = M_avg[X413_re][col]; Ttgt[7] = M_avg[X413_im][col];
          Ttgt[8] = M_avg[X414_re][col]; Ttgt[9] = M_avg[X414_im][col];
          Ttgt[10] = M_avg[X423_re][col]; Ttgt[11] = M_avg[X423_im][col];
          Ttgt[12] = M_avg[X424_re][col]; Ttgt[13] = M_avg[X424_im][col];
          Ttgt[14] = M_avg[X434_re][col]; Ttgt[15] = M_avg[X434_im][col];
          Ptgt = 0.;
          for (k=0; k<Npp*Npp; k++) Ptgt += Ttgt[k]*Ttgt[k];
          for (area = 1; area < Narea; area++) {
            Ptgtclass = 0.; PtgtclassRe = 0.; PtgtclassIm = 0.;
    ...
    free_vector_float(distance);
    free_vector_float(Ttgt);
