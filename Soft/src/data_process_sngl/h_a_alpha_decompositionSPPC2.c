...
D = I = DegPol = 0.;
#pragma omp parallel for private(col, k, M, V, lambda, M_avg) firstprivate(alpha, delta, phase, p, D, I, DegPol) shared(ligDone, minHS, maxHS, minHSI, maxHSI, minHSP, maxHSP)
...
********************************************************************/
  ...
  if ((OutFile2[1] = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);
  Npts = -1;
  rewind(in_valid); rewind(OutFile2[0]);
  for (lig = 0; lig < Sub_Nlig; lig++) {
    PrintfLine(lig,Sub_Nlig);
    fread(&bufferdatafloat[0], sizeof(float), Sub_Ncol, OutFile2[0]);
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    ...
    fwrite(&bufferdatafloat[0], sizeof(float), Sub_Ncol, OutFile2[1]);
    } /* lig */
  fclose(OutFile2[0]);
  fclose(OutFile2[1]);
