...
strcpy(UsageHelp,"\nfreeman_2components_decomposition.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%s%s", out_dir, "Freeman2_Ground.bin");
  if ((out_grd = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%s%s", out_dir, "Freeman2_Vol.bin");
...
/*-----------------------------------------------------------------*/   
  ...
  M_grd = matrix_float(NligBlock[0], Sub_Ncol);
...
#pragma omp parallel for private(col, M_avg) firstprivate(CC11, CC13_re, CC13_im, CC22, CC33, HHHH,HVHV,VVVV, HHVVre,HHVVim, FV, FG, RHO, x,y,z1,z2r,z2i,z3r,z3i) shared(ligDone)
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[NwinLM1S2+lig][NwinCM1S2+col] == 1.) {
        ...
        y = -(z3i*(1. + 2.*z3r)) / (z3r*z3r + z3i*z3i + eps);
        x = 1. + (y*z3r / (z3i + eps));
        ...
        M_vol[lig][col] = FV * (3. - RHO);
        M_grd[lig][col] = FG * (1. + x*x + y*y);

        if (M_grd[lig][col] < SpanMin) M_grd[lig][col] = SpanMin;
        if (M_grd[lig][col] > SpanMax) M_grd[lig][col] = SpanMax;
  ...
  write_block_matrix_float(out_grd, M_grd, NligBlock[Nb], Sub_Ncol, 0, 0, Sub_Ncol);
...
/*
  ...
  free_matrix_float(M_grd, NligBlock[0]);
