...
strcpy(UsageHelp,"\ngpf_confusion_matrix.exe\n");
strcat(UsageHelp,"\nParameters:\n");
strcat(UsageHelp," (string)	-id  	input directory\n");
strcat(UsageHelp," (string)	-od  	output directory\n");
strcat(UsageHelp," (string)	-af  	input area file\n");
strcat(UsageHelp," (string)	-thr 	threshold\n");
strcat(UsageHelp," (string)	-redr	reduction ratio\n");
...
if(argc < 25) {
  ...
  get_commandline_prm(argc,argv,"-thr",str_cmd_prm,threshold,1,UsageHelp);
  get_commandline_prm(argc,argv,"-redr",str_cmd_prm,RedR,1,UsageHelp);
...
/* INPUT FILE OPENING*/
  sprintf(file_name, "%sgpf_supervised_class_%dx%d_%s_%s.bin", out_dir, NwinL, NwinC, threshold, RedR);
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%sgpf_confusion_matrix_%dx%d_%s_%s.txt", out_dir, NwinL, NwinC, threshold, RedR);
...
/* Create BMP file*/
if (Bmp_flag == 1) {
  sprintf(file_name, "%s%s_%dx%d_%s_%s", out_dir, "gpf_classified_cluster_set", NwinL, NwinC, threshold, RedR);
