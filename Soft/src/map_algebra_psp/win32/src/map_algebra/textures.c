	...
	{ 
        ...
        while( !feof(fileinput) )
        {
            ...
            if (strstr(Buf,"max val") != NULL)
            {
                ...
                imageTex->ValMax = atof(Tmp);
            }
            if (strstr(Buf,"min val") != NULL)
            {
                p1 = strstr(Buf," = ");
                strcpy(Tmp, ""); strncat(Tmp, &p1[3], strlen(p1) - 3); 
                imageTex->ValMin = atof(Tmp);
            }
        }
        fclose(fileinput);
        
        imageTex->NColor = 0;
        NColor = FreeImage_GetColorsUsed(src);
        if (NColor == 256)
        {
            RGBQUAD *pal_col = FreeImage_GetPalette(src);
            imageTex->NColor = 256;       
            for (i = 0; i<256; i++)
            {
                imageTex->PalCol[i] = 1000000*pal_col[i].rgbRed + 1000*pal_col[i].rgbGreen + pal_col[i].rgbBlue;
            }
        }
		...
		{  
			for (mipidx = 0; mipidx < texCount; mipidx++)
			{
				for (y = 0; y < mipSize[mipidx].y; y++)
				{
					...
					for (unsigned int pi = 0; pi < size; pi += 3)
					{
