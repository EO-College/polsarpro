...
char *vector_char(int nrh)
{
  ...
  if (!v) edit_error("allocation failure in vector_char()", "");
  return v;
}
...
short int *vector_short_int(int nrh)
{
  ...
  if (!m) edit_error("allocation failure 1 in vector_short_int()", "");

  for (ii = 0; ii < nrh; ii++) m[ii] = 0;
...
int *vector_int(int nrh)
{
  ...
  if (!m) edit_error("allocation failure 1 in vector_int()", "");
...
float *vector_float(int nrh)
{
  ...
  if (!m) edit_error("allocation failure 1 in vector_float()", "");
...
char **matrix_char(int nrh, int nch)
{
  ...
  if (!m) edit_error("allocation failure 1 in matrix_char()", "");

  for (i = 0; i < nrh; i++) {
    m[i] = (char *) malloc((unsigned) (nch + 1) * sizeof(char));
    if (!m[i]) edit_error("allocation failure 2 in matrix_char()", "");
    }
  return m;
}
...
short int **matrix_short_int(int nrh, int nch)
{
  ...
  if (!m) edit_error("allocation failure 1 in matrix()", "");

  for (i = 0; i < nrh; i++) {
    m[i] = (short int *) malloc((unsigned) (nch) * sizeof(short int));
    if (!m[i]) edit_error("allocation failure 2 in matrix()", "");
    }
  for (i = 0; i < nrh; i++) {
    for (j = 0; j < nch; j++) {
      m[i][j] = 0;
...
short int ***matrix3d_short_int(int nz, int nrh, int nch)
{
  ...
  if (m == NULL) edit_error("D'ALLOCATION No.1 DANS MATRIX()", "");
  for (jj = 0; jj < nz; jj++) {
    m[jj] = (short int **) malloc((unsigned) (nrh + 1) * sizeof(short int *));
    if (m[jj] == NULL) edit_error("D'ALLOCATION No.2 DANS MATRIX()", "");
    for (ii = 0; ii < nrh; ii++) {
      m[jj][ii] = (short int *) malloc((unsigned) (nch + 1) * sizeof(short int));
      if (m[jj][ii] == NULL) edit_error("D'ALLOCATION No.3 DANS MATRIX()", "");
      }
    }
  for (dd = 0; dd < nz; dd++) {
    for (jj = 0; jj < nrh; jj++) {
...
float ***matrix3d_float(int nz, int nrh, int nch)
{
  ...
  for (jj = 0; jj < nz; jj++) {
    ...
    if (m[jj] == NULL) edit_error("D'ALLOCATION No.2 DANS MATRIX()", "");
    for (ii = 0; ii < nrh; ii++) {
      m[jj][ii] = (float *) malloc((unsigned) (nch + 1) * sizeof(float));
      if (m[jj][ii] == NULL) edit_error("D'ALLOCATION No.3 DANS MATRIX()", "");
      }
    }
  for (dd = 0; dd < nz; dd++) {
...
cplx *cplx_vector(int nh)
{
  ...
  if (!v) edit_error("allocation failure 1 in cplx_vector()","");
...
void cplx_free_matrix( cplx **m,int nrh)
{
  ...
  for(i = nrh-1; i >= 0; i--) free((cplx *) (m[i]));
  free((cplx **) (m));
  m = NULL;
}
