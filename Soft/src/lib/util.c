...
void write_envi_config(char *file, int Nlig, int Ncol, int Type);
...
int my_fseek_position(FILE *in_file, long int fseek_position);
...
********************************************************************/
void write_envi_config(char *file_out, int Nlig, int Ncol, int Type)
{
  char file_name[FilePathLength];
  FILE *file;

  sprintf(file_name, "%s.hdr", file_out);
  if ((file = fopen(file_name, "w")) == NULL)
  edit_error("Could not open configuration file : ", file_name);

  fprintf(file, "ENVI\n");
  fprintf(file, "description = {\n");
  fprintf(file, "PolSARpro File Imported to ENVI}\n");
  fprintf(file, "samples = %i\n", Ncol);
  fprintf(file, "lines   = %i\n", Nlig);
  fprintf(file, "bands   = 1\n");
  fprintf(file, "header offset = 0\n");
  fprintf(file, "file type = ENVI Standard\n");
  fprintf(file, "data type = %i\n", Type);
  fprintf(file, "interleave = bsq\n");
  fprintf(file, "sensor type = Unknown\n");
  fprintf(file, "byte order = 0\n");
  fprintf(file, "band names = {\n");
  fprintf(file, "%s }\n", file_out);
  fclose(file);
}
...
{
#ifdef _WIN32
int FreeMemory = CheckFreeMemoryWin32();
#else
int FreeMemory = CheckFreeMemoryLinux();
#endif
return(FreeMemory);
}
/********************************************************************
...
int CheckFreeMemoryWin32(void)
{
  ...
  sprintf(CmdLine, "wmic OS get FreePhysicalMemory");    
  
  in = popen(CmdLine, "r");
  if ( !in ) {
  //if(!(in = popen(CmdLine, "r"))){
    exit(1);
    }
  fgets(CmdLine, sizeof(CmdLine), in);
  fgets(CmdLine, sizeof(CmdLine), in);
  MemoryValue = floor(0.75*atof(CmdLine)/1024.);
  return(MemoryValue); 
#else
  return(0);
#endif
...
int CheckFreeMemoryLinux(void)
{
  ...
  while(fgets(line,sizeof(line),meminfo)) {
    if (sscanf(line, "MemFree: %d kB", &MemoryFree) == 1) flag = flag + 1;
    if (sscanf(line, "Buffers: %d kB", &MemoryBuffer) == 1) flag = flag + 10;
    if (sscanf(line, "Cached: %d kB", &MemoryCache) == 1) flag = flag + 100;
    if (flag == 111) {
      fclose(meminfo);
      MemoryValue = floor(0.75*(MemoryFree + MemoryBuffer + MemoryCache)/1024.);
      return(MemoryValue);
      }
    }
  fclose(meminfo);
...
//printf("offset avant: %lu\n", position);
#ifdef _WIN32
  position = position + fseek_sign*fseek_arg1*fseek_arg2;
#else
  position.__pos = position.__pos + fseek_sign*fseek_arg1*fseek_arg2;
#endif
fsetpos(in_file, &position);
//fgetpos(in_file, &position); printf("offset apres: %lu\n", position);
...
fgetpos(in_file, &position);
#ifdef _WIN32
  position = position + (fseek_position - position);
#else
  position.__pos = position.__pos + (fseek_position - position.__pos);
#endif
fsetpos(in_file, &position);
