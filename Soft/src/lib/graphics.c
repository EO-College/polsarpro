...
void write_bmp_hdr(int nlig, int ncol, float Max, float Min, int nbytes, char *filebmp);
...
/* Definition of the Header */
  ...
  write_bmp_hdr(nlig, ncol, Max, Min, 8, name);
...
/* Definition of the Header */
  ...
  write_bmp_hdr(nlig, ncol, 0., 0., 24, name);
...
/* Definition of the Colormap */
  ...
  if (strcmp(MaskCol, "white") == 0) {
    blue[0] = 254; green[0] = 255; red[0] = 254;
    }
  if (strcmp(MaskCol, "gray") == 0) {
    blue[0] = 127; green[0] = 128; red[0] = 127;
    }
  if (strcmp(MaskCol, "black") == 0) {
    blue[0] = 0; green[0] = 1; red[0] = 0;
    }
...
{
  FILE *fbmp;
  char filehdr[FilePathLength];
  
 /* Bitmap HDR file opening */
  sprintf(filehdr, "%s.hdr", filebmp);
  check_file(filehdr);

  if ((fbmp = fopen(filehdr, "w")) == NULL)
    edit_error("Could not open the bitmap file ", filehdr);

  fprintf(fbmp, "ENVI\n");
  fprintf(fbmp, "description = {\n");
  fprintf(fbmp, "PolSARpro File Imported to ENVI }\n");
  fprintf(fbmp, "samples = %i\n",ncol);
  fprintf(fbmp, "lines   = %i\n",nlig);
  fprintf(fbmp, "max val = %f\n",Max);
  fprintf(fbmp, "min val = %f\n",Min);
  fprintf(fbmp, "color   = %i bytes\n",nbytes);
  fclose(fbmp);  
}
