...
/*-----------------------------------------------------------------*/   
  ...
  M_out = matrix3d_float(NpolarOut, 1, Sub_Ncol);
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        ...
        for (Np=0; Np<NpolarOut; Np++) M_out[Np][0][col] = mh[Np];

        } else {
        for (Np = 0; Np < NpolarOut; Np++) M_out[Np][0][col] = 0.;
        }
      }
    write_block_matrix3d_float(out_datafile, NpolarOut, M_out, 1, Sub_Ncol, 0, 0, Sub_Ncol);
    } // lig
