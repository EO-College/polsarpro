...
Description :  MatS (operand) MatX = MatS
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if (strcmp(operand,"addmatX") == 0 ) {
          for (Np = 0; Np < NpolarOut; Np++) {
            M_out[Np][lig][2*col] = M_in[Np][lig][2*col]+MatX[Np][0];
            M_out[Np][lig][2*col+1] = M_in[Np][lig][2*col+1]+MatX[Np][1];
            }
          }
