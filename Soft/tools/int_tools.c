...
********************************************************************/
  ...
  datatmp = matrix_int(Sub_Nlig, Sub_Ncol);

  if (Nlig >= Ncol) bufferdata = vector_int(Nlig);
  if (Ncol >= Nlig) bufferdata = vector_int(Ncol);
...
/* WRITE OUTPUT DATA FILE */
  ...
  if (strcmp(Operation, "rot90l") == 0) {
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      fwrite(&bufferdata[0], sizeof(int), Sub_Nlig, fileoutput);
      }
