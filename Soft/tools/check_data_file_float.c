...
Description :  Check NaN or Infinity in a Float Raw Binary Data File 
...
/*******************************************************************/
  ...
  for (lig = 0; lig < Off_lig; lig++) 
    fread(&bufferdata[0], sizeof(float), Sub_Ncol, fileinput);
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++)
      {
      if (my_isfinite(bufferdata[col+Off_col]) == 0) FlagCheck = 1;
      }
    }
