...
Description :  Cmplx data file processing
...
strcat(UsageHelp," (string)	-op  	operation (ieee, extract, rot90l, rot90r, rot180, fliplr, flipud, transp)\n");
...
********************************************************************/
  ...
  datatmp = matrix_float(Sub_Nlig, 2 * Sub_Ncol);

  if (Nlig >= Ncol) bufferdata = vector_float(2 * Nlig);
  if (Ncol >= Nlig) bufferdata = vector_float(2 * Ncol);
  ...
  for (lig = 0; lig < Sub_Nlig; lig++) {
    if (strcmp(Operation, "ieee") == 0) {
      for (col = 0; col < Ncol; col++) {
        ...
        pc[3] = getc(fileinput); pc[2] = getc(fileinput);
        pc[1] = getc(fileinput); pc[0] = getc(fileinput);
        bufferdata[2 * col] = fl1;
        bufferdata[2 * col + 1] = fl2;
        }
      }
    if (strcmp(Operation, "ieee") != 0)
      fread(&bufferdata[0], sizeof(float), 2 * Ncol, fileinput);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      datatmp[lig][2 * col] = bufferdata[2 * (col + Off_col)];
      datatmp[lig][2 * col + 1] = bufferdata[2 * (col + Off_col) + 1];
      }
    }
...
/* WRITE OUTPUT DATA FILE */
  ...
  if (strcmp(Operation, "ieee") == 0) {
    for (lig = 0; lig < Sub_Nlig; lig++) {
      ...
      for (col = 0; col < Sub_Ncol; col++) {
        bufferdata[2 * col] = datatmp[lig][2 * col];
        bufferdata[2 * col + 1] = datatmp[lig][2 * col + 1];
        }
  ...
  if (strcmp(Operation, "rot90l") == 0) {
    for (col = 0; col < Sub_Ncol; col++) {
      ...
      for (lig = 0; lig < Sub_Nlig; lig++) {
        bufferdata[2 * lig] = datatmp[lig][2 * (Sub_Ncol - 1 - col)];
        bufferdata[2 * lig + 1] = datatmp[lig][2 * (Sub_Ncol - 1 - col) + 1];
        }
      fwrite(&bufferdata[0], sizeof(float), 2 * Sub_Nlig, fileoutput);
      }
  ...
  if (strcmp(Operation, "rot90r") == 0) {
    for (col = 0; col < Sub_Ncol; col++) {
      PrintfLine(col,Sub_Ncol);
      for (lig = 0; lig < Sub_Nlig; lig++) {
        bufferdata[2 * lig] = datatmp[Sub_Nlig - 1 - lig][2 * col];
        bufferdata[2 * lig + 1] = datatmp[Sub_Nlig - 1 - lig][2 * col + 1];
        }
      fwrite(&bufferdata[0], sizeof(float), 2 * Sub_Nlig, fileoutput);
      }
    write_config(DirOutput, Sub_Ncol, Sub_Nlig, PolarCase, PolarType);
  ...
  if (strcmp(Operation, "rot180") == 0) {
    for (lig = 0; lig < Sub_Nlig; lig++) {
      PrintfLine(lig,Sub_Nlig);
      for (col = 0; col < Sub_Ncol; col++) {
        bufferdata[2 * col] = datatmp[Sub_Nlig - 1 - lig][2 * (Sub_Ncol - 1 - col)];
        bufferdata[2 * col + 1] = datatmp[Sub_Nlig - 1 - lig][2 * (Sub_Ncol - 1 - col) + 1];
        }
      fwrite(&bufferdata[0], sizeof(float), 2 * Sub_Ncol, fileoutput);
      }
    write_config(DirOutput, Sub_Nlig, Sub_Ncol, PolarCase, PolarType);
  ...
  if (strcmp(Operation, "fliplr") == 0) {
    for (lig = 0; lig < Sub_Nlig; lig++) {
      PrintfLine(lig,Sub_Nlig);
      for (col = 0; col < Sub_Ncol; col++) {
        bufferdata[2 * col] = datatmp[lig][2 * (Sub_Ncol - 1 - col)];
        bufferdata[2 * col + 1] = datatmp[lig][2 * (Sub_Ncol - 1 - col) + 1];
        }
      fwrite(&bufferdata[0], sizeof(float), 2 * Sub_Ncol, fileoutput);
      }
    write_config(DirOutput, Sub_Nlig, Sub_Ncol, PolarCase, PolarType);
  ...
  if (strcmp(Operation, "flipud") == 0) {
    for (lig = 0; lig < Sub_Nlig; lig++) {
      PrintfLine(lig,Sub_Nlig);
      for (col = 0; col < Sub_Ncol; col++) {
        bufferdata[2 * col] = datatmp[Sub_Nlig - 1 - lig][2 * col];
        bufferdata[2 * col + 1] = datatmp[Sub_Nlig - 1 - lig][2 * col + 1];
        }
      fwrite(&bufferdata[0], sizeof(float), 2 * Sub_Ncol, fileoutput);
      }
    write_config(DirOutput, Sub_Nlig, Sub_Ncol, PolarCase, PolarType);
  ...
  if (strcmp(Operation, "transp") == 0) {
    for (col = 0; col < Sub_Ncol; col++) {
      PrintfLine(col,Sub_Ncol);
      for (lig = 0; lig < Sub_Nlig; lig++) {
        bufferdata[2 * lig] = datatmp[lig][2 * col];
        bufferdata[2 * lig + 1] = datatmp[lig][2 * col + 1];
        }
      fwrite(&bufferdata[0], sizeof(float), 2 * Sub_Nlig, fileoutput);
      }
    write_config(DirOutput, Sub_Ncol, Sub_Nlig, PolarCase, PolarType);
