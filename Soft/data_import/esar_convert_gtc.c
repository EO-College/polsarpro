...
Description :  Convert ESAR Binary Data Files 
               (Format GTC)
...
strcat(UsageHelp," (int)   	-hdr 	data header to be skipped (no: 0, yes: 1)\n");
strcat(UsageHelp,"\nOptional Parameters:\n");
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) {
      ...
      if (IEEE == 1) {
        for (col = 0; col < Ncol; col++) {
          ...
          w = &it2;pc = (char *) w;
          pc[1] = getc(in_file[Np]);pc[0] = getc(in_file[Np]);
          Min[Np][2 * col] = it1;Min[Np][2 * col + 1] = it2;
          }
        }
      }
    for (col = 0; col < Ncol; col++) {
      if ((Min[s11][2*col] < -9995)&&(Min[s11][2*col] > -10005)&&
        (Min[s12][2*col] < -9995)&&(Min[s12][2*col] > -10005)&&
        (Min[s21][2*col] < -9995)&&(Min[s21][2*col] > -10005)&&
        (Min[s22][2*col] < -9995)&&(Min[s22][2*col] > -10005)&&
        (Min[s11][2*col+1] < -9995)&&(Min[s11][2*col+1] > -10005)&&
        (Min[s12][2*col+1] < -9995)&&(Min[s12][2*col+1] > -10005)&&
        (Min[s21][2*col+1] < -9995)&&(Min[s21][2*col+1] > -10005)&&
        (Min[s22][2*col+1] < -9995)&&(Min[s22][2*col+1] > -10005)) {
        for (Np = 0; Np < Npol; Np++) {
          Min[Np][2*col] = 0;
          Min[Np][2*col + 1] = 0;
          }
    ...
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
        S_in[Np][lig][2*col] = (float)Min[Np][2*col] / 1000.;
        S_in[Np][lig][2*col+1] = (float)Min[Np][2*col+1] / 1000.;
        }
      if ((S_in[s11][lig][2*col] == 0.0)&&(S_in[s11][lig][2*col+1] == 0.0)&&
        (S_in[s12][lig][2*col] == 0.0)&&(S_in[s12][lig][2*col+1] == 0.0)&&
        (S_in[s21][lig][2*col] == 0.0)&&(S_in[s21][lig][2*col+1] == 0.0)&&
        (S_in[s22][lig][2*col] == 0.0)&&(S_in[s22][lig][2*col+1] == 0.0)) {
        for (Np = 0; Np < Npol; Np++) {
          S_in[Np][lig][2*col] = my_eps_random();
          S_in[Np][lig][2*col + 1] = my_eps_random();
          }
        }
      for (Np = 0; Np < Npol; Np++) {
