...
File   : rawbinary_convert_Cmplx_C4.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_Cmplx_C4 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligoffset; lig++) {
  fread(&M_tmp[0][0], sizeof(float), Ncol, in_file[C11]);
  fread(&M_tmp[0][0], sizeof(float), 2*Ncol, in_file[C12]);
  fread(&M_tmp[0][0], sizeof(float), 2*Ncol, in_file[C13]);
  fread(&M_tmp[0][0], sizeof(float), 2*Ncol, in_file[C14]);
  fread(&M_tmp[0][0], sizeof(float), Ncol, in_file[C22]);
  fread(&M_tmp[0][0], sizeof(float), 2*Ncol, in_file[C23]);
  fread(&M_tmp[0][0], sizeof(float), 2*Ncol, in_file[C24]);
  fread(&M_tmp[0][0], sizeof(float), Ncol, in_file[C33]);
  fread(&M_tmp[0][0], sizeof(float), 2*Ncol, in_file[C34]);
  fread(&M_tmp[0][0], sizeof(float), Ncol, in_file[C44]);
  }

for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
     if (IEEE == 0) {
      fread(&M_tmp[C11][0], sizeof(float), Ncol, in_file[C11]);
      fread(&M_tmp[C12][0], sizeof(float), 2 * Ncol, in_file[C12]);
      fread(&M_tmp[C13][0], sizeof(float), 2 * Ncol, in_file[C13]);
      fread(&M_tmp[C13][0], sizeof(float), 2 * Ncol, in_file[C14]);
      fread(&M_tmp[C22][0], sizeof(float), Ncol, in_file[C22]);
      fread(&M_tmp[C23][0], sizeof(float), 2 * Ncol, in_file[C23]);
      fread(&M_tmp[C23][0], sizeof(float), 2 * Ncol, in_file[C24]);
      fread(&M_tmp[C33][0], sizeof(float), Ncol, in_file[C33]);
      fread(&M_tmp[C23][0], sizeof(float), 2 * Ncol, in_file[C34]);
      fread(&M_tmp[C33][0], sizeof(float), Ncol, in_file[C44]);
      }
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        M_tmp[C11][2 * col] = fl1;M_tmp[C11][2 * col +1] = 0.;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C12]);pc[2] = getc(in_file[C12]);
        pc[1] = getc(in_file[C12]);pc[0] = getc(in_file[C12]);
        M_tmp[C12][2 * col] = fl1;M_tmp[C12][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C13]);pc[2] = getc(in_file[C13]);
        pc[1] = getc(in_file[C13]);pc[0] = getc(in_file[C13]);
        M_tmp[C13][2 * col] = fl1;M_tmp[C13][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C23]);pc[2] = getc(in_file[C23]);
        pc[1] = getc(in_file[C23]);pc[0] = getc(in_file[C23]);
        M_tmp[C23][2 * col] = fl1;M_tmp[C23][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C34]);pc[2] = getc(in_file[C34]);
        pc[1] = getc(in_file[C34]);pc[0] = getc(in_file[C34]);
        M_tmp[C34][2 * col] = fl1;M_tmp[C34][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C44]);pc[2] = getc(in_file[C44]);
        pc[1] = getc(in_file[C44]);pc[0] = getc(in_file[C44]);
        M_tmp[C44][2 * col] = fl1;M_tmp[C44][2 * col +1] = 0.;
        }
    }
   for (col = 0; col < Ncol; col++) {
     M_in[C411][col] = M_tmp[C11][2 * col];
     M_in[C412_re][col] = M_tmp[C12][2 * col];
     M_in[C412_im][col] = M_tmp[C12][2 * col + 1];
     M_in[C413_re][col] = M_tmp[C13][2 * col];
     M_in[C413_im][col] = M_tmp[C13][2 * col + 1];
     M_in[C414_re][col] = M_tmp[C14][2 * col];
     M_in[C414_im][col] = M_tmp[C14][2 * col + 1];
     M_in[C422][col] = M_tmp[C22][2 * col];
     M_in[C423_re][col] = M_tmp[C23][2 * col];
     M_in[C423_im][col] = M_tmp[C23][2 * col + 1];
     M_in[C424_re][col] = M_tmp[C24][2 * col];
     M_in[C424_im][col] = M_tmp[C24][2 * col + 1];
     M_in[C433][col] = M_tmp[C33][2 * col];
     M_in[C434_re][col] = M_tmp[C34][2 * col];
     M_in[C434_im][col] = M_tmp[C34][2 * col + 1];
     M_in[C444][col] = M_tmp[C44][2 * col];
