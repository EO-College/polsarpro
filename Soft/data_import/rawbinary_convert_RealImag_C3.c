...
File   : rawbinary_convert_RealImag_C3.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_RealImag_C3 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[C311][col] = M_tmp[C311][col];
     M_in[C312_re][col] = M_tmp[C312_re][col];
     M_in[C312_im][col] = M_tmp[C312_im][col];
     M_in[C313_re][col] = M_tmp[C313_re][col];
     M_in[C313_im][col] = M_tmp[C313_im][col];
     M_in[C322][col] = M_tmp[C322][col];
     M_in[C323_re][col] = M_tmp[C323_re][col];
     M_in[C323_im][col] = M_tmp[C323_im][col];
     M_in[C333][col] = M_tmp[C333][col];
