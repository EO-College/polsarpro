...
Description :  Convert JPL - STK data file WITH/WITHOUT Header
...
/* LOCAL VARIABLES */
  ...
  char *buf;
  char Tmp[FilePathLength], HeaderFile[FilePathLength];
...
/* OFFSET HEADER DATA READING */
  for (Np = 0; Np < Npol; Np++) {
    rewind(in_file[Np]);
    for (kl = 0; kl < reclength; kl++) fgets(buf, 2, in_file[Np]);
    }
