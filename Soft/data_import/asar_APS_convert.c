...
Description :  Convert ASAR Binary Data Files (Format AP-SLC)
...
/******************************************************************************/
  ...
  printf("TYPE: asar_APS_convert FileInput DirOutput FileOutput1 FileOutput2 Ncol\n");
  printf("OffsetLig OffsetCol FinalNlig FinalNcol MPHOffset1 MPHOffset2\n");
  printf("SubSamplingRG SubSamplingAZ PolarType\n");
  exit(1);
  }

  dataread = vector_char(MDSOffset + 4*Ncol);
...
/******************************************************************************/
  rewind(fileinput);
  NN = floor(MPHOffset1 / (4*Ncol));
  for (k=0; k<NN; k++)
    {
    fread(&dataread[0], sizeof(char), 4*Ncol, fileinput);
    }
  fread(&dataread[0], sizeof(char), MPHOffset1-NN*4*Ncol, fileinput);

  for (lig = 0; lig < Nligoffset; lig++)
    fread(&dataread[0], sizeof(char), MDSOffset + 4*Ncol, fileinput);
  ...
  for (col = 0; col < Ncol; col++) {
    MS = dataread[MDSOffset + 4*col];
    if (MS < 0)  MS = MS + 256;
    LS = dataread[MDSOffset + 4*col + 1];
    if (LS < 0)  LS = LS + 256;
    xr = 256. * MS + LS;
    if (xr > 32767.) xr = xr - 65536.;
    MS = dataread[MDSOffset + 4*col + 2];
    if (MS < 0)  MS = MS + 256;
    LS = dataread[MDSOffset + 4*col + 3];
    if (LS < 0)  LS = LS + 256;
    xi = 256. * MS + LS;
    if (xi > 32767.) xi = xi - 65536.;
    tmpreal[col] = xr*xr+xi*xi;
    if (tmpreal[col] < eps) tmpreal[col] = my_eps_random();
  }

  for (col = 0; col < Ncolfin; col++) {
    datareal[col] = tmpreal[Ncoloffset + col * SubSampRG];
...
/******************************************************************************/
  rewind(fileinput);
  NN = floor(MPHOffset2 / (4*Ncol));
  for (k=0; k<NN; k++)
    {
    fread(&dataread[0], sizeof(char), 4*Ncol, fileinput);
    }
  fread(&dataread[0], sizeof(char), MPHOffset2-NN*4*Ncol, fileinput);
