...
/* LOCAL VARIABLES */
  ...
  char Tmp[FilePathLength], ConfigFile[FilePathLength], IncAngFile[FilePathLength];
...
/* HEADER FILE */
  ...
  fscanf(headerfile, "%f\n", &incangle0); incangle0 = incangle0 * pi / 180.; 
  fclose(headerfile);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    fread(&incangle[0], sizeof(float), Ncol, incang);
    for (Np = 0; Np < Npol; Np++) {
      fseek(in_file[Np], (iprefix+12), 1);
      for (col = 0; col < Ncol; col++) {
        v = &fl1;pc = (char *) v;
        pc[1] = getc(in_file[Np]);pc[0] = getc(in_file[Np]);
        v = &fl2;pc = (char *) v;
        pc[1] = getc(in_file[Np]);pc[0] = getc(in_file[Np]);
        S_in[Np][lig][2 * col] = fl1*sqrt(sin(incangle[col]*pi/180.)/sin(incangle0))/calfac;
        S_in[Np][lig][2 * col + 1] = fl2*sqrt(sin(incangle[col]*pi/180.)/sin(incangle0))/calfac;
        }
      }
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
