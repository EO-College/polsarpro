...
File   : rawbinary_convert_Cmplx_MLK_C4.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_Cmplx_MLK_C4 DirOutput Ncol OffsetLig OffsetCol\n");
...
/* Nb of lines and rows sub-sampled image */
  ...
  M_in = matrix3d_float(16, Nlook_lig, Ncol);
...
for (lig = 0; lig < Nligoffset; lig++) {
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[C11]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[C12]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[C13]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[C14]);
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[C22]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[C23]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[C24]);
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[C33]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[C34]);
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[C44]);
  }
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (ii = 0; ii < Nlook_lig; ii++) {
     if (IEEE == 0) {
      fread(&M_tmp[C11][ii][0], sizeof(float), Ncol, in_file[C11]);
      fread(&M_tmp[C12][ii][0], sizeof(float), 2 * Ncol, in_file[C12]);
      fread(&M_tmp[C13][ii][0], sizeof(float), 2 * Ncol, in_file[C13]);
      fread(&M_tmp[C13][ii][0], sizeof(float), 2 * Ncol, in_file[C14]);
      fread(&M_tmp[C22][ii][0], sizeof(float), Ncol, in_file[C22]);
      fread(&M_tmp[C23][ii][0], sizeof(float), 2 * Ncol, in_file[C23]);
      fread(&M_tmp[C23][ii][0], sizeof(float), 2 * Ncol, in_file[C24]);
      fread(&M_tmp[C33][ii][0], sizeof(float), Ncol, in_file[C33]);
      fread(&M_tmp[C23][ii][0], sizeof(float), 2 * Ncol, in_file[C34]);
      fread(&M_tmp[C33][ii][0], sizeof(float), Ncol, in_file[C44]);
      }
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        M_tmp[C11][ii][2 * col] = fl1;M_tmp[C11][ii][2 * col +1] = 0.;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C12]);pc[2] = getc(in_file[C12]);
        pc[1] = getc(in_file[C12]);pc[0] = getc(in_file[C12]);
        M_tmp[C12][ii][2 * col] = fl1;M_tmp[C12][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C13]);pc[2] = getc(in_file[C13]);
        pc[1] = getc(in_file[C13]);pc[0] = getc(in_file[C13]);
        M_tmp[C13][ii][2 * col] = fl1;M_tmp[C13][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C14]);pc[2] = getc(in_file[C14]);
        pc[1] = getc(in_file[C14]);pc[0] = getc(in_file[C14]);
        v = &fl2;pc = (char *) v;
        pc[3] = getc(in_file[C14]);pc[2] = getc(in_file[C14]);
        pc[1] = getc(in_file[C14]);pc[0] = getc(in_file[C14]);
        M_tmp[C14][ii][2 * col] = fl1;M_tmp[C14][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C22]);pc[2] = getc(in_file[C22]);
        pc[1] = getc(in_file[C22]);pc[0] = getc(in_file[C22]);
        M_tmp[C22][ii][2 * col] = fl1;M_tmp[C22][ii][2 * col +1] = 0.;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[C23]);pc[2] = getc(in_file[C23]);
        pc[1] = getc(in_file[C23]);pc[0] = getc(in_file[C23]);
        M_tmp[C23][ii][2 * col] = fl1;M_tmp[C23][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C24]);pc[2] = getc(in_file[C24]);
        pc[1] = getc(in_file[C24]);pc[0] = getc(in_file[C24]);
        v = &fl2;pc = (char *) v;
        pc[3] = getc(in_file[C24]);pc[2] = getc(in_file[C24]);
        pc[1] = getc(in_file[C24]);pc[0] = getc(in_file[C24]);
        M_tmp[C24][ii][2 * col] = fl1;M_tmp[C24][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C33]);pc[2] = getc(in_file[C33]);
        pc[1] = getc(in_file[C33]);pc[0] = getc(in_file[C33]);
        M_tmp[C33][ii][2 * col] = fl1;M_tmp[C33][ii][2 * col +1] = 0.;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C34]);pc[2] = getc(in_file[C34]);
        pc[1] = getc(in_file[C34]);pc[0] = getc(in_file[C34]);
        v = &fl2;pc = (char *) v;
        pc[3] = getc(in_file[C34]);pc[2] = getc(in_file[C34]);
        pc[1] = getc(in_file[C34]);pc[0] = getc(in_file[C34]);
        M_tmp[C34][ii][2 * col] = fl1;M_tmp[C34][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[C44]);pc[2] = getc(in_file[C44]);
        pc[1] = getc(in_file[C44]);pc[0] = getc(in_file[C44]);
        M_tmp[C44][ii][2 * col] = fl1;M_tmp[C44][ii][2 * col +1] = 0.;
        }
      }
    for (col = 0; col < Ncol; col++) {
      M_in[C411][ii][col] = M_tmp[C11][ii][2 * col];
      M_in[C412_re][ii][col] = M_tmp[C12][ii][2 * col];
      M_in[C412_im][ii][col] = M_tmp[C12][ii][2 * col + 1];
      M_in[C413_re][ii][col] = M_tmp[C13][ii][2 * col];
      M_in[C413_im][ii][col] = M_tmp[C13][ii][2 * col + 1];
      M_in[C414_re][ii][col] = M_tmp[C14][ii][2 * col];
      M_in[C414_im][ii][col] = M_tmp[C14][ii][2 * col + 1];
      M_in[C422][ii][col] = M_tmp[C22][ii][2 * col];
      M_in[C423_re][ii][col] = M_tmp[C23][ii][2 * col];
      M_in[C423_im][ii][col] = M_tmp[C23][ii][2 * col + 1];
      M_in[C424_re][ii][col] = M_tmp[C24][ii][2 * col];
      M_in[C424_im][ii][col] = M_tmp[C24][ii][2 * col + 1];
      M_in[C433][ii][col] = M_tmp[C33][ii][2 * col];
      M_in[C434_re][ii][col] = M_tmp[C34][ii][2 * col];
      M_in[C434_im][ii][col] = M_tmp[C34][ii][2 * col + 1];
      M_in[C444][ii][col] = M_tmp[C44][ii][2 * col];
      for (np = 0; np < 16; np++) if (my_isfinite(M_in[np][ii][col]) == 0) M_in[np][ii][col] = eps;
      }
    } /* ii */
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ...
        M_out[C311][col] += M_in[C411][ii][ind];
        M_out[C312_re][col] += (M_in[C412_re][ii][ind] + M_in[C413_re][ii][ind]) / sqrt(2);
        M_out[C312_im][col] += (M_in[C412_im][ii][ind] + M_in[C413_im][ii][ind]) / sqrt(2);
        M_out[C313_re][col] += M_in[C414_re][ii][ind];
        M_out[C313_im][col] += M_in[C414_im][ii][ind];
        M_out[C322][col] += (M_in[C422][ii][ind] + M_in[C433][ii][ind] + 2 * M_in[C423_re][ii][ind]) / 2;
        M_out[C323_re][col] += (M_in[C424_re][ii][ind] + M_in[C434_re][ii][ind]) / sqrt(2);
        M_out[C323_im][col] += (M_in[C424_im][ii][ind] + M_in[C434_im][ii][ind]) / sqrt(2);
        M_out[C333][col] += M_in[C444][ii][ind];
        }
      }
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ind = col * Nlook_col + jj + Ncoloffset;
        M_out[C411][col] += M_in[C411][ii][ind];
        M_out[C412_re][col] += M_in[C412_re][ii][ind];
        M_out[C412_im][col] += M_in[C412_im][ii][ind];
        M_out[C413_re][col] += M_in[C413_re][ii][ind];
        M_out[C413_im][col] += M_in[C413_im][ii][ind];
        M_out[C414_re][col] += M_in[C414_re][ii][ind];
        M_out[C414_im][col] += M_in[C414_im][ii][ind];
        M_out[C422][col] += M_in[C422][ii][ind];
        M_out[C423_re][col] += M_in[C423_re][ii][ind];
        M_out[C423_im][col] += M_in[C423_im][ii][ind];
        M_out[C424_re][col] += M_in[C424_re][ii][ind];
        M_out[C424_im][col] += M_in[C424_im][ii][ind];
        M_out[C433][col] += M_in[C433][ii][ind];
        M_out[C434_re][col] += M_in[C434_re][ii][ind];
        M_out[C434_im][col] += M_in[C434_im][ii][ind];
        M_out[C444][col] += M_in[C444][ii][ind];
        }
      }
    for (np = 0; np < Npolar_out; np++) M_out[np][col] /= Nlook_lig * Nlook_col;
    }
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ind = col * Nlook_col + jj + Ncoloffset;
        CC11 = M_in[C411][ii][ind];
        CC12_re = (M_in[C412_re][ii][ind] + M_in[C413_re][ii][ind]) / sqrt(2);
        CC12_im = (M_in[C412_im][ii][ind] + M_in[C413_im][ii][ind]) / sqrt(2);
        CC13_re = M_in[C414_re][ii][ind];
        CC13_im = M_in[C414_im][ii][ind];
        CC22 = (M_in[C422][ii][ind] + M_in[C433][ii][ind] + 2 * M_in[C423_re][ii][ind]) / 2;
        CC23_re = (M_in[C424_re][ii][ind] + M_in[C434_re][ii][ind]) / sqrt(2);
        CC23_im = (M_in[C424_im][ii][ind] + M_in[C434_im][ii][ind]) / sqrt(2);
        CC33 = M_in[C444][ii][ind];
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ind = col * Nlook_col + jj + Ncoloffset;
        M_out[T411][col] += (M_in[C411][ii][ind] + 2 * M_in[C414_re][ii][ind] + M_in[C444][ii][ind]) / 2.;
        M_out[T412_re][col] += (M_in[C411][ii][ind] - M_in[C444][ii][ind]) / 2.;
        M_out[T412_im][col] += (-2 * M_in[C414_im][ii][ind]) / 2.;
        M_out[T413_re][col] += (M_in[C412_re][ii][ind] + M_in[C413_re][ii][ind] + M_in[C424_re][ii][ind] + M_in[C434_re][ii][ind]) / 2.;
        M_out[T413_im][col] += (M_in[C412_im][ii][ind] + M_in[C413_im][ii][ind] - M_in[C424_im][ii][ind] - M_in[C434_im][ii][ind]) / 2.;
        M_out[T414_re][col] += (-M_in[C412_im][ii][ind] + M_in[C413_im][ii][ind] + M_in[C424_im][ii][ind] - M_in[C434_im][ii][ind]) / 2.;
        M_out[T414_im][col] += (M_in[C412_re][ii][ind] - M_in[C413_re][ii][ind] + M_in[C424_re][ii][ind] - M_in[C434_re][ii][ind]) / 2.;
        M_out[T422][col] += (M_in[C411][ii][ind] - 2 * M_in[C414_re][ii][ind] + M_in[C444][ii][ind]) / 2.;
        M_out[T423_re][col] += (M_in[C412_re][ii][ind] + M_in[C413_re][ii][ind] - M_in[C424_re][ii][ind] - M_in[C434_re][ii][ind]) / 2.;
        M_out[T423_im][col] += (M_in[C412_im][ii][ind] + M_in[C413_im][ii][ind] + M_in[C424_im][ii][ind] + M_in[C434_im][ii][ind]) / 2.;
        M_out[T424_re][col] += (-M_in[C412_im][ii][ind] + M_in[C413_im][ii][ind] - M_in[C424_im][ii][ind] + M_in[C434_im][ii][ind]) / 2.;
        M_out[T424_im][col] += (M_in[C412_re][ii][ind] - M_in[C413_re][ii][ind] - M_in[C424_re][ii][ind] + M_in[C434_re][ii][ind]) / 2.;
        M_out[T433][col] += (M_in[C422][ii][ind] + M_in[C433][ii][ind] + 2 * M_in[C423_re][ii][ind]) / 2.;
        M_out[T434_re][col] += (2 * M_in[C423_im][ii][ind]) / 2.;
        M_out[T434_im][col] += (M_in[C422][ii][ind] - M_in[C433][ii][ind]) / 2.;
        M_out[T444][col] += (M_in[C422][ii][ind] + M_in[C433][ii][ind] - 2 * M_in[C423_re][ii][ind]) / 2.;
        }
      }
    for (np = 0; np < Npolar_out; np++) M_out[np][col] /= Nlook_lig * Nlook_col;
    }
  ...
  free_matrix3d_float(M_in, 16, Nlook_lig);
