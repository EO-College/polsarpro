...
File   : rawbinary_convert_Float_MLK_IPP.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_Float_MLK_IPP DirOutput Ncol OffsetLig OffsetCol\n");
  printf("FinalNlig FinalNcol IEEEFormat_Convert (0/1) Symmetrisation OutputDataFormat\n");
  printf("Nlook_col Nlook_lig PolarType (PP5, PP6, PP7)\n");
...
/* Nb of lines and rows sub-sampled image */
  ...
  M_in = matrix3d_float(2, Nlook_lig, Ncol);
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ...
        for (np = 0; np < 2; np++) if (my_isfinite(M_in[np][ii][ind]) == 0) M_in[np][ii][ind] = eps;
        M_out[0][col] += M_in[0][ii][ind];
        M_out[1][col] += M_in[1][ii][ind];
        }
      }
