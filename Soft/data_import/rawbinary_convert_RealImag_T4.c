...
File   : rawbinary_convert_RealImag_T4.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_RealImag_T4 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[T411][col] = M_tmp[T411][col];
     M_in[T412_re][col] = M_tmp[T412_re][col];
     M_in[T412_im][col] = M_tmp[T412_im][col];
     M_in[T413_re][col] = M_tmp[T413_re][col];
     M_in[T413_im][col] = M_tmp[T413_im][col];
     M_in[T414_re][col] = M_tmp[T414_re][col];
     M_in[T414_im][col] = M_tmp[T414_im][col];
     M_in[T422][col] = M_tmp[T422][col];
     M_in[T423_re][col] = M_tmp[T423_re][col];
     M_in[T423_im][col] = M_tmp[T423_im][col];
     M_in[T424_re][col] = M_tmp[T424_re][col];
     M_in[T424_im][col] = M_tmp[T424_im][col];
     M_in[T433][col] = M_tmp[T433][col];
     M_in[T434_re][col] = M_tmp[T434_re][col];
     M_in[T434_im][col] = M_tmp[T434_im][col];
     M_in[T444][col] = M_tmp[T444][col];
