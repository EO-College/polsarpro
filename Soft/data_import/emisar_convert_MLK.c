...
Description :  Convert EMISAR Binary Data Files 
               (Format 3x3 Covariance Matrix)
...
/* LOCAL VARIABLES */
  ...
  char File11[FilePathLength],File12[FilePathLength],File13[FilePathLength],File22[FilePathLength],File23[FilePathLength],File33[FilePathLength];
...
strcpy(UsageHelp,"\nemisar_convert_MLK.exe\n");
strcat(UsageHelp,"\nParameters:\n");
strcat(UsageHelp," (string)	-if1 	input data file: C11.bin\n");
strcat(UsageHelp," (string)	-if2 	input data file: C12.bin\n");
strcat(UsageHelp," (string)	-if3 	input data file: C13.bin\n");
strcat(UsageHelp," (string)	-if4 	input data file: C22.bin\n");
strcat(UsageHelp," (string)	-if5 	input data file: C23.bin\n");
strcat(UsageHelp," (string)	-if6 	input data file: C33.bin\n");
...
if(argc < 39) {
  ...
  get_commandline_prm(argc,argv,"-if3",str_cmd_prm,File13,1,UsageHelp);
  get_commandline_prm(argc,argv,"-if4",str_cmd_prm,File22,1,UsageHelp);
  get_commandline_prm(argc,argv,"-if5",str_cmd_prm,File23,1,UsageHelp);
  get_commandline_prm(argc,argv,"-if6",str_cmd_prm,File33,1,UsageHelp);
...
/* DATA PROCESSING */
  ...
  for (lig = 0; lig < Off_lig; lig++) {
    fread(&C_in[C11][0], sizeof(float), Ncol, in_file[C11]);
    fread(&C_in[C12][0], sizeof(float), 2 * Ncol, in_file[C12]);
    fread(&C_in[C13][0], sizeof(float), 2 * Ncol, in_file[C13]);
    fread(&C_in[C22][0], sizeof(float), Ncol, in_file[C22]);
    fread(&C_in[C23][0], sizeof(float), 2 * Ncol, in_file[C23]);
    fread(&C_in[C33][0], sizeof(float), Ncol, in_file[C33]);
    }
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        pc[3] = getc(in_file[C12]);pc[2] = getc(in_file[C12]);
        pc[1] = getc(in_file[C12]);pc[0] = getc(in_file[C12]);
        C_in[C12][2 * col] = fl1;C_in[C12][2 * col + 1] = fl2;
        ...
        pc[3] = getc(in_file[C13]);pc[2] = getc(in_file[C13]);
        pc[1] = getc(in_file[C13]);pc[0] = getc(in_file[C13]);
        C_in[C13][2 * col] = fl1;C_in[C13][2 * col + 1] = fl2;
        ...
        pc[3] = getc(in_file[C23]);pc[2] = getc(in_file[C23]);
        pc[1] = getc(in_file[C23]);pc[0] = getc(in_file[C23]);
        C_in[C23][2 * col] = fl1;C_in[C23][2 * col + 1] = fl2;
    ...
    for (col = 0; col < Ncol; col++) {
      M_in[C311][lig][col] = C_in[C11][col];
      M_in[C312_re][lig][col] = sqrt(2.)*C_in[C12][2*col];
      M_in[C312_im][lig][col] = sqrt(2.)*C_in[C12][2*col+1];
      M_in[C313_re][lig][col] = C_in[C13][2*col];
      M_in[C313_im][lig][col] = C_in[C13][2*col+1];
      M_in[C322][lig][col] = 2.*C_in[C22][col];
      M_in[C323_re][lig][col] = sqrt(2.)*C_in[C23][2*col];
      M_in[C323_im][lig][col] = sqrt(2.)*C_in[C23][2*col+1];
      M_in[C333][lig][col] = C_in[C33][col];
