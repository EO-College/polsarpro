...
File   : rawbinary_convert_Cmplx_S2.c
...
/******************************************************************************/
  ...
  printf("TYPE: rawbinary_convert_Cmplx_S2 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (np = 0; np < Npolar_in; np++) {
     if (IEEE == 0)
      fread(&M_tmp[np][0], sizeof(float), 2 * Ncol, in_file[np]);
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        pc[3] = getc(in_file[np]);pc[2] = getc(in_file[np]);
        pc[1] = getc(in_file[np]);pc[0] = getc(in_file[np]);
        M_tmp[np][2 * col] = fl1;M_tmp[np][2 * col + 1] = fl2;
        }
      }
    }
  for (np = 0; np < Npolar_in; np++) {
   for (col = 0; col < Ncol; col++) {
     M_in[np][2 * col] = M_tmp[np][2 * col];
     M_in[np][2 * col + 1] = M_tmp[np][2 * col +1];
