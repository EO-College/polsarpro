...
File   : rawbinary_convert_Cmplx_MLK_C3.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_Cmplx_MLK_C3 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (ii = 0; ii < Nlook_lig; ii++) {
    ...
    for (col = 0; col < Ncol; col++) {
      M_in[C311][ii][col] = M_tmp[C11][ii][2 * col];
      M_in[C312_re][ii][col] = M_tmp[C12][ii][2 * col];
      M_in[C312_im][ii][col] = M_tmp[C12][ii][2 * col + 1];
      M_in[C313_re][ii][col] = M_tmp[C13][ii][2 * col];
      M_in[C313_im][ii][col] = M_tmp[C13][ii][2 * col + 1];
      M_in[C322][ii][col] = M_tmp[C22][ii][2 * col];
      M_in[C323_re][ii][col] = M_tmp[C23][ii][2 * col];
      M_in[C323_im][ii][col] = M_tmp[C23][ii][2 * col + 1];
      M_in[C333][ii][col] = M_tmp[C33][ii][2 * col];
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ...
        M_out[C311][col] += M_in[C311][ii][ind];
        M_out[C312_re][col] += M_in[C312_re][ii][ind];
        M_out[C312_im][col] += M_in[C312_im][ii][ind];
        M_out[C313_re][col] += M_in[C313_re][ii][ind];
        M_out[C313_im][col] += M_in[C313_im][ii][ind];
        M_out[C322][col] += M_in[C322][ii][ind];
        M_out[C323_re][col] += M_in[C323_re][ii][ind];
        M_out[C323_im][col] += M_in[C323_im][ii][ind];
        M_out[C333][col] += M_in[C333][ii][ind];
        }
      }
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ind = col * Nlook_col + jj + Ncoloffset;
        M_out[T311][col] += (M_in[C311][ii][ind] + 2 * M_in[C313_re][ii][ind] + M_in[C333][ii][ind]) / 2;
        M_out[T312_re][col] += (M_in[C311][ii][ind] - M_in[C333][ii][ind]) / 2;
        M_out[T312_im][col] += -M_in[C313_im][ii][ind];
        M_out[T313_re][col] += (M_in[C312_re][ii][ind] + M_in[C323_re][ii][ind]) / sqrt(2);
        M_out[T313_im][col] += (M_in[C312_im][ii][ind] - M_in[C323_im][ii][ind]) / sqrt(2);
        M_out[T322][col] += (M_in[C311][ii][ind] - 2 * M_in[C313_re][ii][ind] + M_in[C333][ii][ind]) / 2;
        M_out[T323_re][col] += (M_in[C312_re][ii][ind] - M_in[C323_re][ii][ind]) / sqrt(2);
        M_out[T323_im][col] += (M_in[C312_im][ii][ind] + M_in[C323_im][ii][ind]) / sqrt(2);
        M_out[T333][col] += M_in[C322][ii][ind];
        }
      }
    for (np = 0; np < Npolar_out; np++) M_out[np][col] /= Nlook_lig * Nlook_col;
    }
