...
File   : rawbinary_convert_Cmplx_T3.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_Cmplx_T3 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[T311][col] = M_tmp[T11][2 * col];
     M_in[T312_re][col] = M_tmp[T12][2 * col];
     M_in[T312_im][col] = M_tmp[T12][2 * col + 1];
     M_in[T313_re][col] = M_tmp[T13][2 * col];
     M_in[T313_im][col] = M_tmp[T13][2 * col + 1];
     M_in[T322][col] = M_tmp[T22][2 * col];
     M_in[T323_re][col] = M_tmp[T23][2 * col];
     M_in[T323_im][col] = M_tmp[T23][2 * col + 1];
     M_in[T333][col] = M_tmp[T33][2 * col];
