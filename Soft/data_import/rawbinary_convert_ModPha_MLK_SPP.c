...
File   : rawbinary_convert_ModPha_MLK_SPP.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_ModPha_MLK_SPP DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (ii = 0; ii < Nlook_lig; ii++) {
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[0][ii][2 * col] = M_tmp[0][ii][col]*cos(M_tmp[1][ii][col]);
     M_in[0][ii][2 * col + 1] = M_tmp[0][ii][col]*sin(M_tmp[1][ii][col]);
     M_in[1][ii][2 * col] = M_tmp[2][ii][col]*cos(M_tmp[3][ii][col]);
     M_in[1][ii][2 * col + 1] = M_tmp[2][ii][col]*sin(M_tmp[3][ii][col]);
     for (np = 0; np < 2; np++) {
