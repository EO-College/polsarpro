...
File   : rawbinary_convert_RealImag_S2.c
...
/******************************************************************************/
  ...
  printf("TYPE: rawbinary_convert_RealImag_S2 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  if (lig%(int)(Nligfin/20) == 0) {printf("%f\r", 100. * lig / (Nligfin - 1));fflush(stdout);}
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[hh][2 * col] = M_tmp[hhr][col];M_in[hh][2 * col + 1] = M_tmp[hhi][col];
     M_in[hv][2 * col] = M_tmp[hvr][col];M_in[hv][2 * col + 1] = M_tmp[hvi][col];
     M_in[vh][2 * col] = M_tmp[vhr][col];M_in[vh][2 * col + 1] = M_tmp[vhi][col];
     M_in[vv][2 * col] = M_tmp[vvr][col];M_in[vv][2 * col + 1] = M_tmp[vvi][col];
     for (np = 0; np < 4; np++) {
