...
/* SKIP HEADER */
  ...
  for (lig = 0; lig < Off_lig; lig++) {
    ...
    for (Np = 0; Np < Npol; Np++) fread(&Min[Np][0], sizeof(short int), 2 * Ncol, in_file[Np]);
    Strip++;
    }
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Ncol; col++) {
      for (Np = 0; Np < Npol; Np++) {
        S_in[Np][lig][2*col] = (float)Min[Np][2*col];
        S_in[Np][lig][2*col+1] = (float)Min[Np][2*col+1];
