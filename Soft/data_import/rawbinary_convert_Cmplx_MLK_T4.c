...
File   : rawbinary_convert_Cmplx_MLK_T4.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_Cmplx_MLK_T4 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligoffset; lig++) {
  ...
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[T14]);
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[T22]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[T23]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[T24]);
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[T33]);
  fread(&M_tmp[0][0][0], sizeof(float), 2*Ncol, in_file[T34]);
  fread(&M_tmp[0][0][0], sizeof(float), Ncol, in_file[T44]);
  }
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (ii = 0; ii < Nlook_lig; ii++) {
     if (IEEE == 0) {
      ...
      fread(&M_tmp[T13][ii][0], sizeof(float), 2 * Ncol, in_file[T14]);
      fread(&M_tmp[T22][ii][0], sizeof(float), Ncol, in_file[T22]);
      fread(&M_tmp[T23][ii][0], sizeof(float), 2 * Ncol, in_file[T23]);
      fread(&M_tmp[T23][ii][0], sizeof(float), 2 * Ncol, in_file[T24]);
      fread(&M_tmp[T33][ii][0], sizeof(float), Ncol, in_file[T33]);
      fread(&M_tmp[T23][ii][0], sizeof(float), 2 * Ncol, in_file[T34]);
      fread(&M_tmp[T33][ii][0], sizeof(float), Ncol, in_file[T44]);
      }
    if (IEEE == 1) {
      for (col = 0; col < Ncol; col++) {
        ...
        pc[3] = getc(in_file[T14]);pc[2] = getc(in_file[T14]);
        pc[1] = getc(in_file[T14]);pc[0] = getc(in_file[T14]);
        M_tmp[T14][ii][2 * col] = fl1;M_tmp[T14][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[T24]);pc[2] = getc(in_file[T24]);
        pc[1] = getc(in_file[T24]);pc[0] = getc(in_file[T24]);
        M_tmp[T24][ii][2 * col] = fl1;M_tmp[T24][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        ...
        pc[3] = getc(in_file[T34]);pc[2] = getc(in_file[T34]);
        pc[1] = getc(in_file[T34]);pc[0] = getc(in_file[T34]);
        M_tmp[T34][ii][2 * col] = fl1;M_tmp[T34][ii][2 * col + 1] = fl2;
        v = &fl1;pc = (char *) v;
        pc[3] = getc(in_file[T44]);pc[2] = getc(in_file[T44]);
        pc[1] = getc(in_file[T44]);pc[0] = getc(in_file[T44]);
        M_tmp[T44][ii][2 * col] = fl1;M_tmp[T44][ii][2 * col +1] = 0.;
        }
      }
    for (col = 0; col < Ncol; col++) {
      M_in[T411][ii][col] = M_tmp[T11][ii][2 * col];
      M_in[T412_re][ii][col] = M_tmp[T12][ii][2 * col];
      M_in[T412_im][ii][col] = M_tmp[T12][ii][2 * col + 1];
      M_in[T413_re][ii][col] = M_tmp[T13][ii][2 * col];
      M_in[T413_im][ii][col] = M_tmp[T13][ii][2 * col + 1];
      M_in[T414_re][ii][col] = M_tmp[T14][ii][2 * col];
      M_in[T414_im][ii][col] = M_tmp[T14][ii][2 * col + 1];
      M_in[T422][ii][col] = M_tmp[T22][ii][2 * col];
      M_in[T423_re][ii][col] = M_tmp[T23][ii][2 * col];
      M_in[T423_im][ii][col] = M_tmp[T23][ii][2 * col + 1];
      M_in[T424_re][ii][col] = M_tmp[T24][ii][2 * col];
      M_in[T424_im][ii][col] = M_tmp[T24][ii][2 * col + 1];
      M_in[T433][ii][col] = M_tmp[T33][ii][2 * col];
      M_in[T434_re][ii][col] = M_tmp[T34][ii][2 * col];
      M_in[T434_im][ii][col] = M_tmp[T34][ii][2 * col + 1];
      M_in[T444][ii][col] = M_tmp[T44][ii][2 * col];
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ...
        M_out[T411][col] += M_in[T411][ii][ind];
        M_out[T412_re][col] += M_in[T412_re][ii][ind];
        M_out[T412_im][col] += M_in[T412_im][ii][ind];
        M_out[T413_re][col] += M_in[T413_re][ii][ind];
        M_out[T413_im][col] += M_in[T413_im][ii][ind];
        M_out[T422][col] += M_in[T422][ii][ind];
        M_out[T423_re][col] += M_in[T423_re][ii][ind];
        M_out[T423_im][col] += M_in[T423_im][ii][ind];
        M_out[T433][col] += M_in[T433][ii][ind];
        }
      }
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ...
        M_out[T413_re][col] += M_in[T413_re][ii][ind];
        M_out[T413_im][col] += M_in[T413_im][ii][ind];
        M_out[T414_re][col] += M_in[T414_re][ii][ind];
        M_out[T414_im][col] += M_in[T414_im][ii][ind];
        M_out[T422][col] += M_in[T422][ii][ind];
        M_out[T423_re][col] += M_in[T423_re][ii][ind];
        M_out[T423_im][col] += M_in[T423_im][ii][ind];
        M_out[T424_re][col] += M_in[T424_re][ii][ind];
        M_out[T424_im][col] += M_in[T424_im][ii][ind];
        M_out[T433][col] += M_in[T433][ii][ind];
        M_out[T434_re][col] += M_in[T434_re][ii][ind];
        M_out[T434_im][col] += M_in[T434_im][ii][ind];
        M_out[T444][col] += M_in[T444][ii][ind];
        }
      }
    for (np = 0; np < Npolar_out; np++) M_out[np][col] /= Nlook_lig * Nlook_col;
    }
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ind = col * Nlook_col + jj + Ncoloffset;
        M_out[C311][col] += (M_in[T411][ii][ind] + 2 * M_in[T412_re][ii][ind] + M_in[T422][ii][ind]) / 2;
        M_out[C312_re][col] += (M_in[T413_re][ii][ind] + M_in[T423_re][ii][ind]) / sqrt(2);
        M_out[C312_im][col] += (M_in[T413_im][ii][ind] + M_in[T423_im][ii][ind]) / sqrt(2);
        M_out[C313_re][col] += (M_in[T411][ii][ind] - M_in[T422][ii][ind]) / 2;
        M_out[C313_im][col] += -M_in[T412_im][ii][ind];
        M_out[C322][col] += M_in[T433][ii][ind];
        M_out[C323_re][col] += (M_in[T413_re][ii][ind] - M_in[T423_re][ii][ind]) / sqrt(2);
        M_out[C323_im][col] += (-M_in[T413_im][ii][ind] + M_in[T423_im][ii][ind]) / sqrt(2);
        M_out[C333][col] += (M_in[T411][ii][ind] - 2 * M_in[T412_re][ii][ind] + M_in[T422][ii][ind]) / 2;
        }
      }
    for (np = 0; np < Npolar_out; np++) M_out[np][col] /= Nlook_lig * Nlook_col;
    }
  ...
  for (col = 0; col < Ncolfin; col++) {
    ...
    for (ii = 0; ii < Nlook_lig; ii++) {
      for (jj = 0; jj < Nlook_col; jj++) {
        ind = col * Nlook_col + jj + Ncoloffset;
        M_out[C411][col] += (M_in[T411][ii][ind] + 2 * M_in[T412_re][ii][ind] + M_in[T422][ii][ind]) / 2.;
        M_out[C412_re][col] += (M_in[T413_re][ii][ind] + M_in[T423_re][ii][ind] + M_in[T414_im][ii][ind] + M_in[T424_im][ii][ind]) / 2.;
        M_out[C412_im][col] += (M_in[T413_im][ii][ind] + M_in[T423_im][ii][ind] - M_in[T414_re][ii][ind] - M_in[T424_re][ii][ind]) / 2.;
        M_out[C413_re][col] += (M_in[T413_re][ii][ind] + M_in[T423_re][ii][ind] - M_in[T414_im][ii][ind] - M_in[T424_im][ii][ind]) / 2.;
        M_out[C413_im][col] += (M_in[T413_im][ii][ind] + M_in[T423_im][ii][ind] + M_in[T414_re][ii][ind] + M_in[T424_re][ii][ind]) / 2.;
        M_out[C414_re][col] += (M_in[T411][ii][ind] - M_in[T422][ii][ind]) / 2.;
        M_out[C414_im][col] += (-2 * M_in[T412_im][ii][ind]) / 2.;
        M_out[C422][col] += (M_in[T433][ii][ind] + 2 * M_in[T434_re][ii][ind] + M_in[T444][ii][ind]) / 2.;
        M_out[C423_re][col] += (M_in[T433][ii][ind] - M_in[T444][ii][ind]) / 2.;
        M_out[C423_im][col] += (2 * M_in[T434_re][ii][ind]) / 2.;
        M_out[C424_re][col] += (M_in[T413_re][ii][ind] - M_in[T423_re][ii][ind] + M_in[T414_im][ii][ind] - M_in[T424_im][ii][ind]) / 2.;
        M_out[C424_im][col] += (-M_in[T413_im][ii][ind] + M_in[T423_im][ii][ind] + M_in[T414_re][ii][ind] - M_in[T424_re][ii][ind]) / 2.;
        M_out[C433][col] += (M_in[T433][ii][ind] + M_in[T444][ii][ind] - 2 * M_in[T434_im][ii][ind]) / 2.;
        M_out[C434_re][col] += (M_in[T413_re][ii][ind] - M_in[T423_re][ii][ind] - M_in[T414_im][ii][ind] + M_in[T424_im][ii][ind]) / 2.;
        M_out[C434_im][col] += (-M_in[T413_im][ii][ind] + M_in[T423_im][ii][ind] - M_in[T414_re][ii][ind] + M_in[T424_re][ii][ind]) / 2.;
        M_out[C444][col] += (M_in[T411][ii][ind] - 2 * M_in[T412_re][ii][ind] + M_in[T422][ii][ind]) / 2;
        }
      }
    for (np = 0; np < Npolar_out; np++) M_out[np][col] /= Nlook_lig * Nlook_col;
    }
