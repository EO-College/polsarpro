...
/* SKIP HEADER */
  ...
  for (Np = 0; Np < Npol; Np++) fseek(in_file[Np], Nlig*(Ncol*2)*sizeof(float), SEEK_CUR);
  for (Np = 0; Np < Npol; Np++) fseek(in_file[Np], -(Ncol*2)*(Off_lig+1)*sizeof(float), SEEK_CUR);
  fseek(in_mask, Nlig*Ncol*sizeof(char), SEEK_CUR);
  fseek(in_mask, -Ncol*(Off_lig+1)*sizeof(char), SEEK_CUR);
  fseek(in_incang, Nlig*Ncol*sizeof(float), SEEK_CUR);
  fseek(in_incang, -Ncol*(Off_lig+1)*sizeof(float), SEEK_CUR);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    fseek(in_mask, -2*Ncol*sizeof(char), SEEK_CUR);
    fread(&VIncAngIn[0], sizeof(float), Ncol, in_incang);
    fseek(in_incang, -2*Ncol*sizeof(float), SEEK_CUR);
    for (Np = 0; Np < Npol; Np++) {
      if (IEEE == 0)
      ...
      fseek(in_file[Np], -2*(Ncol*2)*sizeof(float), SEEK_CUR); 
      }
    for (col = 0; col < Ncol; col++) {
      MaskIn[lig][col] = 0.;
