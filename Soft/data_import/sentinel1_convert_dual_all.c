...
/* Internal variables */
  ...
  short int fl1, fl2;
  short int *v;
  unsigned long PointerPosition, CurrentPointerPosition;
...
********************************************************************/
  ...
  BurstSubColMax = BurstSubCol[0];
  for (ii = 0; ii < BurstNumMax; ii++) {
    if (BurstSubColMax < BurstSubCol[ii]) BurstSubColMax = BurstSubCol[ii];
    }
  ...
  for (ii = 0; ii < BurstNumMax; ii++) {
    ...
    for (jj = 0; jj < ii; jj++) {
      for (lig = 0; lig < linesPerBurst; lig++) {
        ...
        for (Np = 0; Np < Npol; Np++) {
          CurrentPointerPosition = ftell(in_file[Np]);
          fseek(in_file[Np], (PointerPosition - CurrentPointerPosition), SEEK_CUR);
          }
    ...
    if (ii == BurstNumMax - 1) {
      for (lig = 0; lig < BurstSubLig[ii]; lig++) {
        ...
        for (col = 0; col < BurstSubCol[ii]; col++) {
          for (Np = 0; Np < Npol; Np++) {
            Sout[Np][2*col] = (float)Sin[Np][2*(col+BurstOffCol[ii])];
            Sout[Np][2*col+1] = (float)Sin[Np][2*(col+BurstOffCol[ii])+1];
            if (my_isfinite(Sout[Np][2*col]) == 0) Sout[Np][2*col] = eps;
            if (my_isfinite(Sout[Np][2*col + 1]) == 0) Sout[Np][2*col + 1] = eps;
            }
          }
        for (col = BurstSubCol[ii]; col < BurstSubColMax; col++) {
          for (Np = 0; Np < Npol; Np++) {
            Sout[Np][2*col] = 0.;
            Sout[Np][2*col+1] = 0.;
            }
