...
File   : rawbinary_convert_RealImag_MLK_S2.c
...
//  Symmetrisation = atoi(argv[8]);
  ...
  printf("TYPE: rawbinary_convert_RealImag_MLK_S2 DirOutput Ncol OffsetLig OffsetCol\n");
...
for (lig = 0; lig < Nligfin; lig++) {
  ...
  for (ii = 0; ii < Nlook_lig; ii++) {
   ...
   for (col = 0; col < Ncol; col++) {
     M_in[hh][ii][2 * col] = M_tmp[hhr][ii][col];M_in[hh][ii][2 * col + 1] = M_tmp[hhi][ii][col];
     M_in[hv][ii][2 * col] = M_tmp[hvr][ii][col];M_in[hv][ii][2 * col + 1] = M_tmp[hvi][ii][col];
     M_in[vh][ii][2 * col] = M_tmp[vhr][ii][col];M_in[vh][ii][2 * col + 1] = M_tmp[vhi][ii][col];
     M_in[vv][ii][2 * col] = M_tmp[vvr][ii][col];M_in[vv][ii][2 * col + 1] = M_tmp[vvi][ii][col];
     for (np = 0; np < 4; np++) {
