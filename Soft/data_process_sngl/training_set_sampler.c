...
/*Training class matrix memory allocation */
  ...
  if (strcmp(PolType,"IPP")==0) {
    for (k = 0; k < Npp; k++) cov_area[k] = vector_float(N_zones);
    } else {
...
/****************************************************/
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (ValidMask[col + Off_col] == 1.) {
        if (border_map[lig + Off_lig][col + Off_col] == zone) {
          ...
          if (strcmp(PolType,"IPP")==0) {
            for (k = 0; k < Npp; k++) cov_area[k][zone] = cov_area[k][zone] + eps + M_in[k][lig][col];
            } else {
            for (k = 0; k < Npp; k++)
              for (l = 0; l < Npp; l++) {
  ...
  if (strcmp(PolType,"IPP")==0) {
    for (k = 0; k < Npp; k++) cov_area[k][zone] = cov_area[k][zone] / cpt_zones[zone];
    } else {
    for (k = 0; k < Npp; k++)
      for (l = 0; l < Npp; l++) {
...
if (border_error_flag == 0) {
  ...
  for (zone = 0; zone < N_zones; zone++) {
    ...
    if (strcmp(PolType,"IPP")==0) {
      for (k = 0; k < Npp; k++) M_trn[k] = cov_area[k][zone];
      }
