...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        M_out1[lig][col] = 0.5 * atan2(sqrt(M_avg[T444][lig][col]),sqrt(M_avg[T411][lig][col]));
        M_out1[lig][col] = M_out1[lig][col] * 180. / pi;
        M_out2[lig][col] = -0.25 * atan2(-2*M_avg[T414_re][lig][col],M_avg[T411][lig][col]-M_avg[T444][lig][col]);
