...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  NptS++;
  MinMaxContrastMedianNN(datatmp, &PsMin, &PsMax, NptS, NN);

  PsMin = eps;
  AreaSsNc = (int)(NptS / Ncluster);

/* Histograms DETERMINATION */
rewind(in_odd); rewind(in_dbl); rewind(in_vol);
if (FlagValid == 1) rewind(in_valid);

for (ii = 0; ii <= Ncluster100; ii++) PsHisto[ii] = 0;
...
//  if (PsHisto[Ncluster100] <= AreaSsNc) {
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  NptD++;
  MinMaxContrastMedianNN(datatmp, &PdMin, &PdMax, NptD, NN);
...
for (ii = 0; ii <= Ncluster100; ii++) PdHisto[ii] = 0;
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  //if (PdHisto[Ncluster100] <= AreaDsNc) {
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  NptV++;
  MinMaxContrastMedianNN(datatmp, &PvMin, &PvMax, NptV, NN);
...
for (ii = 0; ii <= Ncluster100; ii++) PvHisto[ii] = 0;
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  //if (PvHisto[Ncluster100] <= AreaVsNc) {
      ...
      if (Valid[lig][col] == 1.) {
        ...
        if ((Power == M_odd[lig][col])&&((Power / (M_odd[lig][col]+M_dbl[lig][col]+M_vol[lig][col])) > mixed_threshold)) {
          if (PsMax <= M_odd[lig][col]) {
            for (Np = 0; Np < NpolarOut; Np++) MM[odd][Ncluster][Np] += M_avg[Np][lig][col];
            ...
            for (Np = 0; Np < NpolarOut; Np++) MM[odd][ii][Np] += M_avg[Np][lig][col];
        ...
        if ((Power == M_dbl[lig][col])&&((Power / (M_odd[lig][col]+M_dbl[lig][col]+M_vol[lig][col])) > mixed_threshold)) {
          if (PdMax <= M_dbl[lig][col]) {
            for (Np = 0; Np < NpolarOut; Np++) MM[dbl][Ncluster][Np] += M_avg[Np][lig][col];
            ...
            for (Np = 0; Np < NpolarOut; Np++) MM[dbl][ii][Np] += M_avg[Np][lig][col];
        ...
        if ((Power == M_vol[lig][col])&&((Power / (M_odd[lig][col]+M_dbl[lig][col]+M_vol[lig][col])) > mixed_threshold)) {
          if (PvMax <= M_vol[lig][col]) {
            for (Np = 0; Np < NpolarOut; Np++) MM[vol][Ncluster][Np] += M_avg[Np][lig][col];
            ...
            for (Np = 0; Np < NpolarOut; Np++) MM[vol][ii][Np] += M_avg[Np][lig][col];
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        ...
        if ((Power == M_odd[lig][col])&&((Power / (M_odd[lig][col]+M_dbl[lig][col]+M_vol[lig][col])) > mixed_threshold)) {
          ...
          TT[0][0][0] = eps + M_avg[0][lig][col]; TT[0][0][1] = 0.;
          TT[0][1][0] = eps + M_avg[1][lig][col]; TT[0][1][1] = eps + M_avg[2][lig][col];
          TT[0][2][0] = eps + M_avg[3][lig][col]; TT[0][2][1] = eps + M_avg[4][lig][col];
          TT[1][0][0] =  TT[0][1][0]; TT[1][0][1] = -TT[0][1][1];
          TT[1][1][0] = eps + M_avg[5][lig][col]; TT[1][1][1] = 0.;
          TT[1][2][0] = eps + M_avg[6][lig][col]; TT[1][2][1] = eps + M_avg[7][lig][col];
          TT[2][0][0] =  TT[0][2][0]; TT[2][0][1] = -TT[0][2][1];
          TT[2][1][0] =  TT[1][2][0]; TT[2][1][1] = -TT[1][2][1];
          TT[2][2][0] = eps + M_avg[8][lig][col]; TT[2][2][1] = 0.;
