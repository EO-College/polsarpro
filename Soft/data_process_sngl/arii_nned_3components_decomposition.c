...
strcpy(UsageHelp,"\narii_nned_3components_decomposition.exe\n");
strcat(UsageHelp,"\nParameters:\n");
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%s%s", out_dir, "Arii3_NNED_Odd.bin");
  if ((out_odd = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%s%s", out_dir, "Arii3_NNED_Dbl.bin");
  if ((out_dbl = fopen(file_name, "wb")) == NULL)
    edit_error("Could not open input file : ", file_name);

  sprintf(file_name, "%s%s", out_dir, "Arii3_NNED_Vol.bin");
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        epsilon = M_avg[C311][lig][col];
        rho_re = M_avg[C313_re][lig][col];
        rho_im = M_avg[C313_im][lig][col];
        nhu = M_avg[C322][lig][col];
        gamma = M_avg[C333][lig][col];
