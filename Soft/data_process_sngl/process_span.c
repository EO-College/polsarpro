...
strcat(UsageHelp," (string)	-fmt 	Output Format (lin, db)\n");
strcat(UsageHelp,"\nOptional Parameters:\n");
...
/* OUTPUT FILE OPENING*/
  if (strcmp(Format, "lin") == 0) sprintf(file_name, "%sspan.bin", out_dir);
  if (strcmp(Format, "db") == 0) sprintf(file_name, "%sspan_db.bin", out_dir);
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if ((strcmp(PolTypeIn,"S2")==0) || (strcmp(PolTypeIn,"SPP") == 0) 
    ...
    if (strcmp(PolTypeIn,"S2")==0) {
      ...
      read_block_SPP_avg(in_datafile, M_in, PolTypeOut, NpolarOut, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, NwinL, NwinC, Off_lig, Off_col, Ncol);
      }
    } else {
    /* Case of C,T or I */
    read_block_TCI_avg(in_datafile, M_in, NpolarOut, Nb, NbBlock, NligBlock[Nb], Sub_Ncol, NwinL, NwinC, Off_lig, Off_col, Ncol);
    }
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if (strcmp(PolTypeOut, "IPPpp5") == 0) M_out[lig][col] = M_in[0][lig][col]+M_in[1][lig][col];
        if (strcmp(PolTypeOut, "IPPpp6") == 0) M_out[lig][col] = M_in[0][lig][col]+M_in[1][lig][col]; 
        if (strcmp(PolTypeOut, "IPPpp7") == 0) M_out[lig][col] = M_in[0][lig][col]+M_in[1][lig][col];
        if (strcmp(PolTypeOut, "IPPpp4") == 0) M_out[lig][col] = M_in[0][lig][col]+2*M_in[1][lig][col]+M_in[2][lig][col];
        if (strcmp(PolTypeOut, "IPPfull") == 0) M_out[lig][col] = M_in[0][lig][col]+M_in[1][lig][col]+M_in[2][lig][col]+M_in[3][lig][col];
        if ((strcmp(PolTypeOut,"C2")==0)||(strcmp(PolTypeOut,"C2pp1")==0)||(strcmp(PolTypeOut,"C2pp2")==0)||(strcmp(PolTypeOut,"C2pp3")==0)) {
          M_out[lig][col] = M_in[C211][lig][col]+M_in[C222][lig][col];
          }
        if ((strcmp(PolTypeOut,"C3")==0)||(strcmp(PolTypeOut,"T3")==0)) {
          M_out[lig][col] = M_in[C311][lig][col]+M_in[C322][lig][col]+M_in[C333][lig][col];
          } 
        if ((strcmp(PolTypeOut,"C4")==0)||(strcmp(PolTypeOut,"T4")==0)) {
          M_out[lig][col] = M_in[C411][lig][col]+M_in[C422][lig][col]+M_in[C433][lig][col]+M_in[C444][lig][col];
          }
        if (strcmp(Format, "db") == 0) {
          if (M_out[lig][col] <= eps) M_out[lig][col] = eps;
          M_out[lig][col] = 10. * log10(M_out[lig][col]+eps);
          }
        } else {
        M_out[lig][col] = 0.;
        }
