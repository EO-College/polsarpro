...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  for (lig = 0; lig < NligBlock[Nb]; lig++) {
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        TT11 = eps + M_avg[0][lig][col];
        TT12_re = eps + M_avg[1][lig][col];
        TT12_im = eps + M_avg[2][lig][col];
        TT13_re = eps + M_avg[3][lig][col];
        TT13_im = eps + M_avg[4][lig][col];
        TT22 = eps + M_avg[5][lig][col];
        TT23_re = eps + M_avg[6][lig][col];
        TT23_im = eps + M_avg[7][lig][col];
        TT33 = eps + M_avg[8][lig][col];
