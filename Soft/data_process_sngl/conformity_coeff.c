...
Description :  Conformity Coefficient determination
...
/* OUTPUT FILE OPENING*/
  sprintf(file_name, "%s%s", out_dir, "conformity.bin");
...
for (Nb = 0; Nb < NbBlock; Nb++) {
  ...
  if (strcmp(PolTypeOut,"T4")==0) T4_to_C4(M_avg, NligBlock[Nb], Sub_Ncol, 0, 0);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if ((strcmp(PolTypeOut,"T3")==0)||(strcmp(PolTypeOut,"C3")==0)) 
          M_out[lig][col] = (M_avg[C313_re][lig][col]-0.5*M_avg[C322][lig][col]) / (M_avg[C311][lig][col]+M_avg[C322][lig][col]+M_avg[C333][lig][col]);
        if ((strcmp(PolTypeOut,"T4")==0)||(strcmp(PolTypeOut,"C4")==0)) 
          M_out[lig][col] = (M_avg[C414_re][lig][col]-0.25*(M_avg[C422][lig][col]+M_avg[C433][lig][col]+2.*M_avg[C423_re][lig][col])) / (M_avg[C411][lig][col]+M_avg[C422][lig][col]+M_avg[C433][lig][col]+M_avg[C444][lig][col]);
        } else {
        M_out[lig][col] = 0.;
        }
      }
