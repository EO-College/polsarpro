...
Description :  Polarimetry Tomography Specrtum Estimation
...
void BF_Capon_Full_Rank_tomo_profile(float ****R_profile,float ****Tomo_FR_BF,float ****Tomo_FR_Capon,float *z_ax,int Nr, int Nim, int Npol, int Nz,float ***kz_vec, int Nmed);
void B_L(float **Tomo,float **a,float ***L, float *** mvecs,int Nim, int Npol,int BF_flag, float ****dum);
...
void write_tomo_cfg(char *out_dir_bf,char *out_dir_ca,int Nz,int Nx,float zmin, float zmax, int row_cut,sgeo_cfg *g_cfg);
void write_tomo_data(char *out_dir_bf,char *out_dir_ca,int Nz,int Nx,float ****Tomo_FR_BF,float ****Tomo_FR_Capon, float *DEM_profile, float *ztop_profile);
...
int main(int argc, char *argv[])
{
 ...
 char  out_dir_bf[FilePathLength], out_dir_ca[FilePathLength];
...
strcpy(UsageHelp,"\nTomo_NP_Spec_est.exe\n");
strcat(UsageHelp,"\nParameters:\n");
strcat(UsageHelp," (string)	-id  	input directory\n");
strcat(UsageHelp," (string)	-odbf	output beam former directory\n");
strcat(UsageHelp," (string)	-odca	output capon directory\n");
...
if(argc < 27) {
  ...
  get_commandline_prm(argc,argv,"-odbf",str_cmd_prm,out_dir_bf,1,UsageHelp);
  get_commandline_prm(argc,argv,"-odca",str_cmd_prm,out_dir_ca,1,UsageHelp);
...
********************************************************************/
  ...
  check_dir(in_dir);
  check_dir(out_dir_bf);
  check_dir(out_dir_ca);
  check_file(dem_file);
...
********************************************************************/
 ...
 Tomo_BF= matrix4d_float(Nr,Nz,Npol*(Npol+1)/2,2);
 Tomo_Capon= matrix4d_float(Nr,Nz,Npol*(Npol+1)/2,2);
 
 BF_Capon_Full_Rank_tomo_profile(R_profile,Tomo_BF,Tomo_Capon,z_ax,Nr,Nim,Npol,Nz,kz_strip,Nmed);
 
/********************************************************************
********************************************************************/

 write_tomo_cfg(out_dir_bf,out_dir_ca,Nz,Nr,z_ax[0],z_ax[Nz-1],row_cut,&g_cfg);
 write_tomo_data(out_dir_bf,out_dir_ca,Nz,Nr,Tomo_BF,Tomo_Capon,DEM_profile,ztop_profile);

/********************************************************************
********************************************************************/

 free_matrix4d_float(k_strip,Nr,Nc,Nim*Npol);
 /* free_matrix_float(DEM_strip,Nr); */
 /* free_vector_float(DEM_profile); */
 /* free_vector_float(ztop_profile); */
 free_matrix3d_float(kz_strip,Nr,Nc);
 free_matrix4d_float(R_profile,Nr,Nim*Npol,Nim*Npol);
 free_vector_float(z_ax);
 free_matrix4d_float(Tomo_BF,Nr,Nz,Npol*(Npol+1)/2);
 free_matrix4d_float(Tomo_Capon,Nr,Nz,Npol*(Npol+1)/2); 
...
void write_tomo_data(char *out_dir_bf,char *out_dir_ca,int Nz,int Nx,float ****Tomo_FR_BF,float ****Tomo_FR_Capon,float *DEM_profile, float *ztop_profile)
{
 ...
 ind_pol=0;
  ...
  for(r=0;r<3;r++) {
   ...
   sprintf(file_name,"%sT%d%d.bin",out_dir_bf,r+1,r+1);
   if ((file=fopen(file_name,"wb"))==NULL)
    edit_error("Could not open output file : ",file_name);
   for(ind_z=0;ind_z<Nz;ind_z++) {
    for(ind_x=0;ind_x<Nx;ind_x++)
     dum[ind_x]=Tomo_FR_BF[ind_x][Nz-1-ind_z][ind_pol][0];
    fwrite(&dum[0],sizeof(float),Nx,file);
    }
   fclose(file);

   sprintf(file_name,"%sT%d%d.bin",out_dir_ca,r+1,r+1);
   if ((file=fopen(file_name,"wb"))==NULL)
    edit_error("Could not open output file : ",file_name);
   for(ind_z=0;ind_z<Nz;ind_z++) {
    for(ind_x=0;ind_x<Nx;ind_x++)
     dum[ind_x]=Tomo_FR_Capon[ind_x][Nz-1-ind_z][ind_pol][0];
    fwrite(&dum[0],sizeof(float),Nx,file);
    }
   fclose(file);
   ...
   ind_pol++;
   for(c=r+1;c<3;c++) {
    sprintf(file_name,"%sT%d%d_real.bin",out_dir_bf,r+1,c+1);
    if ((file=fopen(file_name,"wb"))==NULL)
     edit_error("Could not open output file : ",file_name);
    for(ind_z=0;ind_z<Nz;ind_z++) {
     for(ind_x=0;ind_x<Nx;ind_x++)
    ...
    fclose(file);
    sprintf(file_name,"%sT%d%d_real.bin",out_dir_ca,r+1,c+1);
    if ((file=fopen(file_name,"wb"))==NULL)
     edit_error("Could not open output file : ",file_name);
    for(ind_z=0;ind_z<Nz;ind_z++) {
     for(ind_x=0;ind_x<Nx;ind_x++)
    ...
    fclose(file);
    sprintf(file_name,"%sT%d%d_imag.bin",out_dir_bf,r+1,c+1);
    if ((file=fopen(file_name,"wb"))==NULL)
     edit_error("Could not open output file : ",file_name);
    for(ind_z=0;ind_z<Nz;ind_z++) {
     for(ind_x=0;ind_x<Nx;ind_x++)
      dum[ind_x]=Tomo_FR_BF[ind_x][Nz-1-ind_z][ind_pol][1];
     fwrite(&dum[0],sizeof(float),Nx,file);
     }
    fclose(file);
    sprintf(file_name,"%sT%d%d_imag.bin",out_dir_ca,r+1,c+1);
    if ((file=fopen(file_name,"wb"))==NULL)
     edit_error("Could not open output file : ",file_name);
    for(ind_z=0;ind_z<Nz;ind_z++) {
     for(ind_x=0;ind_x<Nx;ind_x++)
      dum[ind_x]=Tomo_FR_Capon[ind_x][Nz-1-ind_z][ind_pol][1];
     fwrite(&dum[0],sizeof(float),Nx,file); 
     }
    fclose(file);
    ind_pol++;
  ...
  sprintf(file_name,"%sDEM_profile.bin",out_dir_bf);
  if ((file=fopen(file_name,"wb"))==NULL)
   edit_error("Could not open output file : ",file_name);
  fwrite(&DEM_profile[0],sizeof(float),Nx,file);
  fclose(file);
  sprintf(file_name,"%sz_top_profile.bin",out_dir_bf);
  if ((file=fopen(file_name,"wb"))==NULL)
   edit_error("Could not open output file : ",file_name);
  fwrite(&ztop_profile[0],sizeof(float),Nx,file);
  fclose(file);

  sprintf(file_name,"%sDEM_profile.bin",out_dir_ca);
  if ((file=fopen(file_name,"wb"))==NULL)
   edit_error("Could not open output file : ",file_name);
  fwrite(&DEM_profile[0],sizeof(float),Nx,file);
  fclose(file);
  sprintf(file_name,"%sz_top_profile.bin",out_dir_ca);
  if ((file=fopen(file_name,"wb"))==NULL)
   edit_error("Could not open output file : ",file_name);
  fwrite(&ztop_profile[0],sizeof(float),Nx,file);
  fclose(file);
...
void write_tomo_cfg(char *out_dir_bf,char *out_dir_ca,int Nz,int Nx,float zmin, float zmax, int row_cut,sgeo_cfg *g_cfg)
{
 ...
 //sprintf(file_name,"%s/Tomo_profiles",in_dir); mkdir(file_name,S_IRWXU);
 //sprintf(file_name,"%s/Tomo_profiles/BF",in_dir); mkdir(file_name,S_IRWXU);
 //sprintf(file_name,"%s/Tomo_profiles/BF/T3",in_dir); mkdir(file_name,S_IRWXU);
 //sprintf(file_name,"%s/Tomo_profiles/Capon",in_dir); mkdir(file_name,S_IRWXU);
 //sprintf(file_name,"%s/Tomo_profiles/Capon/T3",in_dir); mkdir(file_name,S_IRWXU);

 sprintf(file_name,"%sconfig.txt",out_dir_bf);
 ...
 sprintf(file_name,"%sconfig.txt",out_dir_ca);
...
/********************************************************************
********************************************************************/
void BF_Capon_Full_Rank_tomo_profile(float ****R_profile,float ****Tomo_FR_BF,float ****Tomo_FR_Capon,float *z_ax,int Nr, int Nim, int Npol, int Nz,float ***kz_strip,int Nmed)
{
 float ***L, ***iL,**a,***mvecs;
 float ****dum; 
 int i,indz,row;
 ...
 for(row=0;row<Nr;row++) {
  ...
  for(indz=0;indz<Nz;indz++)
  {
   ...
   B_L(Tomo_FR_BF[row][indz],a,L,mvecs,Nim,Npol,1,NULL);
   B_L(Tomo_FR_Capon[row][indz],a,iL,mvecs,Nim,Npol,0,dum); 
  }
 }
...
void B_L(float **Tomo,float **a,float ***L, float *** mvecs,int Nim, int Npol,int BF_flag,float ****dum)
{
 int pol1, pol2, i, j, ind_pol, istart,istop;
 ...
 for(pol1=0;pol1<Npol;pol1++) {
  for(j=pol1*Nim;j<(Npol*Nim);j++) {
    mvecs[pol1][j][0] = 0;
    mvecs[pol1][j][1] = 0;
    istop=my_min(j+1,(pol1+1)*Nim);
    for(i=pol1*Nim;i<istop;i++)
    {
     mvecs[pol1][j][0] +=  L[j][i][0]*a[i-pol1*Nim][0]-L[j][i][1]*a[i-pol1*Nim][1];
     mvecs[pol1][j][1] += -L[j][i][0]*a[i-pol1*Nim][1]-L[j][i][1]*a[i-pol1*Nim][0];
    }
   }
  }
 }
 ...
 if(BF_flag == 1) {
  ...
  for(pol1=0;pol1<Npol;pol1++) {
   for(pol2=pol1;pol2<Npol;pol2++) {
    dum[0][pol2][pol1][0] = 0;
    dum[0][pol2][pol1][1] = 0;
    for(j=Nim*pol1;j<Nim*Npol;j++)
    {
     dum[0][pol2][pol1][0] += mvecs[pol1][j][0]*mvecs[pol2][j][0]
      +mvecs[pol1][j][1]*mvecs[pol2][j][1];
     dum[0][pol2][pol1][1] -= - mvecs[pol1][j][0]*mvecs[pol2][j][1]
      +mvecs[pol1][j][1]*mvecs[pol2][j][0];
    }
   }
  }
  cmplx_chol_dec_L(dum[0],dum[1],Npol);
  cmplx_inv_tr_L(dum[1],dum[0],Npol);
  ...
  for(pol1=0;pol1<Npol;pol1++) {
   for(pol2=pol1;pol2<Npol;pol2++) {
    ...
    for(j=pol1;j<Npol;j++)
    {
     Tomo[ind_pol][0] += dum[0][j][pol1][0]*dum[0][j][pol2][0]
      +dum[0][j][pol1][1]*dum[0][j][pol2][1];
     Tomo[ind_pol][1] += dum[0][j][pol1][0]*dum[0][j][pol2][1]
      -dum[0][j][pol1][1]*dum[0][j][pol2][0];
    }
    ind_pol++;
    }
   }
...
********************************************************************/
float  ***read_kz_strip(int row_cut,int ind0,int Wr,int Wc,int Nr_im, int Nc_im,int Nim, char **im_dir)
//int read_kz_strip(int row_cut,int ind0,int Wr,int Wc,int Nr_im, int Nc_im,int Nim, char **im_dir)
{
...
********************************************************************/
float ****read_k_strip(int row_cut,int ind0,int Wr,int Wc,int *Nr, int *Nc,int *W, int Nim, int Npol,int Nr_im, int Nc_im, char **im_dir)
//int read_k_strip(int row_cut,int ind0,int Wr,int Wc,int *Nr, int *Nc,int *W, int Nim, int Npol,int Nr_im, int Nc_im, char **im_dir)
{
...
********************************************************************/
/*
int get_commandline_prm(int argc, char *argv[], char *keyword,
			cmd_prm type, void *ptr, int required, char *usage)
{
 int  i, found=0,missing_prm=0,is_arg,loop,count;
