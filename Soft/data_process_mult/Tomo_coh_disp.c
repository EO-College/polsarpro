...
void bmp_jet_gray(int Nr, int Nc, float **DataBmp, char *name);
...
int main (int argc,char *argv[])
{
 ...
 FILE   *file;
 char  file_name[FilePathLength],dem_file[FilePathLength];
 char  usage[FilePathLength],in_dir[FilePathLength],**im_dir;

 int Nim,Npol=Npolar_in,Nc,Nr,Nc_im,Nr_im;
 int row,col,pol,loop,im;
...
********************************************************************/
 ...
 R_to_coh_im(in_dir,R,Nr,Nc,Npol,Nim);
...
void bmp_jet_gray(int Nr, int Nc, float **DataBmp, char *name)
{
    ...
    extracol = (int) fmod(4 - (int) fmod(Nc, 4), 4);
    ncolbmp = Nc + extracol;

    bufimg = vector_char(Nr * ncolbmp);
...
/* Definition of the Header */
    ...
    headerRas(Nc, Nr, 1, 0, fbmp);
    #else
    header(Nr, Nc, 1, 0, fbmp);
    #endif
...
/* Bitmap colormap and BMP writing */
    ...
    fwrite(&bufimg[0], sizeof(char), Nr * ncolbmp, fbmp);
...
float ****filter_k_to_R(int Wr,int Wc,int Fr,int Fc,int *Nr, int *Nc, int Nim, int Npol,int Nr_im, int Nc_im, char *file_dem, char **im_dir,int comp_dem)
{
 int row,col,im,loop,r,c,ind,ind_pol,Rr,Rc;
 int N;
 int *rind,*cind;
 float phi,dum[2],dd[2];
 float **k,****Rps,*dem,*kz;
 char file_name[FilePathLength];
 FILE **in_file;
 FILE **kz_file;
