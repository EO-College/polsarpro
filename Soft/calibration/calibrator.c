...
/* Internal variables */
  ...
  int ii, Rmax, Rmax2, Rmax3;
  int arret, R3db, R6db, R9db;

  float Amplimax, Amplimaxdb;
  float mu, Resolr, SSLRresolr, Dr, Drbis, DX, DY;
  float Rampli3db, Rampli6db, Rampli9db;
  float EcartR3db, EcartR6db, EcartR9db;
...
Rampli3db= Vector[R3db];
Resolr=((abs(Rmax-R3db))*Dr*Nfft/NfftInterp);
EcartR3db=Amplimaxdb-20*log10(Rampli3db);
...
Rampli6db= Vector[R6db];
Resolr=((abs(Rmax-R6db))*Dr*Nfft/NfftInterp);
EcartR6db=Amplimaxdb-20*log10(Rampli6db);
...
Rampli9db= Vector[R9db];
Resolr=((abs(Rmax-R9db))*Dr*Nfft/NfftInterp);
EcartR9db=Amplimaxdb-20*log10(Rampli9db);
