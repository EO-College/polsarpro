...
/* LOCAL VARIABLES */
  int Config;
  char *PolTypeConf[NPolType] = {"T4"};
  FILE *out_file;
  char file_name[FilePathLength], Format[10], Type[10];
...
strcat(UsageHelp," (string)	-type	Master = T11 - Slave = T22\n");
...
if(argc < 21) {
  ...
  get_commandline_prm(argc,argv,"-type",str_cmd_prm,Type,1,UsageHelp);
...
/* OUTPUT FILE OPENING*/
  if (strcmp(Format, "lin") == 0) sprintf(file_name, "%sspan%s.bin", out_dir,Type);
  if (strcmp(Format, "db") == 0) sprintf(file_name, "%sspan%s_db.bin", out_dir,Type);
    ...
    for (col = 0; col < Sub_Ncol; col++) {
      if (Valid[lig][col] == 1.) {
        if (strcmp(Type,"T11")==0) {
          M_out[lig][col] = M_in[T411][lig][col]+M_in[T422][lig][col];
          } 
        if (strcmp(Type,"T22")==0) {
          M_out[lig][col] = M_in[T433][lig][col]+M_in[T444][lig][col];
          } 
