...
File   : loci_cmplx_plane_extract_T4.c
...
if (argc == 6) {
  ...
  edit_error("loci_cmplx_plane_extract_T4 In_Dir File_Txt FileLine_Txt FileLoci_Txt FileTriplet_Txt\n","");
...
if (strcmp(Operation, "extract") == 0) {
  ...
  T_in = matrix3d_float(16,Length+1,Length+1);
  ...
  free_matrix3d_float(T_in,16,Length+1);
...
CurrentPointerPosition = ftell(in_file[GHigh]);
fseek(in_file[GHigh], (PointerPosition - CurrentPointerPosition), SEEK_CUR);
CurrentPointerPosition = ftell(in_file[GLow]);
fseek(in_file[GLow], (PointerPosition - CurrentPointerPosition), SEEK_CUR);
CurrentPointerPosition = ftell(in_file[Topo]);
fseek(in_file[Topo], (PointerPosition - CurrentPointerPosition), SEEK_CUR);

PointerPosition =  ((PixLig - (int)((Length-1)/2))* Ncol) * sizeof(float);
for (np=0; np < 16; np++) {
  CurrentPointerPosition = ftell(in_fileT[np]);
  fseek(in_fileT[np], (PointerPosition - CurrentPointerPosition), SEEK_CUR);
  }
}
/********************************************************************
  Routine  : ExtractData
...
for(k=0; k<Length; k++) {
  for(l=0; l<Length; l++) {
  for(i=0; i<16; i++) Mean[i] += T_in[i][k][l];
  }
  }
